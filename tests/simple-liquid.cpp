...
int main(int, char **argv) {
  ...
  Downhill downhill(f, &potential, 1e-8);
  potential.setZero();
  retval += test_minimizer("Downhill", &downhill, &potential, 3000);

  PreconditionedDownhill pd(f, &potential, 1e-14);
  potential.setZero();
  retval += test_minimizer("PreconditionedDownhill", &pd, &potential, 500);

  SteepestDescent steepest(f, &potential, QuadraticLineMinimizer, 1e-8);
  potential.setZero();
  retval += test_minimizer("SteepestDescent", &steepest, &potential, 2100);

  PreconditionedSteepestDescent psd(f, &potential, QuadraticLineMinimizer, 1e-14);
  potential.setZero();
  retval += test_minimizer("PreconditionedSteepestDescent", &psd, &potential, 400);

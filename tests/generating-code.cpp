...
int main(int, char **) {
  ...
  const double Rval = 4;
  Functional R(Rval, "R");
  (kT.set_name("temp") + x).create_header("tests/generated/sum.h", "Sum");
  (sqr(kT + x) - x + 2*kT).create_header("tests/generated/quadratic.h", "Quadratic");
  sqrt(x).create_header("tests/generated/sqrt.h", "Sqrt");
  (CallMe(sqrt(x), "Sqrt", "()") - x + 2*kT).create_header("tests/generated/sqrt-and-more.h", "SqrtAndMore");
  ...
  sqr(veff).create_header("tests/generated/sqr-Veff.h", "SquareVeff", "R");
  ...
  phi1.set_name("phi1").create_header("tests/generated/phi1.h", "Phi1", "R");

  phi1(veff).create_header("tests/generated/phi1-Veff.h", "Phi1Veff", "R");
  ...
  phi2.set_name("phi2").create_header("tests/generated/phi2.h", "Phi2", "R");

  phi2(veff).create_header("tests/generated/phi2-Veff.h", "Phi2Veff", "R");

  Functional phi3rf = n2*(sqr(n2) - 3*(sqr(n2x) + sqr(n2y) + sqr(n2z)))/(24*M_PI*sqr(one_minus_n3));
  phi3rf.set_name("phi3rf").create_header("tests/generated/phi3rf.h", "Phi3rf", "R");

  phi3rf(veff).create_header("tests/generated/phi3rf-Veff.h", "Phi3rfVeff", "R");
  ...
  ((kT*phi1).set_name("phi1")+(kT*phi2).set_name("phi2")+(kT*phi3rf).set_name("phi3")).create_header("tests/generated/almostrf.h", "AlmostRF", "R");

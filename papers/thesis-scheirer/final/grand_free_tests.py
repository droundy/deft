...
##nL1 = [data2[i][1] for i in range(0,len(data2))]           # list of the left number density solutions obtained from fsolve
##nR1 = [data2[i][2] for i in range(0,len(data2))]           # list of the right number density solutions obtained from fsolve
##Tlist1 = [data2[i][0] for i in range(0,len(data2))]        # list of the corresponding temperatures for which the above number densitites were found
##
...
for p in range(50,len(T)):
        ...
        def fload():
                ...
                fname = 'RG_final_data/09_250Ts/data2/fit_T%.16g_f5.out'%temp
                #print fname," %.17g"%temp
                if(os.path.isfile(fname)==False):
                        fname = 'RG_final_data2/09_250Ts/data/fit_T%.16g_f5.out'%(temp+1e-16)
                if(os.path.isfile(fname)==False):
                        fname = 'RG_final_data2/09_250Ts/data/fit_T%.16g_f5.out'%(temp-1e-16)
                if(os.path.isfile(fname)==False):
                        fname='RG_final_data2/09_250Ts/data/fit_T%.15g_f5.out'%temp
                if(os.path.isfile(fname)==False):
                        fname='RG_final_data2/09_250Ts/data/fit_T%.15g_f5.out'%(temp+1e-15)
                if(os.path.isfile(fname)==False):
                        fname='RG_final_data2/09_250Ts/data/fit_T%.15g_f5.out'%(temp-1e-15)
                if(os.path.isfile(fname)==False):
                        fname='RG_final_data2/09_250Ts/data/fit_T%.15g_f5.out'%(temp+1e-14)
                if(os.path.isfile(fname)==False):
                        fname='RG_final_data2/09_250Ts/data/fit_T%.15g_f5.out'%(temp-1e-14)
                if(os.path.isfile(fname)==False):
                        fname='RG_final_data2/09_250Ts/data/fit_T%.15g_f5.out'%(temp+1e-13)
                if(os.path.isfile(fname)==False):
                        fname='RG_final_data2/09_250Ts/data/fit_T%.15g_f5.out'%(temp-1e-13)

...
def integrand_ID(T,x,i):
    ...
    if np.isnan(argument):
        print '   NAN!',argument,fbarDvalue,ubarDvalue
        darn_nans
#    return np.exp(-VD(i)/k_B/T*(fbarD(T,x,i) + ubarD(T,x,i)))
    return argument
...
#    value = (frecursive(T,nL*(1+x),i-1) + frecursive(T,nL*(1-x),i-1))/2 - frecursive(T,nL,i-1)
    iplusx = frecursive(T,nL*(1+x),i-1)
    iminusx = frecursive(T,nL*(1-x),i-1)
    nochangex = frecursive(T,nL,i-1)
    value = (iplusx + iminusx)/2 - nochangex
    print 'fbarD'
    if np.isnan(value):
        print '   NAN!',iplusx,iminusx,nochangex
        print '  x is',x
        darn_nans
    return value

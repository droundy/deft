...
energy for fluids that are chain polymers. In our model, we use a hard
sphere reference fluid which renders $F_\textit{chain}$ irrelivant for
our functional.

In a recent paper \cite{hughes2013classical}, Hughes \emph{et al}
constructed a classical density functional theory for water which used
Clark \emph{et al}'s \cite{clark2006developing} five fitting
paramenters for an optimal cDFT for water.  In addition, they
introduced a parameter for the length scale over which the dispersion
interaction is correlated

\section{Method}

Schulte \emph{et al}\cite{schulte2012using} derived expressions for
the correlation fucntoin at contact based on the White Bear version of
FMT. They found their $g_{\sigma}^A$ to agree with Monte Carlo
simulation than Yu and Wu's which we was used in Hughes \emph{et al}'s
paper.  We use Hughs \emph{et al}'s functional and replace the
correlation with Schulte \emph{et al}'s, which we will call
$g_{\sigma}$. \fixme{expand this}.
...
\end{center}
\caption{ The average number of hydrogen bonding per molecule at
  distances from the center of a hydrophobic rod for various
  radii. The solid lines are the results from using the older
  $g_{\sigma}$, while the dotted lines are from Schulte \emph{et al}'s
  asymmetric $g_{\sigma}$.}
...
\begin{figure}
\begin{center}
\includegraphics[width=3.5in]{figs/density-single-rod}
\end{center}

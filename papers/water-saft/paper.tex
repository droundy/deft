...
\fixme{Some stuff about water}.  An important aspects of water is its
ability to form hydrogen bonds.  Each water molecule contains two
hydrogen bond donor sites and two acceptor sites.
...
Wertheim's thermodynamic perturbation theory \fixme{expand/correct this}

In a recent paper \cite{hughes2013classical}, Hughes \emph{et al}
constructed a classical DFT for water which uses SAFT as a basis for
their free energy functional.  Hughes employed Clark \emph{et al}'s
\cite{clark2006developing} five fitting parameters for an optimal cDFT
for water and, in addition, introduces a parameter for the length
scale over which the dispersion interaction is correlated.  Clark
found that the four association site model works best to describe
water, which is physically accurate since each water molecule has four
hydrogen bonding sites.

This functional was successful in \fixme{list stuff}. However, the
correlation function for hard spheres from Yu and
Wu\cite{yu2002fmt-dft-inhomogeneous-associating} that is used in the
association free energy term was \fixme{untested}.

Schulte \emph{et al}\cite{schulte2012using} derived two approximate
expressions for the correlation function at contact for and
inhomogeneous hard sphere fluid based on the White Bear version of
fundamental measure theory (FMT). One of Schulte's expression found
the average number of spheres touching a point $\rr$ and is referred
to as a ``symmetric'' correlation functoin, $g_\sigma^S$ . The other
expression sets $\rr$ at the center of a sphere and find the average
spheres touching the solvent accessable surface, which could be
considered an ``asymmetric'' correlation function, $g_{\sigma}^A$. The
asymmetric function agreed closely with Monte Carlo simulation, and
much better than Yu and Wu's.

\section{Method}

We modify Hughes \emph{et al}'s method of creating a classical DFT
based on SAFT using Clark's four site model with five fitting
parameters and the added fitting parameter for the correlation
distance of the dispersion interaction.  We modify Hughes's
association free energy term to use Schulte's $g_{\sigma}^A$ rather
than Yu and Wu's.  The association free energy for our four-site model
has the form \fixme{I copy/pasted and modified these equations, but
  they still require a number of other equations to come before.
  Maybe only show the $g_\sigma^A$?}
\begin{align}
...
\begin{align}
  ...
  g^\textit{SW}_\sigma(\xx) &= g^\textit{A}_\sigma(\xx) +
...
correlation function $g^\textit{A}_\sigma$, for which we use the
functional of Schulte \emph{et al}\cite{schulte2012using}:
\begin{align}
  g_\sigma^{A}(\rr) &= \frac{1}{n(\rr)n_A(\rr)}\frac{1}{k_B T 4 \pi
    (2R)^2}\frac{\delta F_{hs}}{\delta R(\rr)} 
\end{align}
where the $n_A(\rr)$ is the density of spheres that are touching a
sphere that is located at point $\rr$. The modification to Hughes's
functional appers here where we have replaced $g_\sigma^{HS}$ with
$g_\sigma^A$. \fixme{The equation for $g_\sigma^A$ requires finding
  convolutions of local derivative of the free energy, aking this
  formulation computationally more expensive than the free energy
  itself.}
...
of Hughes in Fig. \ref{fig:density-single-rod}.  For small rods, the
water densities are attenuated near the rods for our functional as
compared to Hughes's.   Since we have changed the association term in
the functional, a noticeable change might occur in the aveerage number
of hydrogen bonds in a given system.  We compare the hydrogen bonds
with the new and old functional for rods of a few different radii
(Fig. \ref{fig:single-rod-X}) and find that there is a very noticeable
difference.  This is not suprising since $g_\sigma^A$ is antisymmetric
and $g\sigma^{HS}$ is symmetric.  \fixme{Physical description for why
  it looks that way}
...
\fixme{More stuff here for spheres}

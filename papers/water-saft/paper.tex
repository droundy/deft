...
\fixme{Talk about Wertheim's TPT}

In a recent paper \cite{hughes2013classical}, Hughes \emph{et al}
constructed a classical density functional theory for water which uses
SAFT as a basis for their free energy functional.  Hughes employed Clark
\emph{et al}'s \cite{clark2006developing} five fitting parameters for
an optimal cDFT for water and, in addition, introduces a parameter for
the length scale over which the dispersion interaction is correlated.
Clark found that the four association site model works best to
describe water, which is physically accurate since each water molecule
has four hydrogen bonding sites.

Schulte \emph{et al}\cite{schulte2012using} derived two expressions
for the correlation function at contact for inhomogeneous fluids based
on the White Bear version of fundamental measure theory (FMT). One
expression was labeled as ``symmetric'' and found the average number
of spheres touching a point $\rr$. The other expression, labeled
``assymetric,'' sets $\rr$ at the center of a sphere, and finds the
average spheres touching the solvent accessable surface.  They found
the asymmetric correlation function, $g_{\sigma}^A$, to agree
closely with Monte Carlo simulation.
...
distance of the dispersion interaction.  Hughe's association free
energy term is modified to use Schulte's $g_{\sigma}^A$ rather than Yu
and Wu's.  The association free energy for our four-site model has the
form \fixme{I copy/pasted and modified these equations, but they still
  require a number of other equations to come before.  Maybe only show
  the $g_\sigma^A$?}
\begin{align}
...
sphere that is located at point $\rr$.\fixme{The equation for
$g_\sigma^A$ requires finidng convolutions of local derivative of
the free energy, aking this formulation computationally more
expensive than the free energy itself.}

We used Clark's five empirical parameters, and fit the calculated
surface tension to experimental surface tension at ambient
conditions.  This fit was found by choosing the value of the
$\lscale$, which we found to be 0.454 for this new functional.
...
\caption{Comparison of Surface tension versus temperature for theoretical and
  ...
  The length-scaling parameter $s_d$ is fit so that the theoretical surface 
...
\caption{ Density profiles for single rods of different
  radii from old and new functionals.}
...
We use X from eq. \ref{eq:X} above to find the average number of
hydrogen bonds per molecule as a function of distance for different
radius hydrophobic rods.  Figure \ref{fig:single-rod-X} compares our
new functional with Hughes's functional, \fixme{Physical description
  for why it looks that way}

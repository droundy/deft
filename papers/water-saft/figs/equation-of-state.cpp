...
int main(int, char **) {
  ...
  if (o == NULL) {
    printf("Unable to open equation-of-state.dat file.  :(\n");
    exit(1);
  }
  FILE *experiment = fopen("papers/water-saft/figs/experimental-equation-of-state.dat", "w");
  if (experiment == NULL) {
    printf("Unable to open experimental-equation-of-state.dat file.  :(\n");
    exit(1);
  }
  int imax=0;
  ...
  Functional f = OfEffectivePotential(WaterSaft(water_prop.lengthscale,

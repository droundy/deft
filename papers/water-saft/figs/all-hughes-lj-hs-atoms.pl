...
foreach $a ( "Xe" ) { #"Ne", "Ar", "Kr",  "Xe" ) {
  foreach $t ( 275, 300, 325, 350 ) {
    ...
    my $memuse = 22000; #sprintf "%.0f", 2750*((($d + 2*$padding)/1.9)*(0.2/$resolution))**3;

    my $scriptname = "papers/water-saft/figs/hughes-lj-$atom-$temperature-HS.tmp.sh";
    open SCRIPT, ">$scriptname" or die $!;
    print SCRIPT "#!/bin/sh
#SBATCH --mem-per-cpu=$memuse
##SBATCH --mail-type ALL
##SBATCH --mail-user daveroundy\@gmail.com
#SBATCH --output hughes-lj-$a-$t-HS.out
...
time nice -19 papers/water-saft/figs/hughes-lj-atom-hs-density.mkdat $temperature

...
%% \end{align}
\pahosays{Was plotting the radial distribution function meant to be commented?}
%% \begin{figure}
%%   \centering
...
\newcommand\paircaption{\pahosays{The MC has one point that jumps up at the
    beginning of the incontact region.} The pair distribution function near a
  hard wall, with packing fractions of 0.1 and 0.3 and $\rr_1$ in contact with
  the hard wall.  On the left is a 2D plot of $g^{(2)}(\rr_1,\rr_2)$ as $\rr_2$
  varies.  The top half of this figure shows the result of the Monte Carlo
  simulation, while the bottom half shows the result of the appproximation
  developed in this work.  On the right is a plot of $g^{(2)}(\rr_1,\rr_2)$ on
  the path illustrated in the figure to the left.  This plot compares the Monte
  Carlo results (black) with those of this work (blue), Sokolowski and Fischer
...
\begin{figure*}
  ...
  \caption{\fixme{Remove one of, or make a note of, the two curves for this
      ...
      for this work mc?}The triplet distribution function

...
to inhomogeneous scenarios.  Work on these approximations date back to
Toxvaerd \cite{toxvaerd1973statistical} and have been tested
rigorously for both functional and non-functional based theories
\cite{kalos1977structure,osborn1980monotonic, mccoy1981comparison}.
The approximations differ in what density is used when evaluating the
radial distribution function $g(r,\eta)$ or in how radial
distributions evaluated at different densities are combined to yield
an effective function \cite{toxvaerd1973statistical}.

One common approximation \cite{gloor2007prediction, gross2009density,
  osborn1980monotonic, mccoy1981comparison,barrett2006some} assumes a
mean-density approximation for the pair distribution function:
\begin{align}
...
Another approximation uses $g(r,\eta)$ and assumes the density
evaulated at the position between the two points:
\begin{align}
  g^{(2)}(\rr_1,\rr_2) \approx g\left(r_{12}, \tfrac{\pi\sigma^3}{6}
  n\left(\tfrac12(\rr_1+\rr_2)\right)\right)
\end{align}

The above two approaches cannot be applied to highly inhomogeneous
systems such as a dense fluid at a solid surface.  These systems
exhibit large density oscillations that lead to mean packing fractions
greater than unity, which cannot occur in the bulk reference system
that defines $g(r, \eta)$.  The above papers restricted themselves to
the liquid-vapor interface, which does not exhibit this pathology.
...
$\rr_1$ and $\rr_2$ is the hard-sphere diameter.  \davidsays{Tang does something similar, but maybe different.\cite{tang1991density}}

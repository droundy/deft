...
\newcommand\kappaO{c_0}
\newcommand\kappaI{c_1}
\newcommand\kappaZ{c_2}
\begin{multline}
  g(r,g_\sigma) = 1 + h_\sigma e^{-\kappaO \left(\frac{r}{\sigma}-1\right)} \\
  + h_\sigma(\kappaO - g_\sigma)  \left(\frac{r}{\sigma}-1\right)e^{-\kappaI  \left(\frac{r}{\sigma}-1\right)} \\
  + B  \left(\frac{r}{\sigma}-1\right)^2e^{-\kappaZ  \left(\frac{r}{\sigma}-1\right)}
\end{multline}
\begin{equation}
  B = \kappaZ^5 \frac{
...
\end{equation}
\davidsays{The above is a sketch of how we might want to write our
  equations.  I think it matches up with the python now.  Below is the
old (presumably wrong) version.}

\begin{multline}
  h(r,g_\sigma) = 1 + h_\sigma e^{-\kappaO z} \\
  + h_\sigma(\kappaO - 2 g_\sigma) \xi e^{-\kappaI \xi} \\
  + \frac{\frac{(1-\eta)^4}{1+4\eta + 4\eta^2 - 4\eta^3 +
      \eta^4}\frac{1}{n}
    - I_0  -I_1(\kappaO - g_\sigma)}{I_2} z^2e^{-\kappaZ z}
\end{multline}
\begin{equation}
  I_0 = \frac{4\pi h_\sigma (2 + \sigma \kappaO (2 + \sigma \kappaO))}{\kappaO^3}
  - \frac{4\pi}{3} \sigma^3
\end{equation}
\begin{equation}
  I_1 = \frac{4\pi h_\sigma (6 + \sigma \kappaI (4 + \sigma \kappaI))}{\kappaI^4}
\end{equation}
\begin{equation}
  I_2 = \frac{8\pi (12 + \sigma \kappaZ (6 + \sigma \kappaZ))}{\kappaZ^5}
\end{equation}
\begin{equation}
  z = r - \sigma
\end{equation}
...
This function includes three fitted parameters, $\kappaO$, $\kappaI$,
and $\kappaZ$, which are given in Table~\ref{tab:fit-parameters}.

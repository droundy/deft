...
both versions of our pair distribution approximation \jeffsays{Is it
  still both? Do we want that?} with two previously published works
and Monte-Carlo simulation, showing favorable results.
\end{abstract}
...
over a wide range of densities.  \jeffsays{We had a clarifying
  paragraph below but I don't think it's needed anymore, at least not
  here.  The explanation up until this point is moving along fine and
...
\jeffsays{In my current version latex's formatting of this and the
  next section is very poor so we should keep our eye on it as the
  text develops} In the approaches for the pair distribution function
...
in consturcting and evaluating the approximations of earlier works.
\jeffsays{With this last sentence: it states that we 'first' use the
  Monte Carlo version for our radial distribution but then doesn't say
  what we use afterward.}
...
\jeffsays{The rest of the fitting needs to be explained, and
  discussing the connection with Gill-Viegas}
...
\jeffsays{I've noted this in the penultimate section as well but I
  think it might be good to include that section after this one.
  Where that section is now, I feel like it seems we think it's less
  important.  This is subjective, though.}
...
\jeffsays{I think this ought to be included after the Seperable Fit
  secction.  It may seem more important that way.}

...
\begin{abstract}
  ...
  \fixme{add acronym, and discuss separable vs. non}
\end{abstract}
...
the ``contact value approximation'' (CVA).  \fixme{Should we mention
  this earlier?}
...
The contact value approximation (described in
Eq.~\ref{eq:g2-our-mean}) is reasonably accurate, but cannot be
computed efficiently with single-center convolutions as written.  In
order to ease computational application of this pair distribution
function in classical density functionals, we have developed a
separable appoximation for the function $g(r,g_\sigma)$.
\begin{align}
...
efficiently computed using fast Fourier transforms.  \fixme{Comb paper
  for places to say CVA-S and CVA.}

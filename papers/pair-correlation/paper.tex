...
\fixme{replace approximation with approach}
...
\fixme{Somewhere above introduce the question of whether g2 is
  expressed in terms of V or n}

\section{Previous theoretical approaches $g^{(2)}(\rr_1,\rr_2)[V(\rr)]$}

\fixme{introduce this as methods given V}
...
\fixme{introduce this as methods given n}
...
been used by two papers to do
\fixme{what?}~\cite{wadewitz2000application, winkelmann2001liquid}.
...
\davidsays{Please edit!}\fixme{Note that the HS free energy is used
...
\fixme{Explain why the separable version is efficient.}

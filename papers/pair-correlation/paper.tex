...
\\
  ...
  \caption{The triplet distribution function
    ...
    curve indicating the path direction, as explained in the
    text.} \label{fig:triplet-contact-distribution}
\end{figure*}
...
function may break the symmetry which is present in the true triplet
...
reasonable agreement, with flatter oscillations in the CVA-S at larger
distances. Also, the Monte Carlo results have perfect left-right
symmetry, while the CVA-S has an asymmetry introduced with
the test particle due to imperfections in the pair distribution
...
Figures~\ref{fig:triplet-contact-distribution}a
and~\ref{fig:triplet-contact-distribution}c. As the theoretical
results are asymmetric, they are also plotted along similar paths that
travel to the left, along the test particle. The different paths are
indicated by arrows along the curves, with right-facing arrows
...
indicating paths traveling along the test particle.  As the work of
Fischer and Methfessel is only defined in contact, it is only plotted
in Figure~\ref{fig:triplet-contact-distribution}b and only along the
portion of the path shown that is in contact with $\rr_2$, and arrows
are omitted. Again, the CVA-S underestimates the oscillations,
especially in contact with the test particle when the points $\rr_1$
and $\rr_2$ are separated by a distance of $2.1\sigma$, as seen in
Figure~\ref{fig:triplet-contact-distribution}d. Otherwise, all methods
perform similarly, with the method of Fischer and Methfessel giving
the best results when the spheres at $\rr_1$ and $\rr_2$ are in
contact.

...
#include "short-range-ghs-analytics.h"
...
double pairdist_this_work_short(const Grid &gsigma, const Grid &density, const Grid &nA, const Grid &n3, const Grid &nbar_sokolowski, Cartesian r0, Cartesian r1) {
  const Cartesian r01 = Cartesian(r0 - r1);
  const double r = sqrt(r01.dot(r01));
...
void run_walls(double eta, const char *name, Functional fhs, bool run_dadz) {
...
int main(int, char **) {
  ...
  bool run_dadz;
  for (int i = 1; i <= 4; i ++) {
    double this_eta = i*0.1;
    if (i == 3) run_dadz = true;
    else run_dadz = false;
    printf("eta: %f, run: %i\n", this_eta, run_dadz);
    run_walls(this_eta, "WB", WB, run_dadz);
  }

...
def switch_to_zeta(e):
  return e.subs(v['xi'], 2*v['zeta']).subs(v['halfkappa_0'], v['kappa_0']/2).subs(v['halfkappa_1'], v['kappa_1']/2).subs(v['halfkappa_2'], v['kappa_2']/2)

# define variables / constants
variables = ['r', 'h_sigma', 'g_sigma']
positive_variables = ['halfkappa_0', 'halfkappa_1', 'halfkappa_2']
...
expressions = [
  ('g_HS', lambda: switch_to_zeta(1 + v['h_sigma']*exp(-v['halfkappa_0']*v['xi']) + v['h_sigma']*(2*v['halfkappa_0'] - 2*v['g_sigma'])*v['xi']/2*exp(-v['halfkappa_1']*v['xi']) + v['B']/4*v['xi']**2*exp(-v['halfkappa_2']*v['xi']))),
  ('B', lambda: switch_to_zeta(4*(v['kappa_2']**5/32/((v['kappa_2']**2/4 + 3*v['kappa_2']/2 + 3)))*((-1 + v['chi'])/(24*v['eta']) -
  ...
  ('kappa_0', lambda: (v['halfkappa_0']*2)),
  ('kappa_1', lambda: (v['halfkappa_1']*2)),
  ('kappa_2', lambda: (v['halfkappa_2']*2)),
  ('R', lambda: v['sigma']/2),
...
# now that we have ghs defined, we want to do a best fit to find halfkappa_0, halfkappa_1, and halfkappa_2
lam = sympy.utilities.lambdify(('halfkappa_0', 'halfkappa_1', 'halfkappa_2', 'eta', 'r'),
...
c_code += r"""
const double halfkappa_0 = %g;
const double halfkappa_1 = %g;
const double halfkappa_2 = %g;

...
$\eta = 0.3$. We also explore various translation scales $\delta_0 = 0.005R$ and
$0.05R$. The simulations explore the
...
%\begin{figure}
  ...
  %\caption{The average entropy error for each method is shown for $\eta = 0.3$ with
  %SAMC followed by TMMC serving as the reference system.}\label{fig:N50-ff0.3-avg-error-fast}
%\end{figure}
...
$\delta_0 = 0.05R$). For the $\eta = 30\%$ and $N = 50$ system, we
explore the choice of $\delta_0$ as a parameter.

{\color{red}
$S_{\max} = 120$ and $S_{\min} = 265$ for this system.  The number of
...
converged for $\delta = 0.05R$ do not proportionaly converge for a
translation scale of $\delta = 0.005R$. We see that SAD handles
...
translation scale $\delta_0 = 0.05R$).
...
convergence for the $N = 50$ system with $\delta = 0.05R$ do not

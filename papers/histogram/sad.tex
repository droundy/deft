...
\begin{align}
\Delta S_{\text{tot}} = \int_{t_L}^{t_{\min}} \gamma dt \approx t_L + t_L \ln\left(\frac{t_{\min}}{t_L}\right) \\
+ N_S t_L \Delta S_{\text{tot}} \left(\frac{1}{t_{\min}}-\frac{1}{t_L}\right) 
\end{align}
...
\begin{align}
\Delta S_{\text{tot}} = \Delta S_{\text{SAMC}} + N_S t_L \Delta S_{\text{tot}} \left(\frac{1}{t_{\min}}-\frac{1}{t_L}\right)
\end{align}
Solving for the minimum time necessary $t_{\min}$, we find that
\begin{align}
t_{\min} = t_L \left( \frac{N_S \Delta S_{\text{tot}}}{\Delta S_{\text{tot}} + \Delta S_{\text{SAMC}}} + 1\right)
\end{align}
Immediately we see that the minimum time necessary is roughly proportional
to the last time the simulation has found an energy times a ratio of entropies.

\jpsays{Are these right??? Check!}

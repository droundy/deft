...
\davidsays{Edit this paragraph: introduce novelty of square well fluid
in particular, and importance.}
...
Monte Carlo algorithms~\davidsays{add a few citations}.  The LJ31
...
barrier between the two low-energy states~\davidsays{add citations}.
In particular, the LJ31 cluster features a solid-solid phase
transition at a temperature of around $T=0.03\epsilon/k_B$ that is
challenging to converge~\davidsays{cite}.  Poulain \emph{et al.}
tested a variety of variants of WL on LJ31 and found
...
Figure~\ref{fig:lj-cv} shows the heat capacity of the LJ31 cluster as
a function of temperature.  There are two peaks, a large peak at 0.2?
corresponding to melting temperature, and a small peak around 0.03 for
the solid-solid transition from a mackay to antimackay
wahatever~\davidsays{cite}.  We keep our atoms in a spherical box with
radius 2.5$\sigma$, as others do~\davidsays{cite}.  We find that using
a larger box (with radius $5\sigma$) has a large effect on the melting
peak, and a smaller but still significant effect on the solid-solid
transition.  Since our focus is on the convergence rate, we focused on
the smaller box size.  We further restricted our simulations to
negative energies, as the microcanonical energy at infinite
temperature is rather high for such a small box.

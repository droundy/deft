...
Wang and Landau introduced an algorithm (WL) that uses an update factor
...
requires the user to select a number of parameters, including the range
of energies to be studied~\cite{landau2004new}. This adds an additional
hurdle to its application to a variety of systems because the user must
simulate until the energy range of interest is known.  The simulation
...
Liang, Liu, and Carrol began to consider that WL could be considered
...
does not require an array of non-physical, user-defined inputs and
therefore should be easily applicable to a given system. The method
does require the user to define a temperature range ($T_\text{min}$ to $T_\infty$).
We call this method SAD (Stochastic Approximation with a dynamic update
factor), and will discuss it in detail in the methods section. We
compare it along with several flat histogram methods which include WL,
$1/t$-WL, and SAMC.
...
%describe in detail flat-histogram methods.
...
works.  The following methods are discussed and simulated for the
...
which change the system and should satisfy detailed balance.  Each
algorithm differs in how it determines the probability of accepting a
move and in what additional statistics must be collected in
order decide on that probability.

Flat histogram methods calculate
the density of states $D(E)$ for a discrete set of
energies~\cite{haber2018performance}. For this reason, energy binning
becomes an important consideration for systems with a continuum of
energies possible.
...
In this section we will introduce four related flat histogram methods
which each rely on a weight function $w(E)$, noting that the method proposed in
...
this violation decreases as we decrease $\gamma$.  The various
weight-based methods discussed here differ in how they schedule the
decreasing of $\gamma$.
...
error in the density of states beyond a given nonzero value.
...
that enforced that if the update factor $\gamma$ is less than
$\nicefrac{N_S}{t}$ then the update factor is set to
$\nicefrac{N_S}{t}$ where $N_S$ is the number of energy states and the
histogram is no longer tracked. Employing this scheduler, they found
that the error saturation is avoided since the correct density of
...
liang2007stochastic}.  In addition, the energy range need not be known
a priori.  The time to converge depends only on the choice of parameter
$t_0$.  Unfortunately, there is no prescription for finding an
acceptable value for $t_0$, and while the algorithm formally converges,
for a poor choice of $t_0$ that convergence can be far too slow to be
practical. Liang et al. give a rule of thumb in which $t_0$ is chosen
in the range from $2N_S$ to $100N_S$ where $N_S$ is the number of
energy bins~\cite{liang2007stochastic}.  Schneider et al. found that
for the Ising model this heuristic is helpful for small spin systems,
but that larger systems require an even higher $t_0$
value~\cite{schneider2017convergence}.  Also we find that for one case
with a small displacement distance for the square-well fluid, $t_0$
needs to be as much as two orders of magnitude higher than the rule of
thumb of $100N_S$ in order to converge in $10^{12}$ moves.
...
A primary difficulty in using the SAMC method lies in identifying an appropriate
value for $t_0$.  A value that is either too high or too low will result
in slow convergence to the true entropy of the system.  It is instructive to consider
...
factor could introduce errors in entropy that may take many iterations
...
that energy binning is handled dynamically. SAD should perform
independently for a reasonable range of binning because $\gamma \propto
\nicefrac{N_S}{t}$.  As the number of energy states $N_S$ found
increases (fine binning), the time spent $t$ in each bin will decrease
with the effect that the convergence should be roughly independent of
the bin size chosen.
This argument explains why SAD should perform well for continuum systems
in terms of binning. SAMC has a prefactor $\gamma_0$ to aid in a
similar way~\cite{werlich2015stochastic} but this adds yet another parameter for the user to choose.
...
\includegraphics[width=0.5\textwidth]{figs/s000/periodic-ww1_30-ff0_30-N50-gamma-and-entropy-error-slow.pdf}
  \caption{(a) The average entropy error for each MC method for $N=50$,
               ...
               as a function of number of iterations run. The number of important
               energies is $129$ of about $280$ energies.  The error is
           ...
           (c) The average entropy error for each MC method for $N=50$,
               ...
               as a function of number of iterations run. The number of important
               energies is $407$ of about $1800$ energies, and
...
we consider two values for the displacement distance by which atoms are moved
...
resulted in an acceptance rate of 86\%, which as expected converge
more slowly.
...
are able to converge regardless of the displacement distance used in Monte
Carlo moves.
...
Next we will introduce a considerably larger simulation containing
256 atoms which has a total entropy range of around 1500.  This makes
exploring the entire range of energies extremely expensive, and strongly
favors methods that restrict the energy (or temperature) range of
interest.
For this simulation, we chose a much higher minimum reduced
...
system is then $N_S = 407$.
The entropy of this system differs by 395, corresponding to a ratio of
$10^{118}$ between the highest and lowest density of states.
...
\section{Conclusions}
Both SAD and 1/t-WL converge more rapidly than Wl or SAMC. SAD requires
the user to specify a temperature range rather than an energy range as 1/t-WL
does. Both WL and 1/t-WL require a prior simulation in order to determine the energy range
of interest. WL saturates in convergence as discussed in the introduction and
requires the user to optimize the choice of parameters to avoid this such as
increasing the flatness criterion.  SAMC converges for a reasonable choice of
$t_0$ but this parameter can be difficult to tune especially across different
energy bins, displacement distances, or systems. Additionaly, SAMC does not
...
priori how quickly the update factor $\gamma$ should decrease as the

...
~\jpsays{We need to define or talk about Tmin.  This is a disadvantage
of of SAD. WL requires an energy range. SAMC removes the energy range
requirement but requires simulating every possible energy.  For SAD,
Tmax set to $\infty$ is fine.  Easy to simulate this way. SAD focuses
on energies between Tmin and infinity.}
...
~\jpsays{change name to $\epsilon_H$ and $\epsilon_L$. Include formula
for updating weights above and below high/low energy.  Explain how we
compute the maximum entropy state and minimum important energy.  Then
we are in a position to talk about racheting.} Since SAD explores all
energy states it is imperative that the method have a reasonable idea
about where the minimum important energy and maximum entropy state are
located in energy space. Since these are not user input parameters like
for methods suchas WL-TMMC, the simulation is responsible for
determining and updating these energy locations. In order to prevent,
the method from experiencing errors near the energy extrema, we employ
a high energy $\epsilon_H$ and a low energy $\epsilon_L$ to define the
range over which the energy histogram is made flat. $\epsilon_H$ is the
highest ever value that the maximum entropy state has taken while
$\epsilon_L$ is the lowest ever value that the minimum important energy
has taken. SAD uses these two energies to control the weights $w(E)$
above $\epsilon_H$ and $\epsilon_L$. If the energy $E$ is less than
$\epsilon_H$ ~\jpsays{greater than sign backward} then the weights can
be calculated as ~\jpsays{equation formatting problems!}
\begin{equation}
\begin{aligned}
\ln w(E) = \min(\ln w(E) - \gamma, \\
    \ln w(\epsilon_H) - \log(e^{\ln w(\epsilon_H) -w(E)}))
\end{aligned}
\end{equation}
If the energy $E$ is greater than $\epsilon_L$ ~\jpsays{greater than
sign backward} then the weights can be calculated as
\begin{equation}
\begin{aligned}
\ln w(E) = \min(\ln w(E) - \gamma, \ln w(\epsilon_L) \\
    + \frac{E-\epsilon_L}{T_\text{min}}
    - \log(\gamma + e^{\ln w(\epsilon_L) -w(E)
    + \frac{E-\epsilon_L}{T_\text{min}}}))
\end{aligned}
\end{equation}
Sad sets the maximum entropy state equal to the energy if either of the
following is true.~\jpsays{less than sign backward}
\begin{align}
...
either of the following is true.~\jpsays{check less than sign}
~\jpsays{equation formatting problems!}
\begin{align}
\ln w(E) < \ln w(E_\text{min}) + \frac{E-E_\text{min}}{T_\text{min}} 
...
Together these two
energies serve as a kind of ratcheting effect to determine the energy
landscape boundaries.
~\jpsays{So outside the range set by the weights $w(E)$, the maximum entropy
state is set to the energy $E$. Likewise with minimum important energy.}

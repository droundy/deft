...
Monte Carlo algorithms~\davidsays{add a few citations}.  The 31-atom
cluster is popular because it features a two-funnel energy landscape
with a significant barrier between the two low-energy
states~\davidsays{add citations}.

Poulain \emph{et al.} found \cite{poulain2006performances}
...
``everyone else'' does.  For purposes of testing the convergence of
the algorithm, we don't need to solve for the true heat capacity.
Furthermore, the heat capacity at fixed volume is not the most
meaningful quantity to predict for a cluster.
...
orders of magnitude.  WL and $1/t$-WL show similar behavior, so this
is not new, but is encouraging.

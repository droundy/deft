...
\begin{abstract}
  ...
  not known in advance. \jpsays{We believe that a
  temperature range is \emph{far} more physical and convenient, and this is the primary
...
swendsen1999transition, wang2001determining, wang2001efficient,
trebst2004optimizing}, which explore a wider range of energies.  Another
...
studied~\cite{landau2004new}. This adds an additional hurdle to its application
...
states~\cite{belardinelli2008analysis}.
Zhou et. al further
confirmed that the original WL algorithm never converges exponentially and
...
including scaling the update factor with the number of energy bins~\cite{schneider2017convergence}.
...
Kim \emph{et al.} introduced Statistical Temperature Monte Carlo (STMC) and
the related Statistical Temperature Molecular Dynamics (STMD) which is an
improvement on the WL method which approximates the entropy (or log density of states)
as a piecewise
linear function, which improves convergence for systems with a continuously varying
energy~\cite{kim2006statistical, kim2007statistical}. STMC
applied to WL requires a temperature range be specified rather than an energy
range.  Kim \emph{et al.} extended this work as Replica Exchange Statistical
Temperature Monte Carlo (RESTMC), which uses replica exchange of multiple overlapping
STMC simulations to improve convergence~\cite{kim2009replica}.
More recently, Junghans \emph{et al.} have demonstrated a close
connection between metadynamics, which was introduced by Laio and
Parinello~\cite{laio2002escaping}, and WL-based Monte Carlo
methods, with STMD forging the connection~\cite{junghans2014molecular}.
...
be easily applicable to any system. The method (like STMC above) does require the user to define a
temperature range of interest ($T_\text{min}$ to $T_\infty$).  We call this
method SAD (Stochastic Approximation with a Dynamic update factor), and will
discuss it in detail in the methods section. We compare its convergence
properties with three existing flat histogram methods: WL, $1/t$-WL, and SAMC.
...
\jpsays{REFEREE 2: The description of flat histogram methods in section II was a bit hard
to understand at first and may need to be edited to better highlight
the differences between the methods. Also, the entropy argument in
section II.D is unclear. The relationship between entropy, the
histogram, and the update factor needs to be more fully described and
the basis for equation 7 (and the approximations made there) needs to
be made clearer.}
...
The Wang-Landau approach~\cite{wang2001efficient,wang2001determining,
  landau2014guide} begins with $\gamma_t=1$, and then decreases $\gamma$
in discrete stages.  When the energy histogram is sufficiently flat,
$\gamma_t$ is decreased by a specified factor of $\frac12$.  The
...
specified threshold (typically 0.8), the $\gamma_t$ value is decreased
...
until $\gamma_t$ reaches a desired cutoff.

The Wang-Landau approach thus has three parameters that need be
specified: the factor by which to decrease $\gamma_t$ when flatness is
...
\jpsays{remove and add to intro as this does not convey how to implement WL!}
...
and does decrease $\gamma_t$ so quickly that it will never (for any
...
Secondly, when the $\gamma_t$ becomes less than $N_S/t$, the algorithm switches
to use
\begin{align}
  \gamma_t = \begin{cases}
     \gamma_{WL} & \gamma_{WL} > \frac{N_S}{t} \\
     \frac{N_S}{t} & \text{forever more}
 \end{cases}
\end{align}
where $t$ is the number of moves and $N_S$ is the number of energy bins.
\jpsays{David and I got to here!}
...
\jpsays{a better description of the relationship between entropy and the
...
the coherence time of the Monte Carlo simulation \jpsays{Michael Perlin asked about this in his comments and my understanding was that we meant the time for $\gamma_t$ to reach 1/t behavior. Since this is part of a section needing editing I thought we should clarify}.  Since $\gamma_t$ is
...
\begin{figure}
  ...
  \caption{The update factor $\gamma_t$ versus iteration for $N=256$
    ...
    and SAD \jpsays{edit linestyles to highlight SAD}}
...
size. \jpsays{Do we just remove this?} SAD should perform similarly over a wide range of bin sizes
...
\includegraphics[width=0.5\textwidth]{periodic-ww1_30-ff0_30-N50-gamma-and-entropy-error-slow.pdf}
  \caption{(a) The average entropy error for each MC method for $N=50$,
           ...
           (d) the update factor $\gamma_t$ versus iteration number
               for the same simulations.
           \jpsays{edit linestyle to highlight SAD!}
...
\includegraphics[width=\columnwidth]{periodic-ww1_50-ff0_17-N256-entropy-error-default.pdf}
  ...
  The average entropy error for each MC method for $N=256$,
               ...
               system is in Fig.~\ref{fig:gamma-vs-t} above. \jpsays{edit linestyle to highlight SAD!}}\label{fig:n256}
\end{figure}
...
\section{Conclusions}
\jpsays{We have introduced SAD, a new variant of SAMC, that}

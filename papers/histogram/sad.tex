...
demonstrated to saturate at a non-zero value, i.e. the method does not
converge to the true density of states~\davidsays{Find as many
  citations as we can! Presumably some from the following paragraph?}.
...
\includegraphics[width=0.5\textwidth]{figs/s000/periodic-ww1_30-ff0_30-N50-gamma-and-entropy-error-slow.pdf}
  \caption{(a) The entropy error for each MC method for $N=50$,
           ...
           (c) The entropy error for each MC method for $N=50$,
               $\delta_0 = 0.005\sigma$, $\eta = 0.3$, and $T_{\min} = 1/3$
               as a function of number of iterations run. The error is
               determined exactly as in (a).  

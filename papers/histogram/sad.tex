...
\begin{abstract}
  \davidsays{Let's look at the abstract together.}
...
$\gamma$ is ever less than $\nicefrac{N_S}{t}$ then the update factor
is set to $\nicefrac{N_S}{t}$ where $N_S$ is the number of energy
states and the histogram is no longer tracked. Employing this
...
\nicefrac{N_S}{t}$~\cite{schneider2017convergence}. \davidsays{Above
  we say that 1/t-WL does this.  One must be wrong?} As per the
...
The update factors take approximately 10x longer to reach steady-state
for the slower translation scale. Because of this update behavior,
these methods are less sensitive to the choice of displacement
distance than SAMC is.  \davidsays{This paragraph could benefit from
  going through again.}

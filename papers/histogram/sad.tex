...
Outside of the energy range $E_L \le E \le E_H$, the weights are
determined from the accumulated energy histogram. For each energy $E_i
> E_H$, the energy weights are calculated
\begin{align}
w(E_i)_{t+1} = w(E_H)_{t} + \ln\left(\frac{H(E_i)}{H_{\max}}\right)
\end{align}
assuming of course that the simulation has collected statistics at that energy.
For each energy $E_i
< E_L$, the energy weights are calculated
\begin{align}
w(E_i)_{t+1} = w(E_L)_{t} + \ln\left(\frac{H(E_i)}{H_{\max}}\right) +
\frac{E_i - E_L}{T_{\min}}
\end{align}

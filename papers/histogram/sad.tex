...
Although SAD determines the update factor ``dynamically'', the method must still
satisfy the criteria for convergence. Similar to the discussion that was presented
on the convergence for SAMC, we approximate the total entropy change using an integral.
The update factor is integrated from the last time an energy has been discovered $t_L$ to
some minimum time $t_{\min}$ necessary for convergence.
\begin{align}
\Delta S_{\text{tot}} &= \int_{t_L}^{t_{\min}} \gamma dt \\
&\approx N_S \ln\left(\frac{t_{\min}}{t_L}\right)
   + \Delta S_{\text{tot}} \left(1-\frac{t_L}{t_{\min}}\right)
\end{align}
Simplifying, we see that as $t_{\min} \rightarrow \infty$ then $\gamma \rightarrow \infty$ satisfying
the requirement for SAMC convergence. We can solve for the minimum time
\begin{align}

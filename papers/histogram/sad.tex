...
Although SAD determines the update factor ``dynamically'', the method
must still satisfy the SAMC convergence criteria. As in
Section~\ref{sec:samc-convergence} on the convergence for SAMC, we
will approximate the total entropy change using an integral.  We
integrate update factor from the last time an energy has been
discovered $t_L$ to some minimum time $t_{\min}$ necessary for
convergence.  This gives us a lower bound on the total entropy change
that can be acheived in $t_{\min}$ moves.
\begin{align}
...
&\approx N_S \ln\left(\frac{t_{\min}}{t_L}\right)
   + \Delta S_{\text{tot}} \left(1-\frac{t_L}{t_{\min}}\right)
\end{align}
...
Simplifying, we see that as $t_{\min} \rightarrow \infty$ then $\gamma
\rightarrow \infty$ satisfying the requirement for SAMC
convergence. We can solve for the minimum time
\begin{align}
\Delta S_{\text{tot}} \approx N_S \ln\left(\frac{t_{\min}}{t_L}\right)
+ \Delta S_{\text{tot}} \left(1-\frac{t_L}{t_{\min}}\right)
\end{align}
Solving for the minimum time necessary $t_{\min}$, we find that
\begin{align}
\frac{t_{\min}}{t_L} = e^{\frac{\langle S\rangle t_L}{t_{\min}}}
\end{align}
The solution for $t_{\min}$ can be written in terms of the Lambert-W function
\begin{align}
t_{\min} = \frac{3\langle S\rangle}{W(3\langle S\rangle e^{2\langle S\rangle})} t_L
\end{align}
For $\langle S\rangle \rightarrow 0$, we find that the minimum time
$t_{\min} \rightarrow t_L$ and for $\langle S\rangle \rightarrow \infty$,
we find that the minimum time $t_{\min} \rightarrow \frac{3}{2} t_L$.

We can examine the limiting cases for SAD and see that the behavior at
short times is analogous to the weight-based family algorithms $\gamma \approx 1$.
In the limit that the number of moves $t >> t_L$, $\gamma \approx \nicefrac{N_S}{t}$
which has a familiar 1/t form.

\section{Results}\label{sec:results}

For the square-well fluid, our simulations treat systems with a
well-width of $\lambda = 1.30$ and a filling fraction of
$\eta = 0.30$. We also explore various translation scales $\delta_0 = 0.005 and
0.05$. The simulations explore the
...
SAMC for the best choice of $t_0$.
\begin{align}
...
\begin{figure}
  ...
  \caption{The average entropy error for each MC method is shown for $\eta = 0.30$ and $T_{\min} = 0.2$ with
  SAMC $t_0 = 10^5$ serving as the `golden' standard.}\label{fig:N50-ff0.3-avg-error}
\end{figure}
\begin{figure}
  ...
  serving as the `golden' standard.}\label{fig:N50-ff0.3-avg-error-slow}
\end{figure}
%\begin{figure}
  %\includegraphics[width=\columnwidth]{figs/s000/periodic-ww1_30-ff0_30-N50-entropy-error-fast.pdf}
  %\caption{The average entropy error for each method is shown for $\eta = 0.30$ with
  %SAMC followed by TMMC serving as the `golden'
  %standard.}\label{fig:N50-ff0.3-avg-error-fast}
%\end{figure}
\begin{figure}
\includegraphics[width=\columnwidth]{figs/s000/periodic-ww1_30-ff0_30-N500-entropy-error-default.pdf}
  \caption{The average entropy error for each MC method is shown for $\eta = 0.30$ and $T_{\min} = 0.5$ with
  SAMC $t_0 = 10^6$ serving as the `golden' standard.}\label{fig:n500-ff0.3}
\end{figure}

\subsection{A periodic system with $\eta = 30\%$ and $N = 50$}
...
simple cubic unit cell with lattice constant chosen to give a $\eta =
0.30$.  These particles are randomly inserted into the system and
random moves are proposed (with a translation scale $\delta_0 = 0.05$).
For the $\eta = 30\%$ and $N = 50$ system, we explore the choice of
$\delta_0$ as a parameter. Smaller translation scales cause the given
method to take additional time to explore all energies. Ideal methods
should scale linearly with the $\delta_0$, that is, a decrease in the
translation scale should result in a proportional increase in
convergence time. However, as can be seen from
Fig.~\ref{fig:N50-ff0.3-avg-error-slow} SAMC methods that rapidly
converged for $\delta = 0.05$ do not proportionaly converge for a
translation scale of $\delta = 0.005$. We see that SAD handles
arbitrary translation scales and proportionaly converges based on the
translation scale that is chosen.

\subsection{A periodic system with $\eta = 30\%$ and $N = 500$}

A system is configured with dimensions $x = y = z = 19\sigma$.  A
filling fraction $\eta = 0.30$ is chosen which yields $N = 500$
...
For the larger system, SAMC parameter values of $t_0$ that gave rapid convergence for
the $N = 50$ system with $\delta = 0.05$ are too small.  Several additional
larger $t_0$ parameters are chosen to compare with SAD.  SAD has significantly smaller
fluctuations in the average entropy error for a given number of moves.
...
$\nicefrac{1}{\sqrt{t}}$. It can be observed  for the simulations that were carried out that all the methods from
the weight-based family struggle to converge if the update factor decreases too rapidlly. Fig.~\ref{fig:n500-ff0.3} shows
...
We compare each method to a `golden' standard SAMC simulation for an ideal choice of $t_0$.  This gives an
...
can correctly identify an appropriate choice of $t_0$ for SAMC).  SAD
...
We have introduced a new algorithm SAD (Stochastic Approximation
with a Dynamic $\gamma$) that effectively

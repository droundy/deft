...
\begin{figure}
  ...
  \caption{The entropy of a square well fluid with 50 atoms and filling fraction
        ...
        ~\jpsays{I fixed this to use samc-1e4-s1. Remove comment}
...
\includegraphics[width=0.5\textwidth]{figs/s000/periodic-ww1_30-ff0_30-N50-gamma-and-entropy-error-slow.pdf}
  \caption{(a) The average entropy error for each MC method is shown for $\delta_0 = 0.05\sigma$, $\eta = 0.3$, and $T_{\min} = 1/3$ with
               a fixed energy range reference as discussed in the text. The update factor $\gamma_t$ for
               $N=50$ versus iteration for the four different methods: WL, $1/t$-WL, SAMC, and SAD is shown beneath.
           (b) The average entropy error for each MC method is shown for $\delta_0 = 0.005\sigma$, $\eta = 0.3$, and $T_{\min} = 1/3$ with
               the same fixed energy range reference used in (a). The update factor $\gamma_t$ for
               $N=50$ versus iteration for the four different methods: WL, $1/t$-WL, SAMC, and SAD is shown beneath.}\label{fig:fast-slow-gamma}
\end{figure*}

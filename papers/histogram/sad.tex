...
wider range of energies.  \jpsays{In addition to simulating a range of
temperatures}, in contrast with low-temperature canonical Monte Carlo, is that
these approaches \jpsays{avoid being} trapped in a local energy minimum.
...
the method is incredibly powerful, it has a few disadvantages. \jpsays{The most
...
schulz2003avoiding, yan2003fast}. This \jpsays{requirement} adds an additional
...
Liang began to consider \jpsays{Stochastic Approximation as a mathematical
...
adaption of the WL method \jpsays{that} approximates the entropy (or log density of
...
\begin{equation}
	...
	+\gamma_t
\end{equation}
where $t$ is number of the current move, \jpsays{$\gamma_t$} is an update factor
...
\jpsays{Michael prefers removing $t_0$ since he found it confusing with SAMC later.
replace the formula with this:
\begin{align}
\gamma_t^{1/t\text{-WL}} = \min \left( \gamma_t^{WL}, N_S/t \right)
\end{align}
This has problems with not defining when all the states have been visited once!}
...
Werlich \emph{et al.} proposed scaling the SAMC \jpsays{$\gamma_t^{\text{SA}}$}
...
convergence is harder to estimate when $t_0$ is large, but in general $\gamma_t$
itself forms a lower bound on the accuracy with which the entropy may be known,
with an unknown prefactor which is related to the coherence time \jpsays{define
coherence time!} of the Monte Carlo simulation.  Since $\gamma_t$ is given by
$t_0/t$, the time to converge to a given accuracy is increased in proportion to
the ratio by which we overestimate $t_0$.  Thus, while it is exponentially
painful to underestimate $t_0$, overestimating by several orders of magnitude is
also not acceptable.  We should note that these extreme limiting cases do not
preclude the possibility that there is a wide range of $t_0$ values that lead to
...
Allowing infinite temperature further ensures that the system can surmount
\jpsays{an energy barrier of any size}.  Finally, the high temperatures are
``easy'' to simulate, and tend to converge very quickly.  We note, however, that
this is under the assumption that infinite temperature corresponds to a finite
energy, as is the case for configurational sampling.  Were we to include kinetic
energy in a simulation (as in molecular dynamics), or to examine a system such
as a high density Lennard-Jones fluid \jpsays{with an entropy maximum at an
energy that is far from the energy range of physical interest} which might have
an energy of maximum entropy far from the energy range of physical interest,
then we would need to apply a maximum temperature, which could slow down the
...
\jpsays{Perlin asks: Did you mean to define $t_L$ as the last time you encountered a new energy of interest.}
...
root-mean-square displacement distance \jpsays{what type gaussian?} $\delta_0 = 0.05\sigma$ for

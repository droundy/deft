...
parameter $t_0$.  Unfortunately, there is no
...
system is then $N_S = E_{\min} - E_{\max} + 1 = 407$.
...
\davidsays{Also give the entropy difference, looking at the data.}
\jpsays{$395.2833013702813 - 4.754731035740479e^{-8}$ = $395.2833$ so $2^{396.2833}$
...
\davidsays{We find both 1/tWL and sad do really well and very similarly.
SAD wins if you want to specify a temperature range rather than an energy.
1/tWL cheated because you had to run a prior simulation in order to find
the energy range of interest.  Yay good stuff.}

\jpsays{Both SAD and 1/t-WL converge more rapidly than Wl or SAMC. SAD requires
the user to specify a temperature range rather than an energy range as 1/t-WL
does. In addition to requiring the energy range,
1/t-WL requires a prior simulation in order to determine the energy range
of interest. WL saturates in convergence as discussion in the introduction and

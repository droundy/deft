...
explores only a small range of energies.~\jpsays{
Berg and Neuhaus developed multicanonical
methods that improved the sampling of configuration space and attempted to
...
decrease asymptotically as $1/\sqrt{t}$~\cite{belardinelli2008analysis}.
...
be easily applicable to any system. The method does require the user to define a
...
~\jpsays{STMC uses a dynamic microcanonical temperature $\tilde{T}$ to
accelerates convergence by refining the temperature estimate rather than the
density of states estimate~\cite{kim2006statistical, kim2007statistical}. STMC
applied to WL requires a temperature range rather than an energy range.
From what I can tell the user needs to input a temperature range $T_l$ and $T_h$
in general and an energy range for continuum systems $E_l$ and $E_h$.
}

~\jpsays{The benefits of Monte Carlo methods such as WL cannot easily be
exploited on systems where MC moves are not easily
defined~\cite{kim2006statistical}, such as in the case of molecular simulations,
or where for continuum systems where energy bins increase extensively with
system size~\cite{kim2009replica, junghans2014molecular}.}
...
as $S(E) \equiv \ln(D(E))$ (where $k_B \equiv 1$), the logarithm of the weights can be
...
Schneider et. al outline minor refinements to the $1/t$-WL algorithm
including $\nicefrac{N_S}{t}$ scaling and switching from standard WL
to $1/t$-WL when the update factor $\gamma_t <

...
\newcommand{\supervisor}{Dr. David Roundy} \renewcommand{\date}{21
  April 2015}
...
function of $N$ void of constant prefactors, e.g. $N\log N$, or
$2^N$). These collection rules ensure that scaling up simulations does
not cause them to asymptotically spend all computation time collecting
...
temperature anyways (addressed in Section \ref{sec:energy_range}). In
...
density of states $D\p{E}$ of a system. The density of states can in
turn be used via the flat histogram method (Section
\ref{sec:flat_histogram}) to determine appropriate weights for
subsequent Monte Carlo simulations. TMMC introduces a new object: the
energy transition matrix, $T$, whose components $T_{ij}$ are the
...
The transition matrix $T$ is trivially square and positive-definite,
but it is not symmetric. Given that Metropolis Monte Carlo samples all
of state-space randomly, meaning all possible system states are
equally likely to occur during simulation, a transition $E_i\to E_f$
from an energy with a low state density to one with a high state
density, $D\p{E_i}<D\p{E_j}$, is more probable than the inverse
transition $E_j\to E_i$ simply due to the fact that there are more
states with energy $E_i$ than those with energy $E_j$. Thus
$D\p{E_i}<D\p{E_j}$ implies $P\p{E_i\to E_j}>P\p{E_j\to E_i}$, which
means $T_{ji}>T_{ij}$. Said otherwise, the asymmetry of the transition
matrix is a direct consequence of the second law of thermodynamics:
systems are exceedingly likely to evolve towards macroscopic states
with higher accessible microstate densities.

In principle, computationally determining the transition matrix of a
system is straightforward. One need only simulate a system and collect
a histogram $\tilde T\p{E_i,E_j}$ of the energy transitions after each
move, directly after deciding whether to reject the move based on
whether it resulted in an (in)valid system state. That is, transition
data should be collected (only) on all moves as they would have been
accepted by Metropolis Monte Carlo, i.e. before deciding whether to
reject any given move for any reasons involving weights, or other
biasing schemes.
...
\fixme{schemes for initializing to collect transition matrix}

\fixme{normalizing transition matrix}

\fixme{computing density of states via transition matrix}

\fixme{storing transition matrix in the form $T\p{E,\Delta E}$}

\vspace{3cm}


For the square-well fluid, as with many systems, there is a maximum
amount $\Delta_ME$ by which a single move can change the energy of the
system. The transition matrix can therefore be stored in the form
$T\p{E,\Delta E}$


the transition matrix is band-diagonal because . In implementation,
the transition matrix can therefore be stored in the form
$T\p{E,\Delta E}$, making it an $L\times\Delta_ME$ matrix, where $L$
is the number of energy levels in a given system. Given that $L$
scales as $N\Delta_ME$, where $N$ is the number of spheres in the
square-well fluid, the memory required to store the transition matrix
therefore scales as $N\p{\Delta_ME}^2$, an improvement over the $N^2$
growth of a transition matrix in the form $T\p{E_i,E_j}$.
...
\fixme{Explain briefly. No need to provide algorithm.}

...
We find that Wang-Landau is capable of performing the best of all
methods, but fails catastrophically without a predefined energy range
for the square-well fluid. Our first method gives results comparable
to successful Wang-Landau simulations, but with some anomalous
cases. Our implementation of the optimized ensemble using a transition
matrix results in worse performance over the straightforward
...
after initialization anyways. We always set
$w\p{E<E_-}=e^{-\p{E-E_-}/kT_{\t{min}}}$ after initialization of all
histogram methods with $kT_{\t{min}}/\epsilon=0.2$. We chose this
...
We made a special case for Wang-Landau, as the design of this method
assumes knowledge of the energy range of interest a priori. In order
to more fairly compare Wang-Landau to other methods, we therefore
simulated two cases: one in which the minimum important energy $E_-$
is identified via other methods and hard-coded into the Wang-Landau
initialization routine, and one in which Wang-Landau simply assumes
that the lowest energy a given simulation has ever observed is the
minimum important energy. The simulations in which $E_-$ is hard-coded
we will refer to as ``vanilla Wang-Landau'' simulations.

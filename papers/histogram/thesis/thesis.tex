...
temperatures. Previous works have developed several generic Monte
Carlo histogram methods for collecting statistics on low energy system
states, but little or no literature exists on these methods'
systematic comparison, as well as their application to the square-well
...
discuss\cite{wang_landau, wang_landau_analysis, wang_landau_mod}. This
...
Interestingly, the flatness condition $C\sp{H}$ for updating $f$ and
restarting the process should not be too stringent: if the histogram
is completely flat, say $H\p{E}=H_0$ for all $E$, then the weights
$w\p{E}$ have all been modified by the same factor of $f^{H_0}$.
Remembering the scale-invariance of $w\p{E}$, this result would mean
that the weights have not actually changed at all! Similarly, too lax
of a flatness condition will prompt the initialization process to
\begin{enumerate*}[label=\roman*)]
...
\end{enumerate*}
Tuning the flatness condition is thus crucial to the success of the
Wang-Landau method.
...
reasonable. Too small value of $n$ will waste computation time
...
$n=L$, where $L$ is the number of energy levels for the given
square-well fluid (as discussed in Section \ref{sec:simple_flat}).
Papers in the literature typically neglect providing any heuristic or
formula for $n$, reporting a system-independent $n=10^3$ or
$10^5$\cite{wang_landau, wang_landau_mod}.
...
heuristics for assigning $f_0$, literature typically suggests
$f_0=e$\cite{wang_landau}; after some experimentation with the free
parameters in the Wang-Landau method, we used $f_0=e^{1/8}$ (in
...
to satisfy these requirements. Previous papers have used $u=2$, which
we have adopted as well\cite{wang_landau, wang_landau_mod}.

\fixme{Walk through $C\sp{H}$ and $c$. Finish discussion of WL.}

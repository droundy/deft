...
sampling the low energy fluid states which dominate the partition
function at low liquid-state temperatures. Previous works have
developed several generic Monte Carlo histogram methods for collecting
statistics on low energy system states, but little or no literature
exists on these methods' systematic comparison, as well as their
application to the square-well fluid. The square-well fluid in
...
Having covered the basics of the square-well fluid as well as unbiased
and biased Monte Carlo simulations, we can now discuss several
histogram methods for constructing energy weights. In Section
...
week. To control for iteration number in the results from any given
set of simulations, we compare against ``converged flat''
simulations. Converged flat simulations read in the transition matrix
from golden simulations to construct a weight array, and simulate for
the same number of iterations as the standard simulations.
...
between liquid and gas phases, and take this fact as given. \fixme{say
  more convincingly}
...
that the optimized ensemble is failing to improve TMMC because the
theory behind it neglects the discretization of energies.

\fixme{make conclusions more satisfying?}
...
in the fun and interesting world of scientific research. \fixme{Thank
  Jansen and McIntyre}

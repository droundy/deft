...
\begin{figure}
  ...
  \caption[Density of states]{Sample density of states for square-well
    ...
    $\eta=0.3$, and $N=20$ spheres, as computed by several histogram
...
\begin{figure}
  ...
  \caption[Transition matrix]{Sample transition matrix $T_d\p{E,\Delta
    ...
    filling fraction $\eta=0.3$, and $N=20$ spheres.  The band at
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figs/array-samples.pdf}
  \caption[Sample arrays]{Sample arrays computed by various histogram
    methods for a square-well fluid with a well width $\lambda=1.3$,
    filling fraction $\eta=0.3$, and $N=20$ spheres after simulating
    for $10^5N^2$ iterations: (top left) energy histogram, (top right)

...
\begin{figure}
  \centering
  \includegraphics[width=0.6\textwidth]{figs/dos-sample.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width=0.6\textwidth]{figs/transitions-sample.pdf}
...
have thus far neglected three subjects: initialization end conditions
(i.e. the means by which we decide when an algorithm to construct
weights is finished), the energy range about which we care, and the
minimum temperature $T_{\t{min}}$ which we sometimes make reference
to. The last two of these subjects turn out to be related, as the
minimum temperature $T_{\t{min}}$ chosen for any given simulation
determines the minimum energy of interest.
...
$R=\sp{E_-,E_+}$ before terminating. Furthermore, samples at the end
of initialization should be numerous enough to be statistically
significant.\fixme{include mean sample error?} Given that any samples
of $E_-$ necessarily require first sampling all other energies in $R$
on the way down from $E_+$, this energy ($E_-$) will generally have
the fewest number of samples. As an end condition, one can therefore
enforce some minimum number of optimistic or pessimistic samples at
$E_-$, i.e. $s_o\p{E_-}$ or $s_p\p{E_-}$. A more sophisticated end
condition would be to enforce some minimum value to the mean
fractional error $\bk{\delta_s}_{E,T_{\t{tmin}}}$ in either of the
sample counts $s_o\p{E}$ or $s_p\p{E}$ at the minimum temperature
$T_{\t{min}}$. These mean errors are
\begin{align}
  \bk{\delta_s}_{E,T_{\t{tmin}}}=\bk{s^{-1/2}}_{E,T_{\t{min}}}
  =\sum_Es\p{E}^{-1/2}P\p{E,T_{\t{min}}}
  =\f{\sum_Es\p{E}^{-1/2}D\p{E}e^{-E/kT_{\t{min}}}}
  {\sum_{E'}D\p{E'}e^{-E'/kT_{\t{min}}}},
\end{align}
...
\subsubsection{Determining the energy range of interest}
\label{sec:energy_range}

While some particular systems, such as the Ising model, have
well-defined and easily determined minimum and maximum energies, the
same is not true of all systems in general.

...
net and down-going walker diffusivity at an energy $E$ respectively by
$\alpha\p{E}$ and $\alpha_+\p{E}$.

We can meaningfully discuss a down-going walker flux $j\p{E}$, which
is defined by
\begin{align}
  j=\f{dn_+}{dt}=\alpha_+\f{dn_+}{dE}=f\alpha\f{dn_+}{dE}.
...
\begin{align}
  j=f\alpha\sp{n\f{df}{dE}+f\f{dn}{dE}}.
...
\end{align}
For (\ref{eq:walker_current}) to be valid, we want the ratio in
...
$n\p{E_-}=0$ to $n\p{E_+}=n\p{E_+}$ meaning $\ln n_+$ runs from $\ln
n\p{E_-}\to-\infty$ to a finite positive value $\ln n_+\p{E_+}=\ln
n\p{E_+}$. It is therefore reasonable to assume that $d\ln n_+/d\ln
n\gg 1$, which makes the ratio in
...
\begin{align}
  \int_{f=0}^1\f{df}{j}=\int_{E=E_-}^{E_+}\f{dE}{f\alpha n}.
  \label{eq:walker_current_setup}
\end{align}
At equilibrium, $j=dn_+/dt$ is constant, which means we can evaluate
one of the integrals in (\ref{eq:walker_current_setup})
\begin{align}
  \f1j=\int_{E=E_-}^{E_+}\f{dE}{f\alpha n}.

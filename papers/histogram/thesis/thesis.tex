...
we simulated only one system: the well width $\lambda=1.3$ and filling
fraction $\eta=0.3$ square-well fluid, for all sizes (i.e. number of
...
Three arrays contain the raw data for all analysis performed in this
paper:
...
\begin{figure}[H]
  ...
  {Example pessimistic energy sampling rates computed by various
    ...
    after simulating for $10^5N^2$ iterations. Sample rates for each
    method are averaged over 30 simulations with different random
    number seeds.}
...
\subsubsection{Example curves}
\label{sec:property_examples}
...
\subsubsection{Error scaling and comparisons}
\label{sec:errors}

...
The down-going walker current $j\p{E}$ is defined in terms of the
...
\begin{align}
  ...
  \label{eq:walker_current_intermediate}
\end{align}
We can observe that
\begin{align}
  \f{f~dn/dE}{n~df/dE} =\p{\f{dn_+/dE-fdn/dE}{f~dn/dE}}^{-1}
  =\p{\f1f\f{dn_+}{dn}-1}^{-1}.
  \label{eq:walker_current_approx_ratio_setup}
\end{align}
Furthermore,
\begin{align}
  \p{\f1f\f{dn_+}{dn}-1}^{-1}
  =\p{\f1f\f{d\p{fn}}{dn}-1}^{-1}
  =\p{\f nf\f{df}{dn}+\f{dn}{dn}-1}^{-1}
  =\p{\f nf\f{df}{dn}}^{-1}=\f nf\f{dn}{df}=\f{d\ln n}{d\ln f},
  \label{eq:walker_current_approx_intermediate}
\end{align}
which means
\begin{align}
  \f{f~dn/dE}{n~df/dE}=\f{d\ln n}{d\ln f}.
  \label{eq:walker_current_approx_ratio}
\end{align}
The walker current is therefore
\begin{align}
  j=\alpha n\f{df}{dE}\p{1+\f{d\ln n}{d\ln f}}.
  \label{eq:walker_current_full}
\end{align}
We know that $f$ runs from $f\p{E_-}=0$ to $f\p{E_+}=1$ meaning $\ln
f$ runs from $\ln f\p{E_-}\to-\infty$ to $\ln f\p{E_+}=0$. If the
walker density $n$ varies weakly with energy, e.g. because we are
simulating with flat histogram weights, then $\ln n$ varies even
weaker still. We may therefore reasonably to assume that $\abs{d\ln
  n/d\ln f}\ll 1$ and express
\begin{align}
  j=\alpha n\f{df}{dE}.
  \label{eq:walker_current}
\end{align}
The work developing the optimized ensemble in
\cite{optimized_ensemble} states (\ref{eq:walker_current}) without
showing how it follows from the definition of walker current expressed
in (\ref{eq:walker_current_definition}) and making explicit the
approximation between the exact result (\ref{eq:walker_current_full})
and the working form (\ref{eq:walker_current}).

We can rearrange (\ref{eq:walker_current}) and integrate over the
energy range to say
\begin{align}
  \int_{f=0}^1\f{df}{j}=\int_{E=E_-}^{E_+}\f{dE}{\alpha n}.
  \label{eq:walker_current_integral_setup}
\end{align}
At equilibrium, the walker current $j\p{E}$ must be constant with
respect to energy. If $j\p{E}$ is not constant with respect to energy,
some energies would have either a net incurrent or net outflow of
walkers, which would mean that the ensemble is not at equilibrium. We
can therefore evaluate one of the integrals in
(\ref{eq:walker_current_integral_setup}) to get
\begin{align}
...
walker current $j$, by varying the walker density $n$ with the

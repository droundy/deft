...
improved with additional MC steps.  This WL version is more accurate for
a given number of simulation steps.

\paragraph{Initializing Wang-Landau:} We begin the simulation by
defining a minimum important energy.  In traditional Wang-Landau the WL
factor begins at 1.0 and the factor is modified by 2.0 until the
simulation is cutoff typically around $10^{-10}$.  Rane et. al. in the
first published implemenation of WL-TMMC~\cite{rane2013monte} suggests
using a WL factor between 1.0 and $10^{-2}$ with a cutoff $<10^{-5}$.
In our implementation of WL-TMMC, we allow the WL factor to be 1 but
the cutoff is now $10^{-4}$. A single sweep is defined as each
macrostate being sampled a number of times in this case 100 and it is
sufficient that each macrostate is visited at least once.  The goal of
...
therefore take an unexceptable length of time to
...
reach a temperature of interest say T = 0.1.
I don't think we have yet implemented a biasing function to control acceptance

...
\end{align}
where unlike in the original paper detailing the method, we compute the diffusivity directly from the transition matrix normalized with canonical weights.  The optimal ensemble therefore relates the density of states to the inverse of the square root of the diffusivity.
\begin{align}
...
in phase separation.  As the temperature of the system decreases, an
increase in particle aggregation near the sticky-wall becomes evident.
As the energy of the particles is decreased, the particles are unlikely
to be able to escape the attractive sticky-wall potential.  By
examining the particle number density vs the system length (z-axis),
phase transitions to a liquid-vapor system from a vapor dominated
...
A system is configured with dimensions $x = y = z = 10?$.  A
...
A system is configured with dimensions $x = y = z = 19?$.  A

...
energy and maximum entropy state being calculated (with the exception 
of WL-TMMC where both of these parameters are needed apriori).

In the results, we are going to be looking at N = 50 (from Tanner) with
no sticky-wall, N = 25 (from Jordan) with a sticky-wall, or N = 200
(from Jordan) with a sticky-wall.  We (David, Tanner) are both running N
= 500 simulations but these seem unlikely to converge in the immediate
future?

We are likely not doing SA-TMMC and/or TMI3 (however still may be worth while to
mention why these methods are not shown?)?

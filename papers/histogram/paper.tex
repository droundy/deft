...
\davidsays{It just occurred to me that we could (should?) test various
  sets of parameters for Wang-Landau.  We wouldn't be looking for the
  ``optimal'' set of parameters, but simply checking to what extent
  the algorithm is sensitive to parameter choice.  This would give us
  more to say and more to compare, potentially.  Also, it would
  side-step a criticism that we simply used WL with the wrong set of
  parameters.}
...
The Transition Matrix Monte-Carlo algorithm is in the family of broad histogram
methods~\cite{swendsen1999transition}.  The idea of the transition matrix was 
first developed in the 1970s by Kieth Hastings~\cite{hastings1970monte}.  We begin

...
name) uses the following algorithm to determine the weights.  The method
periodically recomputes the weights during the simulation.  Although no
bottlenecks result when updating the weights more frequently, the
algorithm will nonetheless see additional computational time.  Because
...
At energies above the maximum in the density of states, we set the weights equal to the weight at the DOS maximum.  This avoids expending extra effort to equally sample states that are not favored at any positive temperature.

\subsubsection{Weights at Interesting Energy}
At energies below the maximum, we would like to set the weight equal to the inverse of the density of states, \emph{unless} there are insufficient statistics to justify that.  We define a modification factor in terms of a fractional uncertainty
\begin{align}
  \mathcal{F}(\epsilon) &= \frac{1}{\sqrt{N_s(\epsilon)}}
\end{align}
where $N_s(\epsilon)$ is the number of round-trip samples taken at energy
...
\begin{align}
  \mathcal{W}_{E} &= \mathcal{W}_{E+1}\max\left(\frac1{\epsilon(E)},
  \frac{D(E)}{D(E+1)}\right)
  \\
   &= \mathcal{W}_{E+1}\max\left(\frac1{\sqrt{N_s(E)}},
  \frac{D(E)}{D(E+1)}\right)
\end{align}

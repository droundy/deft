...
\ignore{[I think this is framed incorrectly... markov chains are used to generate the transition matrix, no? That's the procedure outlined by de Oleveira, reinforced by michael...]}
...
\end{align}
and then normalizing appropriately. This method unfortunately loses substantial accuracy far from the original temperature and causes an untoward spike near the average value of the original histogram. Further, this reweighting detracts from the accuracy of the tails of the histogram. We choose instead to implement a similar approach to that of P.M.C. de Oleveira and coworkers\cite{de1998broad}, the ``broad histogram''. [more] This process is outlined in Algorithm~\ref{alg_tmmc}.\\

[stuff: markovian walk along energies known a priori, accept state always if $\Delta E< 0$, potentially accept state if\dots what? We can't directly enumerate states. Must be in Michael thesis somewhere. Check wang-landau paper? or [8] on michael thesis]

\begin{algorithm}[tb]
    \caption{Transition Matrix Monte Carlo}
    \label{alg_tmmc}
    \hrulefill
    \begin{enumerate}
        \item hi

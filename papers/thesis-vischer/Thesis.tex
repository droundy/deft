...
Monte-Carlo methods utilize random or pseudorandom sampling and check for certain conditions or measure certain quantities after a specified number of iterations. Traditional random sampling requires that the current step, or ``move'', is irrespective of previous steps. True random movement is an ideal simulation characteristic for the canonical ``drunken rambler'' scenario, in which a intoxicated man attempts to make his way home by taking random steps. Since the drunken man chooses his next move arbitrarily, his movement is considered ``random''; it follows that true random sampling is desirable for this physical system. Statistical mechanics, however, does not exhibit this independence. Each time the drunken man makes a move there is an equal probability that he will move in any particular direction. Rather than all movements having equal probability, statistical mechanics stipulates that the probability of a particular move occurring depends explicitly upon the energy difference of the current and resulting configurations. Instead of true random Monte Carlo methods, we clearly need some way to control the probability of transitions from a given state. In pursuit of this control, we employ the method of Markov chains, specifically Metropolis-Hastings Monte Carlo.
\subsubsection{Markov Chain}
[cite me pls]
Markov chain methods eschew the true randomness of regular Monte Carlo by taking the current configuration into account before moving. If we define the initial state of the system by a vector column $\mathbf{q}$ (where the components refer to general coordinates, typically taken to be position of each sphere), and the resulting state $\mathbf{p}$ after a single time step, the relationship
\begin{align*}
    \mathbf{p} &= \mathcal{P}\, \mathbf{q}\\
\end{align*}
defines a ``transition'' of the system. $\mathcal{P}$ is a necessarily square matrix (as long as the number of coordinates remain fixed) that contains information about these transitions. We call this construction $\mathcal{P}$ the {\it transition matrix}. The transition matrix's $i^{\text{th}}$ eigenvalue the probability of the $i^{\text{th}}$ state occurring which, for the canonical ensemble. If all elements of the transition matrix are nonzero, we can see that any state of the system is reachable after a finite number of transitions from any given initial state. [This is the best I could synthesize Hansen, following statement could be wrong!] A physically judicious choice of transition matrix is one that satisfies the thermodynamic steady state condition; given the equilibrium state $\mathbf{q}_{eq}$, we enforce that
\begin{align*}
    \mathcal{P}\,\mathbf{q}_{eq} &= \mathbf{q}_{eq}
\end{align*}
The transition matrix thus embodies the physics of the simulation. [more?]

\ignore{\subsubsection{Histogram Methods}
Histograms are graphical representatio

n of ``counting'' data. Monte Carlo histogram methods simply track the number of times the system has visited a particular energy. If the transition matrix 
\begin{figure}
    \caption{Sample histogram.}
\end{figure}}
\subsection{Broad Histogram Monte-Carlo}
If one records the energy of the system at each time step, it is straightforward to construct a histogram of energies. Typical implementations of histogram methods exhibit histograms that sharply peak near a central value $E_0$ which then symmetrically decays. Of course, at higher temperatures, the histogram broadens and flattens out as more energies become easily accessible and, in the limit that temperature becomes infinite, the histogram becomes completely flat since all states are equally likely [flat? what about $D(E)$]. If the simulation is allowed to run for an infinite amount of time, the histogram will be precisely detail the system. x The tails of the histogram, however, contain spurious statistics because the system rarely visits these energies. Gathering sufficient statistics  
...
Since the excess hard sphere free energy is really just the entropy[cite] (scaled by temperature), the statement that the square well and hard sphere excess free energies need be equal in the infinite temperature limit is really a constraint on the {\it entropy} of the two systems. We then conclude that
\begin{align*}
...
Finding coexistence requires use of the grand free energy $\Phi$. Liquid and vapor are simultaneously present in the fluid at densities that cause local minima in the grand free energy. Referring back to Eq.~\ref{Phidef}, we can compute an absolute grand free energy given the absolute free energy and number of spheres. The chemical potential $\mu$ is a somewhat unintuitive quantity that, for our purposes, we will treat as a free parameter. Tweaking the value of the chemical potential will allow us to easily identify coexistence in the fluid. 
\begin{figure}
\centering
...
\ignore{\subsection{Comparison With A Modified Statistical Associating Fluid Theory}
[more]}

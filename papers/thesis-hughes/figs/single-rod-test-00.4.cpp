...
int main(int, char **) {
  FILE *o = fopen("paper/figs/single-rod-test-00.4.dat", "w");
  ...
  Functional S = OfEffectivePotential(SaftEntropy(water_prop.lengthscale, 
						  ...
						  water_prop.lambda_dispersion,
						  water_prop.length_scaling));
  for (cavitysize=0.4*nm; cavitysize<=0.41*nm; cavitysize += 0.1*nm) {
    ...
    sprintf(plotname, "paper/figs/single-rod-test-slice-%04.1f.dat", cavitysize/nm);
    Grid density(gd, EffectivePotentialToDensity()(water_prop.kT, gd, potential));

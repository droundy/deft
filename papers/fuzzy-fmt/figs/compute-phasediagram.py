...
def run_new_melting(kT, n, seed, gwstart, gwend, gwstep, fv=1e-4, dx=0.5, 
    ...
    cmd += ' --d data/phase-diagram_fv1e-4'
...
#kTs = np.arange(0.5, 0.02, -0.1)  #only do kT=0.5
#kTs = np.arange(3, 0.05, -2.5)  #only do kT=3 and kT=0.5
kTs = np.arange(2.9, 2.8, -0.15)  
#densities = np.arange(0.01, 1.53, 0.02)
densities = np.arange(0.89, 0.9, 0.02)
...
for kT in kTs:
    for n in densities:
	    ...
	    # We want high accuracy for this one!
            run_new_melting(kT, n, seed=1, gwstart=0.01, gwend=0.11, gwstep=0.01, mcerror=1e-4) #trouble data points
        else:

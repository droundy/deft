...
double lj_potential(Cartesian r) {
  ...
  double V = 4*eps*(uipow(sigma/distance,12) - uipow(sigma/distance,6));
...
double run_soft_sphere(double reduced_density, double temp) {
  ...
  softspherepotential.Set(lj_potential);
  ...
  *potential = softspherepotential - temperature*log(reduced_density*pow(2,-5.0/2.0)/(1.0*radius*radius*radius))*VectorXd::Ones(gd.NxNyNz); // Bad starting guess
  ...
  sprintf(plotname, "papers/fuzzy-fmt/figs/radial-lj-%06.4f-%04.2f.dat", temp, reduced_density);
...
int main(int argc, char *argv[]) {
  printf("dx = %g, xmax = %g\n", dx, xmax);
  if (argc != 3) {
    fprintf(stderr, "Usage: %s Reduced Density kT\n", argv[0]);
    exit(1);
  }

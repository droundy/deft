...
data find_energy_new(double temp, double reduced_density, double fv, double gwidth, char *data_dir, double dx, bool efficient, bool verbose=false) {
  ...
  //NOTE: dx_proper chops up lattice constant a (dx_proper=a/number of chunks), whereas the current definition  
  //of dx chops up a/2 (dx=(a/2)/some number of chunks) to create chunks that correspond in number  
  //to infinitesimal lengths, or chunks, along the lattice_vectors. 

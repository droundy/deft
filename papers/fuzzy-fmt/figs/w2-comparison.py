...
plt.plot(r, fp_wca, '-', color=styles.color_from_kT(kT), linewidth=1, label=r"$f'_{wca}$ for $T* = %g$" % kT)

r = np.linspace(rmin, rmax, int(rmax/dr))
fp_erf = np.exp(-(r-alpha)**2/Xi**2)/Xi/np.sqrt(np.pi)
plt.plot(r, fp_erf, '--', color=styles.color_from_kT(kT), linewidth=1, label=r"$f'_{erf}$ for $T* = %g$" % kT)

...
SFMT is a generalization of FMT to soft interactions and \fixme{
  Schmidt chose because the range of the weighted densities is at a
  distance of one sphere radius}. The weight functions are modified
but retain: the direct correlation in the low-density limit, the exact
free energy in the zero-dimensional cavity limit, and relation to the
generating weight function $w_3(\rr)$~\cite{schmidt1999density}. The
new weight functions are also made to deconvolve the Mayer function
\begin{equation}
  f(r) = \exp (-\beta V(r)) - 1
\end{equation}
but retain the same relationships as in FMT\fixme{redundant?}
\begin{align}
...
\end{equation}
This gives two options for constructing a theory from SFMT: choose a
pair potential and solve for $w_2$(difficult) or construct a $w_2$ and
find the pair potential(easier). Schmidt tested SFTM for a mixture of
soft and hard spheres as well as for star
...
repulsive pair potential and creates a hard sphere reference fluid
...
\end{align} 
where $\beta = \frac{1}{kT}$, $V(r)$ is the pair potential, and
$\sigma_{BH}$ is the radius for which $V(r)$ goes to zero. We utilize
this variable diameter and create a White Bear hard sphere DFT to
compare its behavior with the soft sphere DFT we construct in this
paper.
...
We use a WCA (Weeks-Chandler-Anderson) pair potential\cite{weeks1971},
which is the Lennard-Jones potential with the attractive portion
removed. We approximate the Mayer function as
\begin{align}

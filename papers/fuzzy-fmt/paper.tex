...
any soft repulsive potential can be reproduced  
%mapped at different temperatures 
%using a corresponding hard sphere fluid with 
by mapping it at each temperature to a fluid composed of hard spheres 
with a diameter that depends on the temperature.\cite{barker1967perturbation}
...
split the Lennard-Jones potential differently than Barker and Henderson did
...
%
%FIXME While
The WCA repulsive potential makes an excellent reference fluid for 
...
Fundamental Measure Theory~\cite{rosenfeld1989, rosenfeld1997} (FMT) -  a powerful classical Density
...
Tarazona improved on the functional by introducing a tensor weight~\cite{tarazona2000}
to eliminate 
%the problem of 
the divergences 
...
functionals~\cite{cuesta1997dimensional, roth2002whitebear, hansen2009fundamental, 
...
~\cite{shen2020inhomogeneous, sermoud2021inhomogeneous, shen2022inhomogeneous, bultmann2022inhomogeneous}.
...
Like FMT, SMFT uses a set of weighted densities 
...
%
In his 2010 review of FMT, Roth point out that the most important future
...
The weighted densities $\lbrace{n_i(\textbf{r})\rbrace}$ are
...
%step function.  
The weight functions are related by
\begin{align}
  w_1 &= \frac{w_2}{4\pi r} &
...
which is not exhibited by a WCA potential. In the next section, 
...
produced by a Barker-Henderson hard

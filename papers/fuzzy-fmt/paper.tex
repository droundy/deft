...
and with the limiting case of a zero-dimensional cavity. Tarazona's tensor 
weight eliminates the problem of divergences present in earlier versions 
of FMT so that it can be applied to freezing.
...
\item Finish freezing section with figures.
...
\begin{enumerate}
\item Go over plots and text sections II.B (ASK about text in red)
\end{enumerate}
...
\end{align}
We then make the $\alpha$ and $\Xi$ parameters temperature-dependent 
...
about the peak of the derivative of the Mayer function of the WCA potential
...
Equation~\ref{eq:gaussianw2}, is used in     
Equation~\ref{eq:mayerandw2} 
...
Next, we set the second virial 
coeffecient, $B_2$, formed using the mayer function for the $\Vwca(r)$ potential 
...
horizontal dotted lines \textcolor{red}{illustrate the two temperatures.} 
% 

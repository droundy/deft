...
\begin{align*}
  f'(r) &= \int dr' w_2 (r')w_2(r - r') \\ 
     ...
     &\times \left( B (r-r') e^{- \gamma \left(1 - \frac{r-r'}{R}
                                ...
                                (r-r')) \right) \\
     &= \alpha \int dr' r'(r-r') \exp \left [-\frac{2\gamma}{R} \left (r' -
...
\begin{align}
 f_{r \geq R}'(r) &= \alpha \int_{r - R}^R dr' r'(r-r') \exp \left [ -
   \frac {2\gamma}{R}\left( r' - \frac{r}{2} \right)^2 \right]
\end{align}
...
\int_{-\infty}^0 w_{2c}(u) du &= \frac{B^2 R^4}{2 \gamma^2}
           ...
           u e^{-2u^2} \right]du \\
           &= \frac{B^2 R^4}{2 \gamma^2} \left[ \frac{\sqrt{\pi}}{2}
             \left( \frac{-\pi}{4}(4\gamma - 1) + 2 \sqrt{\gamma}
             -\frac{2 + \pi}{4 \sqrt{\pi}} \right) - \frac{1}{4} \right].
\end{align}
\end{widetext}
...
\begin{align}
  \int f'(r)dr &= \int w_2(r)\ast w_2(r) dr\\
   1 - e^{-\beta V_0} &= \frac{B^2 R^4}{4 \gamma^2} \left[ \sqrt{\pi}
             \left( \frac{-\pi}{4}(4\gamma - 1) + 2 \sqrt{\gamma}
             -\frac{2 + \pi}{4 \sqrt{\pi}} \right) - \frac{1}{2}
             \right]
\end{align}
Which gives
\begin{align}
  B = \frac{4\gamma}{R^2} \left[ \frac{1 - e^{-\beta V_0}}{8
             \sqrt{\pi \gamma} + \pi - \pi^{3/2}(4\gamma -1) -4} \right]^{1/2} 
\end{align}
...
\begin{align}
  ...
  w_0(r) &= \frac{w_2(r)}{4\pi r^2}\\
  \mathbf{w}_{2V}(\rr) &= \frac{w_2(r)}{r}\rr\\
  \mathbf{w}_{1V}(\rr) &= \frac{w_2(r)}{4\pi r^2} \rr
\end{align}

\subsection{Weighting functions in fourier space}\

\begin{align}
  \tilde{w}_2(k) &?= \frac{i}{k} \int_{0}^{\infty} r w_2(r) \sin(kr) dr \\
  \tilde{w}_3(k) &= \\
  \tilde{w}_1(k) &= \\
  \tilde{w}_0(k) &= \\

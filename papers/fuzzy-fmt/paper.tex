...
SFMT was created to generalize the weight functions of FMT to soft
interactions while keeping the range of the weighted densities at a
distance of one sphere radius. Schmidt gives three criteria for the
weight functions: they will deconvolve the Mayer function, yield the
exact zero-dimensional single cavity limit, and give a reasonable
approximation for the multi-cavity limit~\cite{schmidt1999density}.  A
key feature of SFMT is that the weighted densities retain the same
relationship as in FMT:
\begin{align}
...
approach\cite{barker1967perturbation}. This theory is a model for
...
higher order repulsive terms. The hard sphere diameter is
\begin{align}

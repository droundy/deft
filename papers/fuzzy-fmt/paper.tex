...
any soft repulsive potential can be reproduced using 
%mapped at different temperatures 
a corresponding hard sphere fluid with 
a temperature dependent diameter. 
%a diameter sized ...
Weeks, Chandler and Anderson (WCA) developed a different approach in which
they used  a hard sphere fluid with a temperature and density dependent diameter 
to recreate specifically the soft repulsive potential that reproduces 
the repulsive force of a Lennard-Jones interaction. 
...
Fundamental Measure Theory (FMT) which is a powerful classical Density
...
roth2002whitebear} FIXME LOOK UP A FEW DIFFERENT REFERNCES THESE DONT USE
IT AS A REFERNCE FLUID.  MENTION WCA HERE??? However, the hard-sphere fluid remains a
...
Soft Fundamental Measure Theory (SFMT) is a classical Density Functional 
...
will involve treating soft repulsions
and attractions~\cite{roth2010fundamental}.


In this paper, we introduce a \textit{soft} sphere fluid which can be 
used with SFMT and which has
temperature-dependent parameters to model the soft, repulsive 
potential of a Weeks-Chandler-Andersen fluid. 
...
%
To obtain the free energy of the fluid from which the other
...
we use Schmidt's version of SFMT introduced in 2000~\cite{schmidt2000fluid}
...
The excess Helmholtz free energy of SFMT is a functional of the number density 

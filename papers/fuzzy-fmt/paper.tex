...
is widely studied and well understood, not only for the homogeneous
...
including those in the previous two chapters.
%
...
a one-dimensional integral over radius.
...
function $w_2$ The weight function for the erf model is
\begin{align}
...
repulsive pair potential, $V(r)$, and creates a hard sphere reference
fluid with a temperature dependent diameter and higher order repulsive
terms. The hard sphere diameter is
\begin{align}
  d = \int_0^{\infty}\left( 1 - e^{-\beta V(r)} \right)\mathrm{dr}.
  \label{eq:bh-diameter}
\end{align}
We utilize this variable diameter to create a White
...
\begin{align}
V_{SW}(0 < z \leq R_0) = 2\pi\rho\epsilon\Big[&
  \frac{2\sigma_W^{12}}{45} \left(\frac{1}{z^9}-\frac{1}{R_0^9}\right)
  +\frac{\sigma_W^6}{3}\left(\frac{1}{R_0^3}-\frac{1}{z^3}
  \right) \notag \\
  & + \frac{z^3-R_0^3}{6} + \left( R_0 - z \right)\left(\frac{R_0^2}{2} +
...
wall plus the radius of a sphere in the fluid. The potential is zero
when $z>R_0$, and is infinite for $z\leq 0$. Both $\epsilon$ and
$\sigma_W$ are Lennard-Jones parameters between the wall and the
fluid. $R_0$ and $\sigma_W$ are related by $\sigma_W = 2^{5/6} R_0$.

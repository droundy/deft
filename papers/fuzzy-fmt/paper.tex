...
\item Finish p vs V equation of state figure and format on all plots.
%\item Touch up the freezing figures.
\end{enumerate}

\subsection{Meeting to-do list}
\begin{enumerate}
\item go over plots and begin looking at text (and text in blue)
\end{enumerate}
...
2^{5/6} R$. \textcolor {blue}{In this paper we will use the dimensionless reduced
...
It is written as an integral of a set of functions
\begin{equation}
...
and we have used the tensor version of $\Phi_3$ formulated by 
Tarazona~\cite{tarazonaFIXME,santos2012phi3}.
%The integrands 
...
the pair potential that weighting function represents.  
%In this paper, we will 
%This is the approach we use in this paper.
This is the approach we use in this paper. We use a weighting function 
for $w_2(r)$ introduced by Schmidt in Ref.~\citenum{schmidt2000fluid} 
...
\end{align}
This model is called the \emph{error function model} (or \emph{erf model}). 
...
weighting functions is provided in Appendix~\ref{sec:fourier}.
...
Our approach is to make the $\alpha$ and $\Xi$ parameters in the erf potential 

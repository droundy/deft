...
(r) \Theta_2 (R - r)
\end{equation}
where B is a constant to be determined by fitting \fixme{stuff} and $R =
\frac{r_0}{2}$.  Now we'll first want to convolve $w_2(r)$ with
itself:
\begin{align*}
...
\begin{align}
  \int f'(r)dr &= \int w_2(r)\ast w_2(r) dr\\
...
\begin{align}
  B = \frac{2\gamma}{(\sqrt{\pi \gamma}-1)R^2} \left(
    e^{-\beta V_0}-1 \right)^{1/2}
\end{align}

\subsection{Weighting functions in real space}

\begin{align}
  w_2(r) &= B r e^{-\gamma \left ( 1 - \frac{r}{R} \right)^2}
           \Theta_1(r) \Theta_2 (R - r )\\
...
\begin{align}
  \tilde{w}_2(k) &= \frac{2\pi B}{k} \int_{0}^{\infty} r w_2(r)
  \sin(kr) dr \\
  \tilde{w}_3(k) &=
\end{align}
\begin{align}
  \tilde{w}_1(k) &= \int_0^\infty \int_0^{2\pi} \int_{-1}^{1} w_1(r)
    ...
    \\
    &= \frac{2\pi}{k} \int_{-\infty}^\infty r w_1(|r|) e^{-ikr} dr
    \\
    &= -\frac{2\pi}{k} \int_0^{R} r \frac{B}{4\pi} e^{-\gamma\left(1-\frac{r}{R}\right)^2} \sin(kr) dr
    \\
    &= \frac{B}{k} \int_{-R}^{R} r e^{-\gamma\left(1-\frac{r}{R}\right)^2} e^{-ikr} dr
    \\
    &= \frac{B}{k} \int_{-R}^{R} r e^{-\gamma\left(1-\frac{r}{R}\right)^2 - ikr} dr
\end{align}
I'm not sure about the above math---I expect there are math errors
lurking about.  So it needs to be looked over (or redone) carefully.
But I expect that there aren't fundamental errors, and the final
integral is solvable by completing the square in the exponent, and
then doing a change of variables ($u$ substitution) to turn it into an

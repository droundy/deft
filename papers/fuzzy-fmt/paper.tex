...
Inspired by Rowlinson's work~%\cite{?}
, Barker-Henderson generalized the method.
In the widely celebrated Barker-Henderson approach, any 
...
\end{align}
Weeks-Chandler-Andersen (WCA) derived a density and temperature dependent
hard sphere diameter to model a soft, repulsive potential given by~
%\cite{?}
...
%------------------------
\color{red}FIX!...through meeting the condition 
\begin{align}
  \int \big(y_d(r) e^{-\beta u_o} -1 \big)~d\vec r &= \int \big(y_d(r) e^{-\beta u_d} -1 \big)~d\vec r
\end{align} 
...
In this paper, we introduce a soft sphere reference fluid 
%with a potential that approximates the potential of
to model
a WCA fluid by making the second virial coefficent temperature dependent 
so that it can be made to match the second virial coefficent of a 
WCA fluid. 
\begin{align}
...
to obtain the free energy directly from the potential.

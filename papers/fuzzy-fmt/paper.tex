...
soft interactions developed by Schmidt~\cite{schmidt1999density}.
Like Rosenfeld's FMT~\cite{rosenfeld1989}, the free energy is written
as an integral of functions of a set of weighted densities known as
\emph{fundamental measures}:
\begin{equation}
...
\Phi_3 &= \frac{n_2^3 - 3 n_2\mathbf{n}_{V2}\cdot\mathbf{n}_{V2}^2}{24\pi(1-n_3)^2}
\end{align}
...
applicable to energetic interactions such as dominate repulsion
between molecules.  Here we will discuss the \emph{error function model}
(or \emph{erf model}) introduced by Schmidt in
Ref.~\citenum{schmidt2000fluid}.

The erf model corresponds to a Gaussian form for the weighting
function $w_2$ The approximate weight functions from the erf model are
\begin{align}
...
\caption{Reduced pressure versus temperature.  The SFMT result is
  ...
  parameters as $p^{*} \equiv p \sigma^3/\epsilon$. \fixme{Add
    Barker-Henderson to this plot.}}
\label{fig:p-vs-T}
\end{figure}
...
as is evident from Fig.~\ref{p-vs-T}.  In contrast, the very high
...
We begin with the simplest test for inhomogeneous one dimensional
...
funtions.  Results for reduced densitied below $n^* = 0.6$ have been
...
\begin{figure*}
  ...
  \end{center}
  \caption{Radial distribution functions of Argon. \fixme{Add
      description of the pressure and temperature in physical units
      for each plot.}}
...
densities from 0.6 to 1.0.\fixme{Add some description of the pressures
  and temperatures in physical units.}  For each system we compute the
radial distribution function with Monte Carlo simulation, as well as
SFMT and the Barker-Henderson approach.  For our WCA fluid and
Lennard-Jones test particle, we used the Lennard-Jones parameters

...
\begin{enumerate}
\item Go over plots and look at text I. and II.A (ASK about text in red)
\end{enumerate}
...
that is temperature-independent can only be modeled using the error 
function model if the expressions are evaluated at one temperature.
...
decreases with increasing temperature.  Next, we set the second virial 
coeffecient formed using the mayer function for the $\Vwca(r)$ potential 
...
and solve for the parameter $\Xi$ at a given temperature.
...
show the value of $\alpha$ at which the potential functions have been matched, and

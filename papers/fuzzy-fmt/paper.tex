...
\begin{align}
  d &= \int_0^{\infty}\left( 1 - e^{-\beta u(r)} \right)~\mathrm{dr}
...
\end{align}
where $A_{HS}$ is evaluated using the temperature-dependent diameter.
%\begin{align}
...
modeled by a hard sphere fluid with a density and 
...
is 
determined by making the compressibility of the hard sphere fluid equal that
of the WCA potential
through meeting the condition 
\begin{align}
  \int \big(y_{HS}(r) e^{-\beta V_{wca}} -1 \big)~d\vec r &= \int \big(y_{HS}(r) e^{-\beta V_{HS}} -1 \big)~d\vec r
\end{align} 
...
Tang~\cite{?} later arrived a result similar to that of Weeks-Chandler-Andersen 
by following along the 
line of Barker-Henderson's derivation but making the first order 
correction to the free energy vanish exactly.
\begin{align}
  \int_0^\sigma y_o(r) e^{-\beta u_{rep}(r)}r^2 ~\mathrm{dr} &= 
  \int_d^\sigma y_o(r) r^2~\mathrm{dr}
\end{align}
%-----------------------
...
\textit{soft} sphere reference fluid 
...
It is a mean-value thereom, that uses a set of weighted densities 
...
$\lbrace{w_i\rbrace}$ with forms that arise from noting that 
...
\begin{align}
    f(r) = w_0*w_3 + w_1*w_2 + \vec {w}_1*\vec {w}_2
\end{align}
SFMT extends the use of the weight functions to soft sphere fluids 
by preserving the general 
form of the relations between the weights which are all related to each other,
and replacing the particular expression used for one of the weights with 
a form compatible with a soft sphere potential from which the particular 
expressions for the other weights can be derived.
...and which 

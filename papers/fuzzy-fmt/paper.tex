...
\begin{enumerate}
\item Update Results section and enlarge font on plots
...
Rowlinson introduced a temperature dependent reduced length 
%given by
%\begin{align}
  %\rho &= \bigg(\frac{\epsilon}{k_BT}\bigg)^{\frac{1}{n}}\bigg[1+\frac{\gamma}{n}\bigg]
%\end{align}
for the inverse-power potential 
\begin{align}
  u(r) &= \epsilon\bigg(\frac{r}{R_m}\bigg)^{-n}
\end{align}
which is of a form similar to the repulsive portion of the Lennard-Jones potential 
where $\epsilon$ and $R_m$ correspond to the Lennard-Jones parameters. 
He showed that the Helmholtz free energy could then be expressed in terms
of the Helmholtz free energy of a hard sphere reference fluid adjusted 
by the temperature-dependent reduced length.
%, $\rho$
%\begin{align}
  %A_n(V,T) &= A_{HS}(V\rho^{-3}, T) - 3RT\ln\rho
%\end{align}
%and cites the correspondence of the form of this equation to that derived by 
%Longuet-Higgens~%\cite{?} 
%to take into account the change of molecular size on the 
%free energy of a conformal assembly.

Barker-Henderson generalized Rowlinson's method by developing an approach
where 
%In the widely celebrated Barker-Henderson approach, which is a generalization
%of Rowlinson's method, 
any 
repulsive pair potential, $u(r)$ can be used to construct a corresponding hard
sphere reference fluid with a temperature dependent diameter given by
\begin{align}
  d(T) &= \int_0^{\infty}\left( 1 - e^{-\beta u(r)} \right)~\mathrm{dr}
  \label{eq:bh-diameter}
\end{align}
This diameter eliminates a low-order term in the expansion of the Helmholtz
free energy about a steepness paramenter $\alpha$ that describes the 
steepness of the repulsive part of the potential, and a well-depth parameter $\gamma$
that describes the depth of the attractive part of the potential energy well. 
When both parameters are set to equal one, the resulting potential equals
the Lennard-Jones potential, whereas when both parameters are set to zero, 
the resulting potential equals that of a hard sphere fluid. When the low-order term 
referring to the steepness of the repulsive potential is eliminated in the 
expansion of the Helmholtz free energy along 
with higher order terms that are neglected, only the Helmholtz free energy 
of a hard sphere reference fluid remains along with a low-order term associated
with the well-depth parameter of the attractive potential energy. This enables
the use of perturbation theory to describe the Helmholtz free energy of 
the full potential.
\begin{align}
  A &= A_{hs} + \alpha(0) + \gamma(...) + \text{higher order terms}  \\
  A &\approx A_{hs} + \gamma(...) 
\end{align}
where the free energy of the hard sphere fluid, $A_{hs}$, is evaluated using the temperature-dependent diameter.
%\begin{align}
  %A &= A_o + \alpha stuff\bigg(d + \int_0^\sigma (1 - e^{-u(z)/k_BT})~dz \bigg) + \mathrm{...}  \\
    %&= A_o + \alpha stuff\bigg(d - d\bigg) + \mathrm{...}
%\end{align}

Weeks-Chandler-Andersen (WCA) 
created a soft, repulsive reference fluid 
modeled by a hard sphere fluid with a temperature and density
dependent hard sphere diameter.
They split the Lennard-Jones potential into two
parts differently than Barker-Henderson had done 
%where one part is 
to create 
a soft, repulsive reference potential 
%that arises from only
%which is 
associated only with the 
repulsive forces between particles 
to which a perturbative potential associated with only attractive forces
is added to recover the full Lennard-Jones potential. 
%which, when added to the reference potential
%recovers the full Lennard-Jones potential. 
The repulsive WCA potential is given by 
...
%to which 
%which, when added, 
%associated with attractive forces.
%which can be added to the reference potential
%added 
%recovers 
%to recover the full Lennard-Jones potential. 
%with is added to account for attractive forces.
%The WCA potential is modeled by a hard sphere fluid with a 
They derived a temperature and density
dependent hard sphere diameter 
%derived
%to model the soft, purely repulsive reference potential 
%for a reference fluid consisting of the soft, repulsive part of the potential which is 
%given by
%\cite{?}
%------------------------
%\color{red}
%The hard sphere diameter 
%of the corresponding hard sphere reference fluid 
%The hard sphere diameter of this reference fluid 
by making the compressibility of the hard sphere fluid equal that
of the WCA potential
through meeting the condition 
\begin{align}
  %\int \big(y_{HS}(r) e^{-\beta V_{wca}(r)} -1 \big)d\vec r &= \int \big(y_{HS}(r) e^{-\beta V_{HS}(r)} -1 \big)d\vec r
  \int y_{hs}(r) e^{-\beta V_{wca}(r)} -1 ~d\vec r &= \int y_{hs}(r) e^{-\beta V_{hs}(r)} -1 ~d\vec r
\end{align} 
where it is assumed that
\begin{align}
  y_{wca}(r) &\equiv g_{wca}(r)e^{V_{wca}(r)/k_BT} \\
         &\approx g_{hs}(r)e^{V_{hs}(r)/k_BT} \\
         &\approx y_{hs}(r)
\end{align} 
%The WCA potential reproduces
%the repulsive force of a Lennard-Jones interaction, which makes it an
%ideal model for interatomic repulsion. 
Tang~\cite{?} later arrived at a result similar to that of Weeks-Chandler-Andersen 
by following along the 
line of Barker-Henderson's derivation but making the first order 
correction to the expansion of the Helmholz free energy of a repulsive potential 
about that of a hard sphere fluid vanish exactly.
\begin{align}
  \int_0^\sigma y_{hs}(r) e^{-\beta u(r)}r^2 ~\mathrm{dr} &= 
  \int_d^\sigma y_{hs}(r) r^2~\mathrm{dr}
\end{align}
%-----------------------
%split the Lennard-Jones potential differently than BH
%  \davidsays{This might be a good place to mention more about BH and similar?}
%T
%\textcolor{red}{Given that the repulsive interaction between particles is the dominate 
%factor in determining the structure of fluids at high densities, these SITE THIS!}
%works cemented the hard-sphere
%model as the reference system of choice for the theory of
%liquids~\cite{gil-villegas-1997-SAFT-VR, clark2006developing,
  %lafitte2013accurate}.  
  
The hard-sphere fluid is well understood and
%is widely studied and well understood. 
has served as the reference system of choice for the theory of
liquids~\cite{gil-villegas-1997-SAFT-VR, clark2006developing,
  lafitte2013accurate}
not only for the homogeneous
...
In this paper, we introduce a temperature-dependent 
\textit{soft} sphere fluid 
%with a potential that approximates the potential of
to model
the soft, repulsive potential of the WCA fluid. 
The WCA potential reproduces
the repulsive force of a Lennard-Jones interaction, which makes it an
ideal model for interatomic repulsion. 
We model the WCA fluid by constructing a soft-sphere potential with 
temperature-dependent parameters, $\alpha$ and $\Xi$, 
that we use to
make the second virial coefficent of our soft sphere 
reference fluid match the second virial coeficent of the WCA fluid at 
each temperature.
%\begin{align}
    %B_2(T) &=-\frac{1}{2}\int{(e^{-\beta V(r, \alpha(T), \Xi(T))} - 1)~d{\vec{r}}}  \\
        %&=-\frac{1}{2}\int{(e^{-\beta V_{wca}(r)} - 1)~d{\vec{r}}}
%\end{align}
...
(SFMT) introduced in 1999 by Schmidt~\cite{schmidt1999density} with a 
tensor weight introduced by Tarazona~\cite{tarazona2000,santos2012phi3}
shortly after.
%without the need to refer to a hard-sphere fluid??
%
%In 1999, Schmidt introduced \emph{Soft} Fundamental Measure Theory
%(SFMT)~\cite{schmidt1999density} which 
...
exactly. Tarazona's tensor 
weight eliminates the problem of divergences present in earlier versions 
of FMT so that it can be applied to freezing and inhomogeneous fluids 
with superior results.
 %that prevented them from giving good results for freezing.

%and can be used for inhomogeneous systems.
%\begin{align}
    %A[n(\vec r)]=k_BT\int{\Phi(\lbrace{ n_i(\vec r)\rbrace})~d{\vec{r}}}
%\end{align}
%The weighted densities are formed using a set of weight functions 
%$\lbrace{w_i(\vec r)\rbrace}$ with forms that arise from noting that 
%the Mayer function of a hard sphere fluid can be expressed in terms of 
%convolutions involving the set of simple weight functions.
%\begin{align}
    %f_{hs}(r) = -2(w_0*w_3 + w_1*w_2 + \vec {w}_1*\vec {w}_2)
%\end{align}
%SFMT extends the use of the weight functions to soft sphere fluids 
%by preserving the general 
%form of the relations between the weight functions (which are all related to each other)
%and replacing the particular expression used for one of the weight functions with 
%a form compatible with a soft sphere potential from which the particular 
%expressions for all the other weights can then be derived.
%...and which 
%...preserves/satisfies ? conditions...  discuss forms of the s$\Phi$s ...

%There have been a few versions of SFMT differring in the explicit form 
%of the expression for $\Phi_3$. 

Due to its combination of
computational efficiency with accuracy, FMT has been used as the
...
%In this paper, we will apply SFMT to study the Weeks-Chandler-Andersen
%(WCA) repulsive potential~\cite{weeks1971}.  This potential reproduces
%the repulsive \textcolor{red}{force?} of a Lennard-Jones interaction, which makes it an
...
In the low-density excess limit, the free energy of a homogeneous fluid 
with a large number of particles is given by
\begin{align}\label{eq:Fexcess}
    F_{excess} &\approx k_BTn^2 B_2 V
...
\kirstiesays{I'm not sure how to tie this into how accurate our functional is at 
low densities...still working on this}
\begin{figure}
\begin{center}
\includegraphics[width=\columnwidth]{figs/w2-comparison}
\end{center}
...
\end{center}
\caption{\kirstiesays{fix dotted lines} The WCA potential, and approximations to this potential
  constructed for two different temperatures.}
\label{fig:potential-plot}
\end{figure}


Fig.~\ref{fig:potential-plot} shows the erf potential at two different 
tempertures alongside the temperature independent WCA potential.  
%
%We show the resulting approximation for the potential in 
%%Fig.~\ref{fig:potential-plot} for two different temperatures.  
%
Vertical dotted lines show the values of $\alpha$ at
which $\Verf$ and $\Vwca$ would ideally be matched, a solid black vertical line
represents the distance at which the WCA force goes to zero. 
...
\begin{figure*}
  ...
  \caption{Radial distribution functions of Argon. From left to right
    ...
    K and 1.1 GPa, and at 48 K and 9.92 MPa. \davidsays{FIXME these don't match.}}
...
A phase diagram for the WCA fluid is shown below with $\sigma=1$ and $\epsilon=1$.
...
%\begin{figure}
 ...
 %\end{center}
%\caption{Plot of the reduced pressure verses the reduced temperature. The lines bending downward indicate freezing.}

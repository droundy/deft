...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
  We demonstrate a easy-to-implement variation on the Soft Fundamental
  ...
  accuracy to the Barker-Henderson approach applied to the WCA fluid,
  combined with Fundamental Measure Theory for the hard-sphere
  fluid.
\end{abstract}
...
Like Rosenfeld's FMT~\cite{rosenfeld1989}, the excess free energy is
written as an integral of functions of a set of weighted densities
known as \emph{fundamental measures}:
\begin{equation}
...
\Phi_3 &= \frac{n_2^3 - 3
  n_2\mathbf{n}_{V2}\cdot\mathbf{n}_{V2}^2}{24\pi(1-n_3)^2}
\label{eq:sfmt-phi3}
\end{align}
...
\begin{equation}
  ...
  \label{eq:n-3}
\end{equation}
where the integral in Eq.~\ref{eq:n-3} is a one-dimensional integral
over radius.

Schmidt proves that SFMT reproduces the exact functional in
...
Ref.~\citenum{schmidt2000fluid}.  The erf model corresponds to a
Gaussian form for the weighting function $w_2$:
\begin{align}
...
from Eq.~\ref{eq:Verf}, this simple functional relationship for $w_2$
...
energy of an inhomogeneous hard-sphere fluid.  This functional differs
...
energy rather than scaled-particle theory for homogeneous systems.  We
note that the weighting function $w_2$ for a hard-sphere system is
given by the Dirac $\delta$ function:
\begin{align}
  w_2(r) = \delta(r - R),
\end{align}
where $R$ is the hard-sphere radius.

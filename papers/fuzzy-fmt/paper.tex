...
\begin{enumerate}
\item Go over plots and text sections I. and II.A (ASK about text in red)
\end{enumerate}
...
2^{5/6} R$. \textcolor {red}{Results are presented in this paper 
in terms of the dimensionless reduced
density $n^* \equiv n \sigma^3$, reduced temperature $T^* \equiv
k_BT/\epsilon$, reduced pressure $P^* \equiv P\sigma^3/\epsilon$, and 
reduced volume per particle $V^* \equiv V/\sigma^3$.}
...
FMT~\cite{rosenfeld1989}, is a functional of the number density 
...
\end{equation}
%with integrands
of \textcolor{red}{a set of functions} defined as
\begin{align}
...
\textcolor{red}{Tarazona~\cite{tarazonaFIXME,santos2012phi3} to take 
into account lost cases}. 
These are functions of a set of weighted densities $n_{i}(\textbf{r})$ 
...
functions similar to those of hard-sphere FMT. 

Like hard-sphere fundamental measures, the new weight
...
for $w_2(r)$ which has a Gaussian form as was first introduced by Schmidt~\cite{schmidt2000fluid} 
...
%Fourier transforms of the weight functions are used to solve for the 
%weighted densities in Equation~\ref{eq:n-convolution}.  
...
We begin by relating the parameter $\Xi$ to the variance of the deriviative 
of the mayer function with respect to the radius r at a given temperature.
\begin{align}
    \frac{\Xi}{\sqrt 2}= \sqrt{(<r^2>-<r>^2)} 
\end{align}
where
\begin{align}
    <r> = \frac{\int f'_{wca}(r)~r~d\vec r}{\int f'_{wca}(r)~d\vec r}
\end{align}
and 
\begin{align}
...
horizontal dotted lines illustrate the two temperatures. 
% 
...
show the value of $\alpha$ at, or near where the potential peaks and
...
temperature and overall number density simply by varying the number density profile spatially 
...
We can vary the number density profile $n(\vec{r})$ by changing the width of the gaussians s
...
that occurs when the chemical potential, or Gibbs free energy per atom g,
...
Appendix
\begin{enumerate}
\item A. Fourier Transfrom of Weight Functions $w_0(r), w_1(r), w_2(r)$, and $w_3(r)$ 
\item B. Fourier Transfrom of Vector Weight Functions $\vec{w}_1(r)$ and $\vec{w}_2(r)$  
\item C. Foureir Transform of Tensor Weight $\overleftrightarrow{w}_{m2}(r)$
\item D. show trasform of tensor weight $\overleftrightarrow{w}_{m2}(r)$ goes to zero as k goes to 0
\item E. The tensor weight components in the coordinate system (x,y,z) 
\item F. Integrals used to compute tensor weights
\item G. Deriviation of Tensor Density $n_{m2ij}(\vec{r})$ - Work out how tensor weight in Fourier space relates to the tensor density.
\item H. Deriviation of $\Phi_3$ 
\end{enumerate}

...
atoms as inherently soft with no abrupt border, but it was shown that their repulsion could
still be accurately described using a hard-sphere model, provided the
radius is chosen to be temperature dependent~\cite{rowlinson1964statistical, barker1967perturbation,
  andersen1971relationship}. This reflects the effect of collisions during which 
the particles are pushed closer together at higher temperatures. 

Rowlinson introduced a temperature dependent reduced length given by
\begin{align}
...
\end{align}
which is of a form similar to a portion of the Lennard-Jones potential 
...
of the Helmholtz free energy of a hard sphere fluid adjusted by the 
temperature dependent reduced length
\begin{align}
...
to which a perturbative potential associated with attractive forces
...
to model
the WCA fluid by making the second virial coefficent of the soft sphere 
reference fluid match the second virial coeficent of the WCA fluid at 
each temperature.
\begin{align}
    B_2=-\frac{1}{2}\int{(e^{-\beta V(r)} - 1)~d{\vec{r}}}
\end{align}
To obtain the free energy of the soft sphere fluid, from which 
thermodynamic properites can be found, 
we use Soft Fundamental Measure Theory
(SFMT) introduced in 1999 by Schmidt~\cite{schmidt1999density} with the 
...
\end{equation}
%with integrands
of the free energy densities \davidsays{Probably want to spend more space on
...
interaction and, like the Barker-Henderson diameter discussed later, 
decreases with increasing temperature. \davidsays{Maybe discuss it earlier?}

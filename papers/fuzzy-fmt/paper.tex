...
\item ( C. The tensor weight components in the coordinate system (x,y,z) )
\item D. Integrals used to compute tensor weights
\item E. Deriviation of Tensor Density $n_{m2ij}(\vec{r})$ - Work out how tensor weight in Fourier space relates to the tensor density.
\item F. Deriviation of $\Phi_3$ 
\end{enumerate}
...
\[{}\]
\subsection{Transform coordinate system}
...
Deriviation of tensor density $n_{m2ij}(\vec{r})$ from Fourier space tensor weights:

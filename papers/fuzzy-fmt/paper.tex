...
%we came to understand atoms as
...
%Rowlinson introduced temperature dependent hard spheres and Barker and Henderson 
%generalized 
A hard sphere fluid can be used as a reference fluid %with a repulsive potential
...
%which combines the perturbation of Zwanzig with Rowlinson's 
%temperature dependent hard-sphere radius
%an arbitrary potential function can be used to construct 
an arbitrary soft repulsive potential can be reproduced  
%mapped at different temperatures 
%using a corresponding hard sphere fluid with 
by mapping it at each temperature to the potential exhibited between hard spheres
%fluid composed of hard spheres 
of a diameter that depends on the temperature\cite{barker1967perturbation}. 
%This method did not work well, however, when applied to the repulsive 
%portion of the Lennard-Jones potential~\cite{hendersonperturbation1970}.
%a temperature dependent diameter
%s
%a diameter sized ...
%In reproducing the soft repulsive Lennard-Jones potential, 
Weeks, Chandler and Anderson (WCA) 
%developed a different approach in which they 
separated the Lennard-Jones potential differently than Barker and Henderson did
%in such a way as 
%and used a hard sphere fluid with a temperature and density dependent diameter 
...
%making it an ideal model for interatomic repulsion.
...
%Once a model fluid is described in terms of a hard sphere fluid, 
%its thermodynamic properties can then be found using 
%The 
...
%for the hard sphere fluid %~\cite{rosenfeld1989, rosenfeld1997}
%.
...
%from which the thermodynamic properties and structure of a fluid can be found. 
...
%~\cite{rosenfeld1989, rosenfeld1997, roth2002whitebear}.
~\cite{shen2020inhomogeneous, sermoud2021inhomogeneous, shen2022inhomogeneous, 
bultmann2022inhomogeneous}.
%FIXME LOOK UP A FEW DIFFERENT REFERNCES THESE DONT USE IT AS A REFERNCE FLUID.  MENTION WCA HERE??? 
...
%made for inhomogeneous situations like freezing.
%
% that affected predictions of freezing.
%
%Due to its combination of computational efficiency with accuracy, 
%FMT has been used as the
%basis for a wide variety of classical density
...
%To achieve this we have incorporated temperature-dependent parameters
%into our potential.
%We use Schmidt's version of SFMT he introduced in 2000~\cite{schmidt2000fluid}
%that incorporates Tarazona's tensor weight~\cite{tarazona2000}.
%
%and which has temperature-dependent parameters to model the soft, 
%repulsive potential of a Weeks-Chandler-Andersen fluid. 
...
%In this paper, we introduce a \textit{soft} sphere fluid which can be 
%used with Schmidt's version of SFMT introduced in 2000~\cite{schmidt2000fluid}
%that incorporates Tarazona's tensor weight~\cite{tarazona2000}
%and which has temperature-dependent parameters to model the soft, 
%repulsive potential of a Weeks-Chandler-Andersen fluid. 
%Since the WCA potential reproduces the repulsive force of a Lennard-Jones 
%interaction, it is an ideal model for interatomic repulsion. 
%Our results are compared with those generated by a Barker-Henderson 
%hard sphere fluid.

%
%we use Soft Fundamental Measure Theory
%introduced by Schmidt~\cite{schmidt1999density} in 1999.
...
$n(\textbf{r})$ 
%derived using dimensional crossover from the exact free energy in the zero-dimensional cavity limit~\cite{schmidt1999density}.
%and is 
...
%
%\davidsays{Probably want to spend more space on
...
Tarazona~\cite{tarazona2000,santos2012phi3} to give improved %, which produces excellent
results for the crystallization of hard spheres.
The weighted densities %$\lbrace{n_i(\textbf{r})\rbrace}$
are given by %known as \emph{fundamental measures}.
...
%Like Rosenfeld's hard-sphere fundamental measures, 
As in FMT,
the SFMT weight
...
%\begin{equation}\frac{P}{k_BTn}=1+nB_2+n^2B_3+ \cdot\cdot\cdot \end{equation}
\begin{equation}\frac{P}{k_BTn}=1+nB_2+ \cdot\cdot\cdot \end{equation}  %So ideal pressure must be greater than excess pressure
...
Outside of a small coexistence region where both liquid and solid states 
can exist together,
a pure substance is solid when the Helmholtz free energy it would have 
in its solid state is lower than what it would have in its
fluid state at the same temperature and density.
At freezing an abrupt phase transition occurs and the particular %crystalline
%a WCA fluid forms a crystalline structure. The 
structure and density that result are affected by the interatomic potential. 
A Hard sphere fluid forms a
%as well as a soft sphere fluid with a potential that 
%approaches hard spheres, will have the structure of a 
Face Centered Cubic (FCC) crystalline lattice when it freezes~\cite{Hansen} 
thus it is not suprising that 
%a soft sphere fluid like 
the WCA fluid (which has 
a potential that approaches a hard sphere potential) also forms 
%like the WCA fluid which is not too soft would also form 
an FCC crystal lattic when it freezes. %~\cite{addref!}. 
...
\begin{equation}
  n(\textbf{r})= \frac{1}{1-f_v}\sum_i \frac{1}{\left(\sqrt{2\pi}\sigma_{g}\right)^3}
  e^{-\frac{|\textbf{r}-\textbf{R}_i|^2}{2\left(\sigma_g\right)^2}}
\end{equation}
%\davidsays{missing the fraction of vacancies in the math}
...
A very small fraction of vacancies $f_v$ helps to avoid computational 
difficulties. 
...
which is minimized. 
%to find the equilibrium free energy and number density profile. 
...
% \begin{equation}
%   P =-\frac{\partial F}{\partial V}\bigg|_{T,N}
%     = -\frac{\partial f}{\partial v}\bigg|_{T,N}
% \end{equation}
% where $f$ is the Helmholtz free energy per atom, and $v$ is the volume per atom.
The pressure of the phase transition can be obtained from the point of intersection
that occurs when the Gibbs free energy per atom % $g = f+vP$
...
The Barker-Henderson (BH) fluid is known to give good results, so we 
use it as a reference to compare with our DFT results using SFMT. 
In the BH approach, an arbitrary repulsive pair potential, $V(r)$ is 
used to construct a corresponding hard sphere reference fluid with a 
temperature dependent diameter given by~\cite{barker1967perturbation}
\begin{align}
  d_{BH} = \int_0^{\infty}\left( 1 - e^{-\beta V(r)} \right)\mathrm{dr}.
  \label{eq:bh-diameter}
\end{align}
We utilize this variable diameter with the WCA repulsive potential and 
the White Bear FMT free energy functional for the hard sphere 
fluid~\cite{roth2002whitebear} as a comparison for the SFMT 
...
%results of simulations for a reference fluid.
%For our reference simulations, of a reference fluid construct 
produced by the Barker-Henderson approximation.
We will also compare our results with Monte-Carlo (MC) simulations of the WCA fluid. %The BH fluid is known
...
%\subsection{Homogeneous limit-\textcolor{red}{UPDATE}}
...
%hard-sphere functional as described earlier. 
%

The results
%shown in Figure~\ref{fig:p-vs-V_at_fixed_T} 
...
%and above, even at temperatures above the melting transition.
%
% ASK DR. ROUNDY!
%%%\textcolor{red}{ASK-Our theory %consistently 
...
%
%
%%%%\textcolor{red}{ASK:in contrast to the hard wall felt
%%%%by a rubber ball, which has a soft potential energy as the ball
%%%%changes shape.}  
%
%
%Figure~\ref{fig:hard-walls} shows the density profile
%for the WCA fluid near a hard wall.  
...
\caption{Radial distribution functions with 0.6 (top) and 1.0 (bottom)
  ...
  SFMT result, the dashed lines give Monte Carlo simulation results,
...
%\textcolor{red}{delete?:Results for reduced densities below $n^* = 0.6$ have been
%omitted as they were exact over temperature ranges from $T^*=1$ to $T^*=10$.} 
...
%temperature. The green line marks the boundary from Monte-Carlo simulations.}
...
%TEST~\cite{bultmann2022inhomogeneous}
...
%%%NOTE TO ME: \begin{equation}\frac{\sqrt{2}}{4{\pi}\sqrt{\pi}\Xi}= \frac{1}{\Xi(\sqrt{2\pi})^3}\end{equation}
...
%%set 
%%\begin{equation}{a=\frac{\Xi}{\sqrt{2}}}\end{equation}
%%\begin{equation}{w_2(r)=\frac{\sqrt{2}}{\sqrt{\pi}\Xi}e^{-\left(\frac{r-\frac{\alpha}{2}}{a}\right)^2}}\end{equation}
...
%%%\begin{equation}{\widetilde{w}_{{m2}_{y'y'}}(k)=\widetilde{w}_{{m2}_{x'x'}}(k)}\end{equation}
...
%%%\begin{equation}{\widetilde{w}_{{m2}_{z'z'}}(k)=-2\widetilde{w}_{{m2}_{x'x'}}(k)}\end{equation}
%%\[{}\]

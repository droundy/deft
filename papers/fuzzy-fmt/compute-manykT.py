...
#This program runs figs/new-melting.cpp for many different temperatures for a given denisty
...
#with comand ./compute-manykT.py --n [density] --tensor(optional)
#For list of the many other options enter ./compute-manykT.py --help
...
parser = argparse.ArgumentParser(description='Generates data for plots.')

parser.add_argument('--n', metavar='density', type=float,
                    help='reduced density - REQUIRED')
parser.add_argument('--minkT', type=float,
                    help='min temperature', default=0.01)
parser.add_argument('--maxkT', type=float,
                    help='max temperature', default=2)
parser.add_argument('--dkT', type=float,
                    help='change of temperature', default=0.05)
...
if args.maxkT:
    ...
    maxkT=2   #erf model deviates too much for temperatures higher than this (I think)
...
if args.tensor:    
    for kT in np.arange(args.minkT, args.maxkT, dkT):
        cmd = 'rq run -J manykT-kT%g-n%g-tensor' % (kT, n)
    ...
    for kT in np.arange(args.minkT, args.maxkT, dkT):
        cmd = 'rq run -J manykT-kT%g-n%g' % (kT, n)
        cmd += ' figs/new-melting.mkdat --kT %g --n %g' % (kT, n)
        cmd += ' --gwstart %g --gwend %g --gwstep %g' % (mingw, maxgw, dgw)
        cmd += ' --fv %g --dx %g' % (fv, dx)
        cmd += ' --mc-error %g --mc-constant %g --mc-prefactor %g' % (mcerror, mcconstant, mcprefactor)

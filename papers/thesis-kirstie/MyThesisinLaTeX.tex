...
\begin{figure}[h!]
    \centering
    \includegraphics[height=7cm]{plot_HardSphere_Potential.png}
    %\includegraphics[height=3.5cm]{plot_HardSphere_Potential.png}
    %\caption{Hard Sphere Potential}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=7cm]{LJ_Potential.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=7cm]{LJ_Potential_2part.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=7cm]{WCA_Potential.png}
...
\begin{figure}[h!]
   ...
   % \begin{subfigure}[b]{0.4\linewidth}
      \includegraphics[height=3.5cm]{TwoSpheres.png}
    %\end{subfigure} 
    %\begin{subfigure}[b]{0.4\linewidth}
    %  \includegraphics[height=3.5cm]{2spheres.png}
    %\end{subfigure} 
...
The WCA potential is not well suited for implementing computationally.  Therefore, an erf potential which closely approximates the WCA potential at temperatures of interest will be used instead of the WCA potential. The erf potential is given by: \begin{equation}{V_{erf}=-k_BT\ln\left[\frac{1}{2}\left(\operatorname{erf}\left(\frac{r-\alpha}{\Xi}\right)+1\right)\right]}\end{equation} where the parameter $\alpha$ is derived by setting the erf potential equal to the WCA potentail when r equals $\alpha$ \begin{displaymath}{V_{erf}(r=\alpha)=V_{WCA}(r=\alpha)}\end{displaymath} and is given by \begin{equation}{\alpha=\sigma\left(\frac{2}{1+\sqrt{\frac{k_BT}{\epsilon}\ln(2)}}\right)^\frac{1}{6}}\end{equation}  The parameter $\Xi$ is derived by matching the slope of the erf potential to the slope of the WCA potential at r=$\alpha$:\begin{displaymath}{\frac{dV_{erf}(r=\alpha)}{dr}=\frac{dV_{WCA}(r=\alpha)}{dr}}\end{displaymath} and is given by: \begin{equation}{\Xi=\frac{\alpha}{6\sqrt{\pi}\left(\sqrt{\frac{\epsilon}{k_BT}\ln(2)}+\ln(2)\right)}}\end{equation} 
...
% In these regions $\alpha$ decreases with increasing temperature, and $\Xi$ increases with increasing temperature.
 ...
 \begin{figure}[h!]
    \centering
    \begin{subfigure}[b]{0.4\linewidth}
      \includegraphics[height=4.5cm]{plot_alpha_vs_KbT.png}
      %\includegraphics[height=5cm]{plot_alpha_vs_KbT.png}
      \caption{$\alpha$ versus $k_BT/\epsilon$}
    \end{subfigure} 
    \begin{subfigure}[b]{0.4\linewidth}
      \includegraphics[height=4.5cm]{plot_xi_vs_KbT.png}
      %\includegraphics[height=5cm]{plot_xi_vs_KbT.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=7cm]{Verf.png}
    %\includegraphics[height=5cm]{Verf.png}
...
Another way to find values for $\Xi(T)$ is to find computationally the value $\Xi$ for which the second virial coefficient evaluated for the error function potential $V_{erf}(r)$ is equal to the value for the second virial coefficent evalued for the WCA potential $V_{WCA}$, with $\alpha(T)$ plugged into the equation for $B_{2erf}$ at each temperature T. A plot of $\Xi(T)$ obtained through this method is shown in Figure \ref{fig:XifromB2vsT}.
...
\begin{equation}B_{2erf}(T) = \frac{\pi}{3}\Xi^3\left[\left(\frac{\alpha^3}{\Xi^3}+\frac{3\alpha}{2\Xi}\right)\left(1+\operatorname{erf}\left(\frac{\alpha}{\Xi}\right)\right)+\frac{1}{\sqrt{\pi}}\left(\frac{\alpha^2}{\Xi^2}+1\right)e^{-\left(\frac{\alpha}{\Xi}\right)^2}\right]\end{equation}
and 
\begin{equation}B_{2WCA}=-\frac{1}{2}\int_{Volume}\left(e^{-\beta{V}_{WCA}(r)}-1\right)d\vec{r} \end{equation}
...
\begin{figure}[h!]
    \centering
    %\includegraphics[height=7cm]{Xi_from_B2.png}
    ...
    \label{fig:XifromB2vsT}
  \end{figure}
\color{red}
\noindent !ADD PLOT of Verf !!!
\noindent!ADD PLOT of df/dr=w2*w2 !!! BUT not here!
\color{black}
%\[\]
%\[\]

%----------------BEGIN Virial Expansion--------------
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=5cm]{P-T_Diagram.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=7cm]{MDTC.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=7cm]{MDTC-2.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=7cm]{MDTC-3.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=4cm]{T-n_Diagram.png}
...
%If $\mu=0$ and $V_{ext}=0$, then the Grand free energy $\Phi$ reduces to the Helmholz free energy $F$. %The second and third terms in Eq~\ref{GrandFE} are both functionals of $n(\vec{r})$, so it remains to show that $F$, and thus $\Phi$, is a functional of $n(\vec{r})$.
...
\begin{equation}\frac{U}{N}=\frac{TS}{N}-\frac{PV}{N}+\mu\end{equation}
\begin{equation}\label{d(U/N)long}d\left(\frac{U}{N}\right)=\frac{T}{N}dS+\frac{S}{N}dT+TSd\left(\frac{1}{N}\right)-\frac{V}{N}dP-\frac{P}{N}dV-PVd\left(\frac{1}{N}\right)+d\mu\end{equation}
and using the Gibbs-Duhem relation which follows from Euler's equation
\begin{equation}SdT-VdP+Nd\mu=0\end{equation}
which gives
\begin{equation}\frac{S}{N}dT-\frac{V}{N}dP+d\mu=0\end{equation}
Equation~\ref{d(U/N)long} becomes
\begin{equation}\label{d(U/N)}d\left(\frac{U}{N}\right)=\frac{T}{N}dS+TSd\left(\frac{1}{N}\right)-\frac{P}{N}dV-PVd\left(\frac{1}{N}\right)\end{equation}
%\begin{equation}dU=TdS+SdT-PdV-VdP+\mu{dN}+Nd\mu\end{equation}
\color{black}
...
After plugging in Equation~\ref{d(U/N)}, and setting $f_N=\frac{F}{N}$ this becomes
...
Thus the Helmholtz free energy will be minimized in a system that has a temperature, volume, and number of particles that are fixed. 
...
where conservation of energy was also applied. Using Eq~\ref{d(U/N)} for the surroundings, but fixing the volume and the number of particles so that $dV=0$ and $d\left(\frac{1}{N}\right)=0$ results in
...
\begin{equation}F=\text{min}_{n(\vec r)}(U_{Not.equilibrium} - TS_{Not.equilibrium}) = U_{equilibrium}-TS_{equilibrium}\end{equation} An extremum is reached when 
\begin{equation}\frac{\partial{(U-TS)}}{\partial{n(\vec r)}}=0\end{equation}
Taking this derivative gives  \color{red} CHECK PARTIAL DERVIVATIVE OF FUNCTIONAL \color{black}
...
------------------------------------------------------
%*************************END DFT minimize F/N ****************



%Before looking at the details of cDFT, let us first look at the free energy. 
%To see how cDFT is adventageous, consider what is involved in a straightforward calculation of the free energy without using Density Functional Theory. 
...
\begin{equation}{Z=}\frac{1}{N!}\frac{1}{h^{3N}}\int{...}\int{dPdQ}\exp^\frac{-H(\vec{p}_1,\vec{p}_2,...\vec{p}_N,\vec{r}_1, \vec{r}_2,...\vec{r}_N)}{k_BT}\end{equation}
...
Breaking the Hamiltonians H into its component energies:
...
\begin{equation}{Z=\frac{1}{N!}\frac{1}{h^{3N}}\int...\int{dP}\exp^\frac{-(\frac{|\vec{p}_1|^2}{2m}+\frac{|\vec{p}_2|^2}{2m}+...\frac{|\vec{p}_N|^2}{2m})}{k_BT}\int...\int{dQ}\exp^\frac{-V(\vec{r}_1,\vec{r}_2,{...},\vec{r}_N)-V_{ext}(\vec{r}_1,\vec{r}_2,{...},\vec{r}_N)}{k_BT}}\end{equation}  Multiplying by $\frac{V^N}{V^N}$ where V is the real space volume defined by: \begin{equation}{V^N=}\int{...}\int{dQ}\end{equation} 
the equation can be written:
\begin{equation}{Z=\left(V^N\frac{1}{N!}\frac{1}{h^{3N}}\int{...}\int{dP}\exp^\frac{-(\frac{|\vec{p}_2|^2}{2m}+ \frac{|\vec{p}_2|^2}{2m}+...\frac{|\vec{p}_N|^2}{2m})}{k_BT}\right)\left(\frac{1}{V^N}\int{...}\int{dQ}\exp^\frac{-V(\vec{r}_1,\vec{r}_2,{...},\vec{r}_N)-V_{ext}(\vec{r}_1,\vec{r}_2,{...},\vec{r}_N)}{k_BT}\right)
}\end{equation}  and the partitian function can be expressed as:
\begin{equation}\label{Zmultiply}{Z=Z_{ideal}Z_{excess}}\end{equation}
where:
\begin{equation}{Z_{ideal}=V^N\frac{1}{N!}\frac{1}{h^{3N}}\int{...}\int{dP}\exp^\frac{-(\frac{|\vec{p}_1|^2}{2m}+ \frac{|\vec{p}_2|^2}{2m}+...\frac{|\vec{p}_N|^2}{2m})}{k_BT}}\end{equation}
\begin{equation}{Z_{excess}=\frac{1}{V^N}\int{...}\int{dQ}\exp^\frac{-V(\vec{r}_1,\vec{r}_2,{...},\vec{r}_N)-V_{ext}(\vec{r}_1,\vec{r}_2,{...},\vec{r}_N )}{k_BT}}\end{equation} The ideal partitian function $Z_{ideal}$ is associated with an ideal gas which exhibits no interaction between the atoms - a condition approached by a gas at low density where the atoms rarely encounter one another. The excess partitian function $Z_{excess}$ accounts for the interaction of all the atoms according to their interatomic potential. The free energy can also be broken into two parts using Eq~\ref{Zmultiply}:
%\begin{equation}{F=-k_{B}T\ln(Z)}\end{equation}
...
\begin{equation}{F_{ideal}= -k_BT\ln{\left(V^N\frac{1}{N!}\frac{1}{h^{3N}}\int{...}\int{dP}\exp^\frac{-(\frac{|\vec{p}_1|^2}{2m}+ \frac{|\vec{p}_2|^2}{2m}+...\frac{|\vec{p}_N|^2}{2m})}{k_BT}\right)}}\end{equation} where 
%the integral can easily be evaluated:

\begin{displaymath}{\frac{1}{h^{3N}}\int{...}\int{dP}\exp^\frac{-(\frac{|\vec{p}_1|^2}{2m}+ \frac{|\vec{p}_2|^2}{2m}+...\frac{|\vec{p}_N|^2}{2m})}{k_BT}=\frac{1}{h^{3}}\int{d\vec{p}_1}\exp^\frac{-\frac{|\vec{p}_1|^2}{2m}}{k_BT}\frac{1}{h^{3}}\int{d\vec{p}_2}\exp^\frac{-\frac{|\vec{p}_2|^2}{2m}}{k_BT}{...}\frac{1}{h^{3}}\int{d\vec{p}_N}\exp^\frac{-\frac{|\vec{p}_N|^2}{2m}}{k_BT}}\end{displaymath}
%Each term on the right 
These integrals can conveniently be written in terms of the DeBroglie wavelength $\Lambda$:
\begin{equation}{\frac{1}{h^{3}}\int{d\vec{p}}\exp^\frac{-\frac{|\vec{p}|^2}{2m}}{k_BT}=\left(\sqrt{\frac{k_BTm}{2\pi\hbar^2}}\right)^3=\frac{1}{\Lambda^{3}}}\end{equation} 
where \begin{equation}{\Lambda =\sqrt{\frac{2\pi\hbar^2}{k_BTm}}}\end{equation} 
Rewriting equation (20) in terms of $\Lambda$ gives:
...
\begin{equation}{F_{ideal}= k_BT[N\ln(\frac{N\Lambda^{3}}{V}{)-N}]}\end{equation} Multiplying by $\frac{V}{V}$ gives an expression in terms of the number density $n=\frac{N}{V}$: \begin{equation}{F_{ideal}= k_BT\left[\frac{N}{V}\ln{\left(\frac{N\Lambda^3}{V}\right)}-\frac{N}{V}\right]}V\end{equation}
\begin{equation}{F_{ideal}= k_BT[n\ln(n\Lambda^3)-n]V}\label{eq:Fideal}\end{equation}   
For a spatially varying number density, the ideal free energy in Eq~\ref{eq:Fideal} becomes:
\begin{equation}{F_{ideal}[n(\vec{r})]= k_BT\int_{Vol}[n(\vec{r})\ln(n(\vec{r})\Lambda^3)-n(\vec{r})]d\vec{r}}\end{equation} 
\[\]
xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

Now consider the excess free energy which is written as:
\begin{equation}\label{Fexcess}{F_{excess}= -k_BT\ln{\left(\frac{1}{V^N}\int{...}\int{dQ}\exp^\frac{-V(\vec{r}_1, \vec{r}_2,...\vec{r}_N)-V_{ext}(\vec{r}_1, \vec{r}_2,...\vec{r}_N)}{k_BT}\right)}}\end{equation} 
This integral can be greatly simplified if it can be assumed that the potential energy $V(\vec{r_1},\vec{r_2},...\vec{r_N})$ is a sum of the potential energies between pairs of atoms, where each atom is paired in turn with each of the other atoms in the system. In this case $V(\vec{r}_1, \vec{r}_2,...\vec{r}_N)$ becomes: \begin{equation}{V(\vec{r_1},\vec{r_2},...\vec{r_N})=\frac{1}{2}\sum^N_i\sum^N_{j\neq{i}}V_{ij}(\vec{r_i},\vec{r_j})}\end{equation} 
...
\begin{displaymath}{\exp^{-\frac{\frac{1}{2}\sum^N_i\sum^N_{j\neq{i}}V_{ij}(\vec{r_i},\vec{r_j})}{k_BT}}=\exp^{\frac{-V_{12}(\vec{r_1},\vec{r_2})}{k_BT}}\exp^{-\frac{V_{13}(\vec{r_1},\vec{r_3})}{k_BT}}\exp^{-\frac{V_{14}(\vec{r_1},\vec{r_4})}{k_BT}}...}\end{displaymath}    \begin{equation}{\approx-(1+f_{12})(1+f_{13})(1+f_{14})...}\end{equation}
...
The function $f_{ij}$ is called a Mayer function, and is given by\begin{equation}{f_{ij}(\vec{r_i},\vec{r_j})=\exp^{-\frac{V_{ij}(\vec{r_i},\vec{r_j})}{k_BT}}-1}\end{equation} Multiplying the Mayer function terms, the excess free energy can be written as
...
\begin{equation}{F_{excess}=-k_BT\ln{\left(\frac{1}{V^N}\int{...}\int{dQ}\left[1 + \sum{f_{ij}} + \sum_{i\neq{j}\neq{k}\neq{l}}{f_{ij}f_{kl}} +\sum_{i\neq{j}\neq{k}\neq{l}\neq{m}\neq{n}}{f_{ij}f_{kl}f_{mn}} +... \right]\right)  }}\end{equation} Making a change of variables $\vec{r}=\vec{r}_i-\vec{r}_j $ we get \begin{equation}{\int{...}\int{dQ}=V^N}\end{equation}\begin{equation}{\int{...}\int{dQ}\sum{f_{ij}}=\frac{1}{2}N(N-1)V^{N-1}\int{f(r)}{d\vec{r}}}\end{equation} \begin{equation}{\int{...}\int{dQ}\sum{f_{ij}f_{kl}}=\frac{1}{8}N(N-1)(N-2)(N-3)V^{N-2}\int{f(r)}{d\vec{r}}\int{f(r)}{d\vec{r}}}\end{equation} %which becomes \begin{equation}{F_{excess}=-k_BT\ln{\left(1+ \frac{1}{2}\frac{N(N-1)}{V^{N}}V^{N-2}\int{...}\int{dQ}{f_{ij}} + .....\sum_{i\neq{j}\neq{k}\neq{l}}{f_{ij}f_{kl}} +\sum_{i\neq{j}\neq{k}\neq{l}\neq{m}\neq{n}}{f_{ij}f_{kl}f_{mn}} +...\right)}}\end{equation}% 
%FINISH!!!!! \begin{equation}{\int{...}\int{dQ}\sum{f_{ij}f_{kl}f_{mn}}=\frac{1}{48}N(N-1)(N-2)(N-3)(N-4)(N-5)(N-6)V^{N-3}\int{f(r)}{d\vec{r}}\int{f(r)}{d\vec{r}}\int{f(r)}{d\vec{r}}}\end{equation} 
and so on. In the low-density limit, the terms in the series get smaller, and the excess free energy can then be written as %begin{equation}{F_{excess}=-k_BT\ln\left(1+\frac{1}{2}\frac{N^2}{V}\int{f(r)}{d\vec{r}}+\frac{1}{8}\frac{N^4}{V^{2}}\int{f(r)}{d\vec{r}}\int{f(r)}{d\vec{r}}+ ...\right)}\end{equation}%
...
\begin{equation}{F_{excess}=-k_BT\ln{\left(exp^{\frac{1}{2}\frac{N^2}{V}\int{f(r)}{d\vec{r}}}\right)}}\end{equation} \begin{equation}{F_{excess}=-k_BT\frac{1}{2}\frac{N^2}{V}\int{f(r)}{d\vec{r}}}\end{equation}  %\begin{equation}{F_{excess}=k_BT\frac{N^2}{V}\left(-\frac{1}{2}\int{f(r)}{d\vec{r}}\right)}\end{equation}%
Thus the free excess energy per atom in the low-density limit is  \begin{equation}{\frac{F_{excess}}{N}=k_BTn\left(-\frac{1}{2}\int{f(r)}{d\vec{r}}\right)}\end{equation} where the quantity in parenthesis can be recognized as the second virial expanision coefficient typically called $B_2$. %where $x=\frac{1}{2}\frac{N(N-1)}{V^{2}}\int{...}\int{dQ}{f_{12}}$.% 
...
%(FINISH! Take P=-dF/dV and set equal to virial expansion for P to get virial coefficients in terms of the mayer functions....CLARIFY why cDFT is better than this route!)
...
%In summary, it is the aim of this paper to use these expressions, and apply cDFT to see how well these expressions and this tool predict the freezing phase transistion of a WCA fluid. 
...
%The complexity is greatly reduced making cDFT a powerful tool in modeling fluid behavior.
\[{}\]
II. METHODS
...
To implement cDFT computationally, we begin by picturing one single crystal cell. Although the crystal structure is composed of Face Centered Cubic (FCC) cells, the entire lattice can be generated with a cell one-fourth the size called a primitive cell shown by the shaded region in Figure \ref{fig:primitivecell}. The primitive cell has the shape of a parallelpiped, and contains the equivalent of one atom.
  ...
  \begin{figure}[h!]
    \centering
    \includegraphics[height=4.5cm]{PrimitiveCellLightBlue.png}
    \caption{The Primivitive Crystal Cell is shown by the shaded parallelpiped}
...
In a large collection of simple cells, the atom at the center of each cell will not realistically reside exactly at the lattice point in the center of the cell. Statistically there will typically be a normal distribution of positions about the lattice point as pictured in the far right box in Figure \ref{fig:Ensemble_Gaus}. A plot of a 2-dimensional Gaussian distribution is shown in Figure \ref{fig:Gaus_plot}. A 3-dimenstional Gaussian distribution centered at a lattice point can be used to represent a number density profile $n(\vec{r})$  which describes how the atoms are statistically distrubuted over a collection of cells.
 ...
 \begin{figure}[h!]
    \centering
    \includegraphics[height=6cm]{Gaussian_Plot.png}
...
Once again, it is important to note that the number density is the same for all these number density profiles $n(\vec{r})$. There is still a total of one atom per "bold box" volume. This prodcedure is used only to find the crystal free energy as number density profile for a liquid is homogeneous, as shown in Figure \ref{fig:homogen_denisty}.  
 ...
 \begin{figure}[h!]
    \centering
    %\includegraphics[height=4cm]{HomogeneousDen.png}
    \includegraphics[height=4cm]{Homogeneous_bold-box.png}
...
Figure \ref{fig:weight_functions} shows a 1 dimensional view of the weight functions $w_0$, $w_1$, $w_2$, $w_3$ as they vary with distance along the z-axis. In three dimensions, the weighting functions $w_0$, $w_1$, $w_2$ are spherical shells centered at $\vec{r}$. As shown in the cross-sectional view in Figure \ref{fig:W0andW3}a, $w_2$ peaks at a radius of $|\vec{r}-\vec{r'}|=\frac{\alpha}{2}$, a width determined by $\frac{\Xi}{\sqrt{2}}$, and edges that tapers off on either side of the peak giving the shell. The weighting function $w_3$, shown in Figure \ref{fig:W0andW3}b is a solid sphere up to a radius of ?FIX? after which it tapers off over a distance of ?FIX?.
 ...
 \begin{figure}[h!]
    \centering
    \includegraphics[height=6cm]{weight_functions.png}
...
\[{}\] \color{green}
%\color{red}
%ERROR\begin{displaymath}{\widetilde{w}_{{m2}_{x'x'}}(k)=4{\pi}e^{-\left(\frac{k^2\Xi^2}{8}\right)}\left[\left(\frac{\Xi}{3k\sqrt{2}}-\frac{1}{k^2}\right)\cos(\frac{k\alpha}{2})+\frac{\Xi\alpha}{12*\sqrt{2}}\sin(\frac{k\alpha}{2})\right]}\end{displaymath} 
...
%\textbf{7. Equations related to the Error Function Potential $V_{erf}$ ....:}
%\[{}\]
%\[{}\]
...
\begin{figure}[h!]
    \centering
    \includegraphics[height=3cm]{P_overlap.png}
...
%The first function, valid for $R_o >> R$, yields the dominant term to which the second function, valid for $R_o > R$,  must tend toward in the limit that $R_o$ goes to infinity. By expanding the logrythmic term (ln(1-x)= ...) in the last function, valid for $R_o < R$, the function can be expressed in terms of powers of $R_o$. 
...
%The last two functions, valid for $R_o > R$, and $R_o < R$, and their first and second derivatives must be continuous at $R_o=R$. From these considerations, factors $c_2$, $c_1$ and $c_0$ can be determnined, and using the relation 
...
--------------------------------------------------------------------------------
 \begin{figure}[h!]
    \centering
    \includegraphics[height=6cm]{SimpleCell_Gaus.png}
    \caption{A 3D Gaussian Distribution}
    \label{fig:SimpleCell_Gaus}
  \end{figure}  

The number density profile is varied, and the free energy $F[n(\vec{r})]$ is calculated for each profile until the minimum free energy is found, which is known from cDFT to be unique. This prodcedure is used only to find the crystal free energy as number density profile for a liquid is homogeneous, as shown in Figure ?? .

\[{}\]
Program Implemtation
\[{}\]
A simplified flowchart of the program created to investigate the freezing of a WCA fluid is shown on pages x-x. The program takes four inputs: reduced number density n*, reduced temperature t*, a range of Gaussian widths gw, and a range of fraction of vacancies fv.  Given values for n* and t*, the program varys gw and fv and compares the free energies of the different number density profiles to find the minimum free energy for that combination of n* and t*.

******startA
In the Monte-Carlo Integration vectors are generated randomly according to a Gaussian distribution, the weighted density is computed for each vector, and the average of all the results gives the final weighted density. .....
ABOUT R!!
\begin{displaymath}{n_i += \frac{(1-fv)w_i}{NumPoints}}\end{displaymath}

The result can be made more accurate by increasing NumPoints which is the number of vectors randomly generated. In order to further reduce error, antithetic variates are used. An atithetic variate is an equal, but opposite point at which a function is evaluated. The final result is then given by the average of the two results. This reduces first order error by filtering out....

\begin{displaymath}{n_i += \frac{(1-fv)\left[\frac{w_i(\vec{r}) +w_i(-\vec{r})}{2}\right]}{NumPoints}}\end{displaymath}


(...........Find weights $w_0(|(\vec{r}-\vec{R_j}) - \vec{r'}|)$,  $w_1(|(\vec{r}-\vec{R_j}) - \vec{r'}|)$, $w_2(|(\vec{r}-\vec{R_j}) - \vec{r'}|)$, $w_3(|(\vec{r}-\vec{R_j}) - \vec{r'}|)$ and compute weighted densities.........)
******endA


***startB  old - before corrected center of weight function 
For a vector $\vec{r}=\vec{r}_k$ within the primitive cell, a vector $\vec{R}_j$ to the center of one of the Gaussians is selected that is within the primitive cell, or one of the neighboring cells within a prudently picked inclusion radius as Gaussians farther away will not contriubute much. The weighted densities $n_0(\vec{r}_k)$, $n_1(\vec{r}_k)$, $n_2(\vec{r}_k)$, $n_3(\vec{r}_k)$ at $\vec{r}_k$ are then computed using the weight functions $w_0(|\vec{r}_k-\vec{R_j}-\vec{dr}|)$, $w_1(|\vec{r}_k-\vec{R_j}-\vec{dr}|)$, $w_2(|\vec{r}_k-\vec{R_j}-\vec{dr}|)$, $w_3(|\vec{r}_k-\vec{R_j}-\vec{dr}|)$ where $\vec{dr}$ is a vector from the Gaussian centered at $\vec{R}_j$ to the center of the weight function. Monte-Carlo integration works by randomly generating $\vec{dr}$ according to a Gaussian distribution.
***endB

\begin{figure}[h!]
    \centering
    \includegraphics[height=4cm]{SimpleCell_Gaus.png}
    \caption{A homogeneouse number density profile $n(\vec{r})$}
    \label{fig:homogen_denisty}
  \end{figure} 
 ...
 \begin{figure}[h!]
    \centering
    \includegraphics[height=4cm]{W0.png}
    \caption{A crossectional view of weight function $w_2$ which is a spherical shell with soft edges in 3D. Weight functions $w_0$ and $w_1$ have a similar shape.}
    \label{fig:W0}
    \end{figure} 

 \begin{figure}[h!]
    \centering
    \includegraphics[height=4cm]{W3.png}
    \caption{A crossectional view of weight function $w_3$ which is a sphere with soft edges in 3D.}
    \label{fig:W3}
    \end{figure} 

////////alpha and Xi vary with temp:
\begin{figure}[h!]
    \centering
    \includegraphics[height=4.7cm]{plot_alpha_vs_KbT.png}
    \caption{Shows how $\alpha$ varies with temperature}
    \label{fig:alpha_vs_T}
  \end{figure} 

 \begin{figure}[h!]
    \centering
    \includegraphics[height=4.7cm]{plot_xi_vs_KbT.png}
    \caption{Shows how $\Xi$ varies with temperature}
    \label{fig:xi_vs_T}
  \end{figure} 

/////weight equations
\begin{equation}{n_i(\vec{r})= \int{n(\vec{r'})w_i(|\vec{r}-\vec{r}'|)} {d}\vec{r}'}\end{equation} and the shifted weighting functions $w_0$, $w_1$, $w_2$, $w_3$ are given by:
\begin{equation}{w_0(|\vec{r}-\vec{r}'|)= \frac{w_2}{4\pi(|\vec{r}-\vec{r}'|)^2}}\end{equation} 
\begin{equation}{w_1(|\vec{r}-\vec{r}'|)= \frac{w_2}{4\pi(|\vec{r}-\vec{r}'|)}}\end{equation} 
\begin{equation}{\vec{w}_1(|\vec{r}-\vec{r}'|)= w_1\frac{\vec{r}-\vec{r}'}{|\vec{r}-\vec{r}'|}}\end{equation} 
\begin{equation}{}\end{equation} 
\begin{equation}{\vec{w}_2(|\vec{r}-\vec{r}'|)=w_2\frac{\vec{r}-\vec{r}'}{|\vec{r}-\vec{r}'|}}\end{equation}
\begin{equation}{w_3(|\vec{r}-\vec{r}'|)={\int_r}^{\infty}{w_2(|\vec{r}-\vec{r}'|)}{d}\vec{r}'}\end{equation} 
In this paper, w2 is given by ....
\begin{equation}{w_2(|\vec{r}-\vec{r}'|)=\frac{1}{\Xi\sqrt\pi}\exp^-\frac{(|\vec{r}-\vec{r}'|-\frac{\alpha}{2})^2}{\Xi^2}}\end{equation}
which gives:
\begin{equation}{w_3(|\vec{r}-\vec{r}'|)=\frac{1}{2}}\left(1-erf(\frac{(|\vec{r}-\vec{r}'|)-\frac{\alpha}{2}}{\Xi})\right)\end{equation} where the two parameters $\alpha$ and $\Xi$ are given in equations (3) and (4).


%BEGIN SFMT--------------------------------BEGIN SFMT----------------------------------
%Unlike a standard weighted density approximation, 
Soft Fundamental Measure Theory (SFMT) is used to obtain an expression for $F_{excess}$ in terms of a set of functions $\Phi_1$, $\Phi_2$, and $\Phi_3$ 
\begin{equation}{F_{excess} = k_BT(\Phi_1+\Phi_2+\Phi_3)V}\end{equation} \color{red} which in turn depend on a set of weighted densities $n_0, n_1, n_2, n_3$ that arise naturally in the expression for the excess free energy derived from scaled particle theory. %The weighted densities $n_0, n_1, n_2, n_3$ arise naturally in the expression for the excess free energy derived from scaled particle theory. 


%SFMT uses a set of weighted densities $n_0, n_1, n_2, n_3$ that arise naturally in the expression for the excess free energy derived from scaled particle theory. 
 %There is no one single weight function, however, that can effectively describe the coarse density. Rather, a set of weights will do the job, each giving rise to its own "weighted density". 
%\begin{equation}{F_{excess} = k_BT(\Phi_1+\Phi_2+\Phi_3)V}\end{equation}

\begin{equation}{F_{excess}=k_BT\left(-n_0ln(1-n_3)+\frac{n_1n_2}{1-n_3}+\frac{n_2^3}{24\pi(1-n_3)^2}\right)V}\end{equation}
where \begin{equation}n_i=nw_i\end{equation}
\begin{equation}w_0=\frac{w_2}{4\pi{R^2}}=1\end{equation}
\begin{equation}w_1=\frac{w_2}{4\pi{R}}=R\end{equation}
\begin{equation}w_2=4\pi{R}^2\end{equation}
\begin{equation}w_3=\frac{4}{3}\pi{R}^3\end{equation}
\color{black}
For a spatially varying number density, $F_{excess}$ becomes a functional of the number density profile $n(\vec{r})$ :
%SFMT extends these equations to a spatially varying number density
\begin{equation}{F_{excess}[n(\vec{r})]= k_BT\int(\Phi_1(\vec{r})+\Phi_2(\vec{r})+\Phi_3(\vec{r}{)) d}\vec{r}}\end{equation} 
\begin{equation}{F_{excess}[n(\vec{r})]=k_BT\int\left(-n_0ln(1-n_3)+\frac{n_1n_2-\vec{n_1}\mbox{~\scriptsize\textbullet~}\vec{n_2}}{1-n_3}+  \frac{n_2^3\left(1-{(\frac{\vec{n_2}\mbox{~\scriptsize\textbullet~}\vec{n_2}}{{n_2}^2})}\right)^3}{24\pi(1-n_3)^2 }\right){d\vec{r}}}\end{equation}
where \begin{equation}{n_i(\vec{r})= \int{n(\vec{r'})w_i(|\vec{r}-\vec{r}'|)} {d}\vec{r}'}\end{equation}
%which reduces for a uniform number density to
\color{red}

%///////Compare this to what is involved with a calculation of the free energy using classical Density Functional Theory.//////


\begin{equation}{\Phi_1= -n_0\ln(1-n_3)}\end{equation} 
\begin{equation}{\Phi_2= \frac{n_1n_2-\vec{n_1}\mbox{~\scriptsize\textbullet~}\vec{n_2}}{1-n_3}}\end{equation} 
\begin{equation}{\Phi_3=\frac{n_2^3\left(1-{(\frac{\vec{n_2}\mbox{~\scriptsize\textbullet~}\vec{n_2}}{{n_2}^2})}\right)^3}{24\pi(1-n_3)^2}}\end{equation} %where the weighted densities $n_0$, $n_1$, $n_2$, $n_3$ are given by:

%\begin{equation}{n_i(\vec{r})= \int{n(\vec{r'})w_i(|\vec{r}-\vec{r}'|)} {d}\vec{r}'}\end{equation} and the weighting functions $w_0$, $w_1$, $w_2$, $w_3$ are given by:

\begin{equation}{w_0(r)= \frac{w_2(r)}{4\pi{r^2}}}\end{equation} 
\begin{equation}{w_1(r)= \frac{w_2(r)}{4\pi{r}}}\end{equation}
\begin{equation}{w_2(r)=-\frac{\partial{w_3(r)}}{\partial{r}}}\end{equation}  
\begin{equation}{\vec{w}_1(r)= w_1(r)\frac{\vec{r}}{|\vec{r}|}=\frac{w_2(r)}{4\pi{r}}\frac{\vec{r}}{|\vec{r}|}}\end{equation} 
\begin{equation}{\vec{w}_2(r)=w_2(r)\frac{\vec{r}}{|\vec{r}|}}\end{equation}

%\begin{equation}{w_3(r)={\int_r}^{\infty}{w_2(r')}{dr'}}\end{equation} 
\color{black}
In this paper, $w_3(r)$ is given by 
\begin{equation}{w_3(r)=\frac{1}{2}}\left[1-erf\left(\frac{r-\frac{\alpha}{2}}{\frac{\Xi}{\sqrt{2}}}\right)\right]\end{equation} 
as proposed by Schmidt$^{(?)}$ where $w_3$ is subject to the boundary conditions $w_3(0)\approx{1}$ and $w_3(inf)=0$. These boundary conditions are satisfied if $\Xi{\sqrt{2}<<\alpha}$, or $k_BT << 2.21$. Solving equation (??) for $w_2(r)$
%\begin{displaymath}{\frac{dw_3(r)}{dr}\bigg|_{infinity}-\frac{dw_3(r)}{dr}\bigg|_{r}=w_2(r)}\end{displaymath} 
we get \begin{equation}{ w_2(r)=\frac{\sqrt{2}}{\Xi\sqrt\pi}\exp^{-\left(\frac{r-\frac{\alpha}{2}}{\Xi/\sqrt{2}}\right)^2} }\end{equation} From the deconvolution equation \begin{equation}{-\frac{1}{2}f(r)=w_0*w_3 + w_1*w_2 -\vec{w}_{V1}*\vec{w}_{V2}}\end{equation} it follows that $w_2$ must satisfy the relation
\begin{equation}{\frac{df(r)}{dr}=\int_{-inf}^{inf}{w_2(r')w_2(r-r')dr'}}\end{equation}  Solving equation (??) for the Mayer function
\begin{displaymath}{\frac{df(r)}{dr}=\frac{1}{\Xi\sqrt{\pi}}e^{-\left(\frac{r-\alpha}{\Xi}\right)^2}}\end{displaymath} \begin{displaymath}{f(r)=\int_{inf}^r{ \frac{1}{\Xi\sqrt{\pi}}e^{-\left(\frac{r'-\alpha}{\Xi}\right)^2}{dr'}}}\end{displaymath} we get
\begin{equation}{f(r)=-\frac{1}{2}\left[1-erf\left(\frac{r-\alpha}{\Xi}\right)\right]}\end{equation} This gives an interatomic potential of \begin{displaymath}{V_{erf}=-k_BTln(f(r)-1)}\end{displaymath} \begin{equation}{V_{erf}=-k_BTln\left[\frac{1}{2}\left(erf\left(\frac{r-\alpha}{\Xi}\right)+1\right)\right]}\end{equation} where the two parameters $\alpha$ and $\Xi$ are given in equations (3) and (4), and are determined to make $V_{erf}$ match $V_{WCA}$ as closely as possible.

%The SFMT equations and expressions for $w_2$, $\Xi$ and $\alpha$ are developed in prior work. It is not the aim of this paper to enter into the details of SFMT, but rather to take these given expressions, and apply cDFT to see how well these expressions and this tool predict the freezing phase transistion of a WCA fluid. 

%END SFMT--------------------------------END SFMT----------------------------------

\color{red} %Virial Expansion
%\begin{equation}U=TS-PV+\mu{N}\end{equation}
%%%\begin{equation}U-\mu{N}=TS-PV\end{equation}
%\begin{equation}e^{-\beta(U-\mu{N})}=e^{-\beta(TS-PV)}\end{equation}
%%%\begin{equation}e^{-\beta(U-\mu{N})}=e^{-\frac{TS}{k_BT}+\frac{PV}{k_BT}}\end{equation}
%\begin{equation}e^{-\frac{(U-\mu{N})}{k_BT}}e^{\frac{S}{k_B}}=e^{\frac{PV}{k_BT}}%\end{equation}
%%\begin{equation}e^{-\beta(U-\mu{N})}e^{\ln\Omega}=e^{\frac{PV}{k_BT}}\end{equation}
%\begin{equation}e^{-\beta(U-\mu{N})}e^{-\sum{P_i}\ln{P_i}}=e^{\frac{PV}{k_BT}}\end{equation}
%%%\begin{equation}e^{-\beta(U-\mu{N})}\Omega=e^{\frac{PV}{k_BT}}\end{equation}
%%%\begin{equation}\ln(e^{-\beta(U-\mu{N})}\Omega)=\ln(e^{\frac{PV}{k_BT}})\end{equation}
%%\begin{equation}\ln(e^{-\beta(U-\mu{N})}\Omega)=\frac{PV}{k_BT}~~~~~~\text{for N particles in microcanonical ensemble?}\end{equation}
%%%\begin{equation}\ln(Z_{Grand})+\Omega=\frac{PV}{k_BT}\end{equation}
%%\begin{equation}\ln((e^{\beta\mu})^Ne^{-\beta{U}}\Omega)=\frac{PV}{k_BT}~~~~~~\text{for N particles in microcanonical ensemble?}\end{equation}
%\begin{equation}\ln((e^{\beta\mu})^Ne^{-\beta{U_N}-\sum{P_i}\ln{P_i}})=\frac{PV}{k_BT}~~~~~~\text{for N particles in microcanonical ensemble?}\end{equation}
\color{black}
...
Starting with Euler's equation
\begin{equation}U=TS-PV+\mu{N}\end{equation}
\begin{equation}\frac{U}{V}=\frac{TS}{V}-\frac{PV}{V}+\frac{\mu{N}}{V}\end{equation}
\begin{equation}\label{d(U/V)long}d\left(\frac{U}{V}\right)=\frac{T}{V}dS+\frac{S}{V}dT+TSd\left(\frac{1}{V}\right)-dP+\mu{dn}+nd\mu\end{equation}
and using the following relation from Euler's equation
\begin{equation}SdT-VdP+Nd\mu=0\end{equation}
which gives
\begin{equation}\frac{S}{V}dT-dP+nd\mu=0\end{equation}
Equation~\ref{d(U/V)long} becomes
\begin{equation}\label{d(U/V)}d\left(\frac{U}{V}\right)=\frac{T}{V}dS+TSd\left(\frac{1}{V}\right)+\mu{dn}\end{equation}
%\begin{equation}dU=TdS+SdT-PdV-VdP+\mu{dN}+Nd\mu\end{equation}
The Helmholtz free energy F is defined as 
\begin{equation}F=U-TS\end{equation}
\begin{equation}\frac{F}{V}=\frac{U}{V}-\frac{TS}{V}\end{equation}
\begin{equation}d\left(\frac{F}{V}\right)=d\left(\frac{U}{V}\right)-d\left(\frac{TS}{V}\right)\end{equation}
\begin{equation}d\left(\frac{F}{V}\right)=d\left(\frac{U}{V}\right)-\frac{T}{V}dS-\frac{S}{V}dT-TS d\left(\frac{1}{V}\right)\end{equation}
After plugging in Equation~\ref{d(U/V)}, and setting $f_V=\frac{F}{V}$ this becomes
\begin{equation}df_V=-\frac{S}{V}dT+\mu{dn}\end{equation}
This result shows that the natural variables of the Helmholtz free energy per volume $f_V$ are temperature and number density
\begin{equation}f_V(T,n)\end{equation}
It is the Helmholtz free energy per volume $f_V$ that will be minimized in a system that has a temperature and number density that are fixed. 


------------------------------------------------------
%*************************END DFT minimize F/V ****************



-------------------------this blue/red part basically garbage------------------:
\color{blue}
A faster, and much easier way to find the excess free energy is to use classical Density Functional Theory (cDFT). As stated earlier, in cDFT the free energy is expressed as a functional of the number density $F[n(\vec{r})]$. 
%The ideal free energy in Eq~\ref{eq:Fideal} can be expressed in terms of a spatially varying number density $n(\vec{r})$. 

%For a spatially varying number density, the ideal free energy in Eq~\ref{eq:Fideal} becomes:
%\begin{equation}{F_{ideal}[n(\vec{r})]= k_BT\int_{Vol}[n(\vec{r})\ln(n(\vec{r})\Lambda^3)-n(\vec{r})]d\vec{r}}\end{equation} 

To form an expression for the excess free energy as a functional of the number density, Soft Fundamental Measure theory (SFMT) is used. The excess free energy of a fluid can be expressed in general as an integral over an excess free energy density $\frac{F}{V}$.
\begin{displaymath}{F_{excess}=\int{\frac{F}{V}{dV}}}\end{displaymath}
Rewriting this in terms of the excess free energy per particle $\phi=\frac{F}{N}$, or in terms of $\Phi=\frac{1}{k_BT}\frac{F}{N}\frac{N}{V}=\frac{\phi{n}}{k_BT}$ we get
%$\frac{F}{V}=\frac{F}{N}\frac{N}{V}=\phi{n}$  
\begin{displaymath}{F_{excess}=\int{\frac{F}{N}\frac{N}{V}{dV}}=\int{\phi{n}{dV}}=k_BT\int\Phi{dV}}\end{displaymath} Showing explicitly the dependence of $\phi$ on the density at each point this becomes
\begin{displaymath}{F_{excess}=\int{\phi(n(\vec{r}))n(\vec{r}){d\vec{r}}}=k_BT\int\Phi(n(\vec{r})){d\vec{r}}}\end{displaymath}
where $n(\vec{r})$ is a smoothly varying function. \color{red} However, there is no real smoothly varying function when looking closeup. The real density is coarse, being zero where there are no atoms, or spheres located in a volume, and having a maximum of one at the center of each sphere over a volume of one sphere. In other places, with various portions of a sphere within a volume of one sphere, it is somewhere in between. To account for, and describe this coarse density as a function of position, the smoothly varying density function is muliplied by a weight function which is centered about each point in space. The weighted density as a function of position then becomes
\begin{displaymath}{n_{weighted}(\vec{r})=\int{n(\vec{r})w(|\vec{r}-\vec{r}'|){d\vec{r}}}}\end{displaymath}
%In a weighted density approximation, the excess free energy per particle $\phi$ is expressed in terms of the weighted density \begin{displaymath}{F_{excess}\approx\int{\phi(n_{weighted}(\vec{r}))n(\vec{r}){d\vec{r}}}}\end{displaymath} or, setting $\Phi=\frac{\phi{n}}{k_BT}$ \begin{displaymath}{F_{excess}\approx{k_BT}\int{\Phi(\vec{r}){d\vec{r}}}}\end{displaymath}  

%(CAREFUL- probability density!)
%(CAREFUL- weighted density is a weighted average!)
\color{black}
-------------------------------end basically garbage------------------

Another way to form an expression for the excess Helmholtz free energy as a functional of the number density profile $n(\vec r)$ is to use Soft Fundamental Measure theory (SFMT). SFMT extends Fundamental Measure Theory (FMT) developed for hard sphere fluids to those with soft potentials like that of the WCA fluid. 


%In summary, it is the aim of this paper to use these expressions, and apply cDFT to see how well these expressions and this tool predict the freezing phase transistion of a WCA fluid. 

In summary, classical Density Functional Theory (cDFT) in conjunction with Soft Fundamental Measure Theory (SFMT) provides a method by which the free energy can be found simply and directly without taking into account the interaction each sphere, or atom, has with every other sphere. For a number density n that is constant everywhere, as would be expected for a homogeneous fluid in equilbrium, the free energy can be written as a function of n where the ideal Helmholtz free energy can be determined analytically, and SFMT is used to form an expresssion for the excess Helmholtz free energy
\begin{equation}{F(n)=F_{ideal}(n) + F_{excess}(n)}\end{equation} 
For a spatially varying number density, as would be expected for a crystal, the free energy can be written as a functional of $n(\vec{r})$ where again SFMT is used to form the expression for the excess Helmholtz free energy.
\begin{equation}{F[n(\vec{r})]=F_{ideal}[n(\vec{r})] + F_{excess}[n(\vec{r})]}\end{equation} 
To solve for the crystal free energy $F[n(\vec{r})]$ using cDFT all that is needed are number density profiles, such as Gaussian profiles that depend only on $\vec{r}$. 
\begin{equation}{n(r)=\frac{1}{\left(\sqrt{2\pi}\sigma\right)^3}\int\exp^{-\frac{|\vec{r}-\vec{R}|^2}{2\sigma^2}}d\vec{r}}\end{equation} The profiles are varied until the free energy is minimized. The minimum free energy and its corresponding number density profile are the equilibrium values for that temperature. 
%The complexity is greatly reduced making cDFT a powerful tool in modeling fluid behavior.

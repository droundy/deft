...
$\vec{r'}$ according to a Gaussian distribution. 
...
Only lengths for $\vec{r'}$ that are within an inclusion radius
...
Gaussian centered about a lattice site is given by
%lattice point located at $\vec R_j$ is given by
\begin{equation}{n_i(\vec{r})= \int_{r'}{(1-f_v)n(\vec{r'})w_i(|\vec{r}-\vec{r}'|)} {d}\vec{r}'}\end{equation} 
and is implemented computationally in the form
\begin{equation}{n_i(\vec{r})= \sum_{r'}{(1-f_v)n(\vec{r'})w_i(|\vec{r}-\vec{r}'|)dV}}\end{equation} 
which can be written as
\begin{equation}{n_i(\vec{r})= \sum_{NumPoints}(1-f_v)P_{r'}w_i(|\vec{r}-\vec{r}'|)}\end{equation} 
where NumPoints is the number of vectors $\vec{r'}$ randomly generated, 
...
from the Gaussians around each lattice point which are then added together 
to get the final value of the weighted density. 
...
number of vectors randomly generated.
 ...
 \begin{figure}[h!]
    ...
    \caption{A Gaussian distribution about a lattice site is shown in blue, and a cross-sectional 
    view of $w_2$ centered at $\vec{r}'$ is shown in purple.} 
...
where the maximum value that $n_3$ (which equals $\overline{w}_3$) can have without error is 1.

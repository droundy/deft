...
In particular, SFMT is used to form the expression for the excess Helmholtz 
free energy without having to take into account the interaction each sphere, 
or atom, has with every other sphere. The excess free energy is then added 
to the ideal free energy calculated analytically to find the resulting 
...
A homogeneous number density profile is shown in Figure \ref{fig:homogen_denisty}.
...
For a given combination of number density and temperature, a suite of number 
density profiles are generated.
% like those represented by Figure~\ref{fig:Ensemble_vary}.
The Helmholtz free energy $F[n(\vec{r})]$ is calculated for each number density profile,
and also for the homogeneous number denisty profile. 
Both the ideal and excess free energies are found, and added together to get the total Helmholtz 
free energy.
...
The process is then repeated to find the free energies at other combinations 
of number densities and temperatures. 
...
be in the crystalline state. If the crystal free energy is higher than 
the homogeneous free energy, then the fluid will be in the liquid state. 
...
just by which energy is lower at that particular number density and
temperature. Maxwell's Double Tangent Construction must be implemented 
using the date collected to determine the actual crystal and liquid states, 
...
\begin{equation}{n_i(\vec{r})=\int_{Volume}{n(\vec{r'})w_i(|\vec{r}-\vec{r'}|)d{\vec{r'}}}}\end{equation}
where the number density profile $n(\vec r)$ within the integral is given by a Gaussian function,
%\begin{align}
...
from Equation~\ref{} with the varying number density profile given by
%\begin{align}

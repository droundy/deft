...
indicate a phase transition, and show that the solid and the liquid 
both have the same temperature and the same pressure at a phase transition.
...
Classical Density Fucntional Theory will be used to find the 
Helmholtz free energies of the liquid and the solid for various 
...
%The reason the minimization works is because DFT satisfies the conditions of the 
...
And so over time teh free energy settles to a minimum. 
...
%~\cite{schroeder}. 
A proof of minimization in general
...
SFMT is an extension of Fundamental Measure Theory (FMT)~\cite{rosenfeld1989}
that was developed for the hard-sphere fluid. FMT can be derived from
...
is added, but $V_{diff}$ will be zero if no sphere is added. 
...
occupied by the spheres divided by the total volumet avaliable, 
...
overlapping with another sphere. Combining  Equations~\ref{workrev} 
...
at an expression for the excess Helmholtz free energy of 

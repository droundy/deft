...
attraction, and so the repulsive potential, which is descriptive of 
...
which is known as the Weeks-Chandler-Anderson model fluid~\cite{andersen1971relationship} . 
...
Percus (1963)~\cite{lobowitzandpercus}, and Mermin (1965)~\cite{merium} amoung others. A review
...
For some more recent developments, see the review of new developments in 
...
and a generalization of the Barker-Henderson model to inhomogeneous 
fluids by Tschopp and coworkers~\cite{BHextended} (2020).
...
crystal as a highly inhomogeneous liquid~\cite{ramakrishnanandyussouff} (1979). 
Oettel and fellow researchers studied hard-sphere crystals with fully developed FMT~\cite{Oettel2010} (2010). 
Mortazavifar and Oettel studied the fluid and crystal phases of the 
Asakura-Oosawa model fluid with FMT~\cite{asakuraoosawa} (2016). 
Lutsko and Lam studied studied crystallization of the Leonard-Jones 
fluid~\cite{lutsko} (2018), and Lutsko and Schoonen looked at cDFT applied 
to the solid state~\cite{lutskoschoonen2020} (2020).
...
\begin{equation}\label{eq:Fexcess}{\frac{F_{excess}}{N}\approx k_BTn\left(-\frac{1}{2}\int{f(r)}{d\vec{r}}\right)}\end{equation} 
...
\end{align}
Rosenfeld also recognized that the Heaviside function can be written as 
...
\end{align}
where the weighted densities $n_\alpha$ all depend on the spatially varying 
...
\begin{align} 
      ...
      \vec {w}_1 &= w_1~\hat r
\end{align}
a tensor weight function is given by

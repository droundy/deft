...
with one another like billiard balls~\cite{lucretius}. This model, called the Hard-Sphere fluid, has served as 
...
In the late 1950s, Scaled Particle Theory (SPT) was developed which provided an expression for the 
excess Helmholtz free energy of a homogeneous Hard-Sphere fluid. 
%This .... allowed for ... 
In the 1960s Percus-Yevick applied Density Functional Theory (DFT) to find an expression 
for the excess Helmholtz free energy of an inhomogeneous Hard-Sphere fluid. In the late 1960s, and early 1970s, 
Barker and Henderson adapted the DFT method to use with fluids composed of 
spheres with softly fading potential functions rather than the hard edges of the spheres 
in the Hard-Sphere fluid. This was done by making the radius of the soft spheres temperature dependent, 
so that at low temperatures the fluid could be modeled by hard spheres with large radii, and at high 
temperatures the fluid could be modeled by hard spheres with smaller radii. This is because at higher 
temperatures, the soft-spheres would penetrate into each other farther when they collide. 
Weeks, Chandler, and Anderson studied the soft repulsive portion of the Leonard-Jones potential, 
which became known as the Weeks-Chandler-Anderson model fluid. 

In 1989 Rosenfeld introduced Fundamental Measure Theory (FMT) to form an expression for the excess 
Helmholtz free energy of an inhomogeneous hard sphere fluid based on the results from SPT. In 
19xx Schmidt adopted FMT for soft-sphere atoms to form Soft Fundamental Measure Theory (SFMT). In this paper, 
Density Functional Theory will be used to predict the freezing of a Weeks-Chandler-Anderson model fluid 
by applying SFMT to form the expression for the excess Helmholtz free energy, and the results will be 
compared to those generated from Monte-Carlo simulations.
%\[{}\]
\newline
\newline

...
\begin{align}
      F_{excess} &= k_BTn^2\left(-\frac{1}{2}\int{f(r)}{d\vec{r}}\right)V   \\
                 ...
                 &+ \color{white}\left[\color{blue}\left(\int n\delta(r-R) d\vec r\right)\left(\int n\frac{\delta(r-R)}{4\pi R} d\vec r \right) \right]\color{blue}V   \\          
...
\end{align}
which are added for the relation to be satisfied, 
...
\begin{align}
     F_{excess} &= k_BT\int d\vec r_i \int d\vec r_j~n(\vec r_i)~n(\vec r_j)~\left(\frac{-f(|\vec r_i-\vec r_j|)}{2}\right)  \\
                ...
                &+ \color{white}\left(\color{blue}\int w_1(\vec u - \vec r_i)w_2(\vec u - \vec r_j) d\vec x ~-\int \vec {w}_1(\vec u - \vec r_i)\cdot\vec {w}_2(\vec u - \vec r_j) d\vec u\right)    \\
                ...
                &+ \color{white}\left[\color{blue} \int n(\vec r_i) \vec {w}_1(\vec u - \vec r_i)~d\vec r_i \cdot \int n(\vec r_j) \vec {w}_2(\vec u - \vec r_j)~d\vec r_j \right]\color{blue}d\vec u  \\
                ...
                &= k_BT\int \Phi(\{n_\alpha(\vec r)\})~d\vec r             
\end{align}
where the weighted densities $n_\alpha$ have the form
\begin{align}
    n_\alpha(\vec r)=\int n(\vec r')w_\alpha(\vec r-\vec r')~d\vec r'  
...
%
The resulting expression for the excess Helmholtz free energy is given in 
Equation~\ref{FexfromSPT} with $\Phi$'s defined in Equations~\ref{Phi1equation} 
through~\ref{oldPhi3equation}, and
and weighted densities defined in Equation~\ref{weighted-density}, 
and reduces to the same result obtained from Scaled Particle Theory for a 

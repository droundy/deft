...
\textcolor{red}{A review of model fluids, and problems studied more 
recently is given in ...}
...
For a hard-sphere fluid with spheres of two different sized radii, 
...
\textcolor{red}{For more recent developments in classical 
Density Functional Theory see (2020), Evan's review~\cite{evansreview} (2016), 
and Theory of Simple Fluids by Hansen and McDonald~\cite{Hansen} (2013).}

FREEZING in literature :
\textcolor{red}{Classical density functional theory was used
to ... (paper from 2010), but not until recently has there been good success 
(other 23 papers...), and in all these the hard-sphere fluid has been 
...
equilibrium), a phase transition occurs, that is, the liquid 
freezes. 
...
\end{align}
For a hard-sphere fluid (simplified in this account by considering only 
equal sized spheres), the Mayer function, $f(r)$ can be written in terms 
of a backward Heaviside step function
\begin{align} 
    f(r) = -\theta(|\vec r|-2R)
\end{align}
where and $\vec r$ is the distance between the centers of two spheres.
The low-density excess free energy given by Equation~\ref{eq:Fexcess} 
can then be written as a weighted density.
\begin{align}
     F_{excess}=k_BT\left(\int n^2(\vec r)\frac{\theta(|\vec r|-2R)}{2}~d\vec r\right) V
\end{align} 
where $n \rightarrow n(\vec r)$ for an inhomogeneous fluid.
Rosenfeld recognized that the Heaviside function describes mathmetically 
...
excess chemical potential can be expanded. He also noted that the 
Heaviside function can uniquely be deconvolved into a set of weight functions
related to the geometry of a sphere.
\begin{align}
    f(r)=-\Theta(|\vec{r}|-2R)= -2(w_3 \otimes w_0 + w_2 \otimes w_1 + \vec{w_2} \otimes \vec{w_1})
\end{align} 
where again this expression is simplified in this account by considering only 
equal sized spheres. 

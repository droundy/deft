...
A liquid will eventually freeze as heat is removed, or the liquid is compressed. 
The state of a simple fluid can be characterized by its entropy per particle~\cite{entropyfingerprint}.
The entropy of a pure substance in a crystalline state is higher than the entropy 
that it would have in its liquid state at a given temperature and pressure, 
since there is more freedom for a particle to move between atoms  
at lattice sites than in a dense liquid with irregular and blocked pathways~\cite{Hansen}.
Both the entropy and the volume (and thus the density) change abruptly 
...
\begin{figure}
    \centering
    \includegraphics[height=8cm]{figs/P-T_Diagram.pdf}
    \caption{Conceptual Pressure-Temperature Phase Diagram}
    \label{fig:P-T_Diagram}
\end{figure}

\textcolor{red}{A phase transition occurs along a line on a 
...
and show that both the solid and the liquid have the same temperature (necessary 
for thermal equilibrium)
and the same pressure (necessary for mechanical 
equilibrium)at a phase transition.
%
, and the same chemical potential (necessary for diffusive equilibrium). 
%
...
\begin{figure}
    \centering
    \includegraphics[height=8cm]{figs/T-n_Diagram.pdf}
...
with the exclusion volume to describe higher order terms in the expansion~\cite{rosenfeld1988}. 
%

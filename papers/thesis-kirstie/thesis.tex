...
\ref{fig:Pvsinvn}. A Temperature-Density (T-n) phase diagram, such as that in Figure \ref{fig:T-n_Diagram}, 
...
always be negative, and thus the free energy tends toward a 
minimum~\cite{schroeder}. This illustrates the tendancy toward minimization 
for a system near equilibrium. A general proof of minimization can be 
found in Theory of Simple Fluids by Hansen and McDonald~\cite{Hansen}.
...
is minimized. The minimum free energy and its corresponding number density 
...
and, that is to use Scaled Particle Theory (SPT).
...
The shaded parallepiped shown within a FCC cell in Figure \ref{fig:primitivecell} 
...
\section{Computing the Liquid and Crystal Ideal Helmholtz free energies}

The liquid ideal Helmholtz free energy $F_{Hideal}$ is computed 
analytically for a given homogeneous reduced density $n^*$, and reduced 
...
\begin{displaymath}{\frac{F_{Hideal}}{V}=k_BTn(\ln(n\Lambda^3)-1.0)}\end{displaymath} 
where $\sigma=1$ (which gives $n^*=n\sigma^3$=n), and $\epsilon=1$ (which 
gives $t^*=\frac{k_BT}{\epsilon}=k_BT$). The quantity $\Lambda$ is the DeBroglie 
wavelength given by
...
The crystal ideal Helmholtz free energy $F_{Cideal}$ is computed from the equation 
...
Unlike the WCA potential, the erf potential depends on temperature, but 
we temperature dependent parameters $\alpha(T)$ and $\Xi(T)$ to compensate for the
variance with temperature. The parameter $\alpha(T)$ is derived by setting 
the erf potential equal to the WCA potential when r equals $\alpha$ 
...
\end{equation} 
%%%%Unlike the WCA potential, the erf potential depends on temperature through the parameters $\alpha$ and $\Xi$. 
Figure \ref{fig:alphaXivsT} shows how 
...
\section{Computing the Liquid and Crystal Excess Helmholtz free energies}

The liquid excess free energy $F_{Hexcess}$ is computed analytically for 
a given homogeneous reduced density $n^*$, and reduced temperature $t^*$ 

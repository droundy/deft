...
written as
\begin{align} 
    f(r) = -\theta(|\vec r_i - \vec r_j|-(R_i + R_j))
\end{align}
where and $|\vec r_i - \vec r_j|$ is the distance between the centers of two spheres.
%
...
this becomes
\begin{align}
       \int_{Volume} \theta(|\vec r_i- \vec r_j|-(R_i+R_j))~d\vec r &=\frac{4\pi}{3}(R_i+R_j)^3  \\
       &= \frac{4\pi}{3}R_i^3 + (4\pi R_i^2)R_j + (4\pi R_j^2)R_i + \frac{4\pi}{3}R_j^3  \nonumber \\
       &= V_i(1) + S_iR_j + R_iS_j+ V_j(1)
\end{align}  
where the volume, surface area, radius, and the number 1 can be written as
\begin{align}
        V &= \int_{Volume} \theta(r-R)d\vec r \\
...
\begin{align}
      F_{excess} &= k_BTn^2\left(-\frac{1}{2}\int{f(r)}{d\vec{r}}\right)V   \\
                 &= k_BTn^2\left(\int \frac{\theta(|\vec r|-2R)}{2}~d\vec r\right)V  \\
                 &= k_BTn^2(2V(1) + 2SR)V  \\
                 &= k_BT\left[\left(\int n\theta(r-R)d\vec r\right)\left(\int n\frac{\delta(r-R)}{4\pi R^2} d\vec r \right)\color{white} \right]   \nonumber \\
                 ...
                 &= k_BT(n_3n_0 + n_2n_1)V \\
                 &= k_BT(\Phi_1 + \Phi_2)V \\
                 &= k_BT\Phi(\{n_i\})V
\end{align} 
where $n_i$ are weighted densities of the form
\begin{align}
     n_i=\int nw_i(r) d\vec r
\end{align}
with weight functions defined as
\begin{align} 
      w_3 &= \theta(r-R)  \label{scalor-weight3} \\
      w_2 &= \delta(r-R)  \\
      w_1 &= \frac{w_2}{4\pi r}  \\
      w_0 &= \frac{w_2}{4\pi r^2} \label{scalor-weight0}
\end{align}
and dimensionless energy densities $\Phi_1$ and $\Phi_2$ are defined as
\begin{align}
      \Phi_1 = n_3n_0  \\
      \Phi_2 = n_1n_2
\end{align}
...
\begin{align}
     F_{excess} = k_BT\int d\vec r_a \int d\vec r_b~n(\vec r_a)~n(\vec r_b)~\frac{\theta(|\vec r_a-\vec r_b|-2R)}{2}
\end{align}
where $\vec r_a$ is a position vector of one sphere, $\vec r_b$ is a position vector to another sphere, and 
$|\vec r_a - \vec r_b|$ is the distance between the centers of the two spheres.
Rosenfeld recognized that the Heaviside function can be written as 
a convolution of the weight functions, 
\begin{align}
...
and the two vector weights are given by
\begin{align}
      \vec {w}_2 &= w_2~\hat r \\
      \vec {w}_1 &= w_1~\hat r
\end{align}
which are needed for the relation to be satisfied (although other expressions 
have been developed without using vector weights). 
...
%
He then noted the following relation
\begin{align}
     w_\alpha(\vec r_a - \vec r_b) \otimes w_\gamma(\vec r_a - \vec r_b) &= \int w_\alpha(\vec r_a - \vec x) w_\gamma(\vec r_b - \vec x) d\vec x \\
                                                                         &= \int w_\alpha(\vec x - \vec r_a) w_\gamma(\vec x - \vec r_b) d\vec x
\end{align}
which simplifies the equation for the excess Helmholtz free energy
\begin{align}
     F_{excess} &= k_BT\int d\vec r_a \int d\vec r_b~n(\vec r_a)~n(\vec r_b)~\frac{\theta(|\vec r_a-\vec r_b|-2R)}{2}  \\
                &= k_BT\int d\vec r_a \int d\vec r_b~n(\vec r_a)~n(\vec r_b)~(w_0\otimes w_3+ w_1 \otimes w_2+\vec {w}_1\otimes \vec {w}_2)  \\
                &= k_BT\int d\vec r_a \int d\vec r_b~n(\vec r_a)~n(\vec r_b)\left(\int w_3(\vec x - \vec r_a)w_0(\vec x - \vec r_b) \vec dx \color{white}\right)  \nonumber \\
                &+ \color{white}\left(\color{black}\int w_1(\vec x - \vec r_a)w_2(\vec x - \vec r_b) d\vec x  + \int \vec w_1(\vec x - \vec r_a)\cdot\vec w_2(\vec x - \vec r_b) d\vec x\right)    \\
                &= k_BT\int \left[\int n(\vec r_a) w_3(\vec x - \vec r_a)~d\vec r_a \int n(\vec r_b) w_0(\vec x - \vec r_b)~d\vec r_b \color{white}\right]  \nonumber \\
                &+ \int n(\vec r_a) w_2(\vec x - \vec r_a)~d\vec r_a \int n(\vec r_b) w_1(\vec x - \vec r_b)~d\vec r_b  \nonumber \\
                &+ \color{white}\left[\color{black} \int n(\vec r_a) \vec {w}_2(\vec x - \vec r_a)~d\vec r_a \cdot \int n(\vec r_b) \vec {w}_1(\vec x - \vec r_b)~d\vec r_b \right]\color{black}d\vec x  \\
                &= k_BT\int n_3n_0 + n_2n_1 + \vec n_1\cdot\vec n_2~d\vec x  \\
                &= k_BT\int \Phi_1 + \Phi_2 + \Phi_3~d\vec x  \\
                & =k_BT\int \Phi(\{n_i(\vec r)\})~d\vec r               
\end{align}
where $\vec n_1\cdot\vec n_2$=0 for the homogeneous case, 
and the weighted densities $n_i$ are defined as
\begin{align}
...
Lifting the restriction to the low density limit,
The expression for the excess Helmholtz free energy derived from FMT
is applicable to the more general case of an inhomogeneous fluid, but reduces 
to the same result obtained from Scaled Particle Theory for a homogeneous 
hard-sphere fluid. 
}

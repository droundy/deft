...
and the cooresponding equilibrium number density profile for 
...
(which can be constructed from SPT~\cite{rosenfeld1989,Hansen,santos2012phi3}) 
to form a functional for the hard-sphere fluid.
FMT is based on the idea that for a hard-sphere fluid the
lowest term in the virial expansion of the excess chemical potential can 
...
a set of weight functions that can be related to the geometry of a sphere.
\begin{displaymath}
   \theta(|\vec r - \vec r'|-(\vec R_i + \vec R_j)) = \end{displaymath}
\begin{equation} w_i^{(3)} x w_j^{(0)} + w_j^{(3)} x w_i^{(0)} + w_i^{(1)} x w_j^{(2)} + w_j^{(1)} x w_i^{(2)} + \vec w_i^{(1)} x \vec w_j^{(2)} + \vec w_j^{(1)} x \vec w_i^{(2)}
\end{equation}
...
In this paper, clasical Density Functional 

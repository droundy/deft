...
for a system near equilibrium. A general proof of minimization 
...
of a WCA fluid is shown below. The program takes four inputs: reduced number 
density n*, reduced temperature t*, a range of Gaussian 
...
We use Soft Fundamental Measure Theory to find the excess Helmholtz free energy.
The functional is written in general form as 
\begin{displaymath}{F_{excess}[n(\vec{r})]= k_BT\int(\Phi_1(\vec{r})+\Phi_2(\vec{r})+\Phi_3(\vec{r}{)) d}\vec{r}}\end{displaymath} 
where
\begin{align}
\Phi_1 &= -n_{0}ln(1-n_{3}) \\
\Phi_2 &= \frac{n_{1}n_{2}-\vec{n_{1}}\cdot\vec{n_{2}}}{1-n_{3}} \\
...
\begin{equation}{n_i(\vec{r})=\int_{Volume}{n(\vec{r'})w_i(|\vec{r}-\vec{r'}|)d{\vec{r'}}}}\end{equation}
where the weight functions $w_0, w_1, w_2,$ and $w_3$ are given by
...
\end{align}
The weight fucntions $w_{2}(r)$ and $w_{3}(r)$ are constructed using 
...
and $a$ with our temperature dependent parameters $\alpha(T)$ and $\Xi(T)$.
...
error function potential $V_{erf}(r)$ which is used to approximate the
...
\begin{equation}{V_{erf}(r=\alpha)=V_{WCA}(r=\alpha)}\end{equation} 
and is given by 
...
the weight function.
 ...
 \begin{figure}[h!]
    ...
    \caption{\color{red}RERUN for NEW XI!\color{black}Shows how the weight functions $w_0$, $w_1$, $w_2$, $w_3$ vary along the 
 ...
 \begin{figure}[h!]
    ...
    \end{subfigure} 
    \caption{\color{red}RERUN for NEW XI!\color{black}Cross-sectional views of the 3D weight functions. (a) $w_2$ is a 

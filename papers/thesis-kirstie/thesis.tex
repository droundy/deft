...
result for the direct correlation function approximation given by Percus-Yevick. 
...
between the atoms. The net attractive energy is maximized at a distance of 
...
than the free energy that it has as a liquid at the same density.
...
where $\phi_{ext}=\frac{V_{ext}}{N}$ and $V_{ext}(\vec r)$
is the externally applied potential energy given in Equation~\ref{Hamiltonian}. 
...
\begin{equation}\label{Hamiltonian}{H = KE + V + V_{ext} = \sum_{i=1}^N\left(\frac{|\vec{p}_i|^2}{2m}\right)+V(\vec{r}_1,\vec{r}_2,{...},\vec{r}_N)+V_{ext}(\vec{r}_1,\vec{r}_2,{...},\vec{r}_N)}\end{equation}
...
that decreases smoothly from infinity over a distance of 2R rather than instanteously 
...
\begin{align}\label{mayerder=w2convolution}
     \frac{df(r)}{dr} = w_2 \otimes w_2(r) = \int{w_2(r')w_2(r-r')dr'}
\end{align} 
...
Unlike the WCA potential, the error function potential $V_{erf}(r)$ is temperature dependent.
To compensate for this temperature dependence so that our functional better approximates
a WCA potential, we make the parameters $\alpha$ and $\Xi$ functions of temperature as well. 

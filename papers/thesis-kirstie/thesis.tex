...
\abstract{
  ...
  using Soft Fundamental Theory (SFMT) with Tarazona's tensor expression 
  for $\Phi_3$, and Schmidt's error function model in which we have 
  incorporated our temperature dependent parameters $\alpha$ and $\Xi$. 
  We find $\Xi$ by making the second virial coefficients of the error 
  function potential and the WCA potential equal. Our functional was 
  successful in predicting the freezing of the WCA fluid with a fairly 
  good match to Monte-Carlo simulations. 
  This result suggests that SFMT may provide a strong foundation for 
  future density functionals that will model real fluids.
}
...
be a fluid, which might be referred to as a "liquid". Points along the 
line that separates the solid and the liquid phases indicate a phase 
transition, and show that the solid and the liquid both have the same 
temperature and the same pressure at a phase transition.
...
We will use cDFT (discussed in the next section) to find the Helmholtz
free energies of the liquid and the solid at a given temperature and 
number density needed to create these curves. 
...
\begin{figure}[h!]
    ...
    \caption{Densities at the solid-liquid transition are identified using 
    Maxwell's Double Tangent Construction}
    \label{fig:MaxwellDT}
  \end{figure}
...
\begin{align}{g_L=g_S\mbox{~~~~and~~~~}P_L=P_S\mbox{~~~~at a phase transition}}\end{align} 
The pressure at the phase transition can be obtained from the point of 
intersection that occurs when the Gibbs free energy per atom, or chemical
potential, is plotted against the pressure for both the liquid and the 
solid at fixed temperature, as shown in Figure~\ref{fig:GibbsvsP}. 
These curves are constructed from data collected from lines tangent 
...
The slope gives -P and corresponding y-intercept gives $g$. The 
curves in Figure~\ref{fig:GibbsvsP} consist of a collection of data 
points ($g$,P) for the liquid and the solid respectively.
...
versus the inverse of the density for the liquid, as shown in Figure 
\ref{fig:Pvsinvn}. The density of the solid at the transition point can 
be found the same way. A Temperature-Density (T-n) phase diagram, such as 
...
%The reason this works is because DFT satisfies the conditions of the 
...
\begin{equation}{Z_{excess}=\frac{1}{V^N}\int{...}\int{dQ}~e^\frac{-V(\vec{r}_1,\vec{r}_2,{...},\vec{r}_N)-V_{ext}(\vec{r}_1,\vec{r}_2,{...},\vec{r}_N )}{k_BT}}\end{equation} 
...
\begin{equation}\label{eq:Fexcess-simplified}{F_{excess}=-k_BT\ln{\left(\frac{1}{V^N}\int{...}\int{dQ}\left[1 + \sum{f_{ij}} + \sum{f_{ij}f_{kl}} +\sum{f_{ij}f_{kl}f_{mn}} +... \right]\right)  }}\end{equation}
  ...
  \begin{figure}[h!]
    ...
    \caption{Two-pair Interactions $f_{ij}f_{kl}$ for N=4 atoms. Interactions 
...
chemical potential of a homogenesous fluid corresponds to the reversible 
...
written for fluids with a cavity radius 
equal to, larger than, or very 
much larger than the hard sphere radius R, Scaled Particle Theory arrives 
...
Fundamental Measure Theory begins by noticing that 
...
\end{align} 
This can easily be checked by evaluating the left side for a Hard-sphere fluid as follows,
...
functions (given by Equations~\ref{eq:w0},~\ref{eq:w1},~\ref{eq:w_v1},
~\ref{eq:w_v2},~\ref{w2_w3_relation},~\ref{eq:tensorweight}) and the 
equation for the derivative of the mayer function 
(given by Equation~\ref{mayerder=w2convolution}) the same, 
but replacing $w_{3}(r)$ with a weight function 
appropriate for a soft sphere potential~\cite{schmidt1999density, schmidt2000fluid}. 
...
region, is found mixtures of various amounts of liquid and solid.  
Within the coexistence region, the liquid and solid have number densities 
that are fixed at the values they had at the phase transition. To construct
the phase diagram, these liquid and solid phase transition number densities
...
free energies verses the inverse density at a given temperature. 
To construct these plots we use classical Density Functional Theory 
...
program run faster.
  ...
  \begin{figure}[h!]
    ...
    \caption{The Primitive Crystal Cell is shown by the shaded parallelepiped}
...
Of course, atoms are always in motion, so when talking about "the location" 
...
cell that has one lattice site (represented by a black dot the center of 
the cell). The atom is slightly off the lattice site in each example, and 
a sampling of many possibilities - more than just the four given 
as examples - would create an average profile that looks like a Gaussian
...
3-dimensional Gaussian distributions centered at lattice points can be 
...
how the atoms are distributed on average over a collection of cells. 
 ...
 \begin{figure}[h!]
    ...
    \caption{Four examples of possible positions an atom (blue sphere) 
    might have within a cell that has one lattice site (black dot) at 
    the center are shown on the left. A large number of such 
...
\begin{displaymath}{ n = (1-f_v){n_{no.vacancies}}}\end{displaymath} 
 ...
 \begin{figure}[h!]
    ...
    \caption{Four examples of possible positions an atom (blue sphere) might 
    have within a fixed volume containing eight cells are shown on the left. 
    Each cell has one lattice site at the center represented by a black dot. 
    A large number of such possibilities would give rise to the Gaussian 
    distributions shown on the right.}
...
By varying the width of the Gaussians and the fraction of vacancies, 
various number density profiles can be generated for a given combination 
of temperature and number density, as shown in Figure~\ref{fig:Ensemble_vary}.
A homogeneous number density profile is used for a liquid is shown in 
...
density profile, and also for the homogeneous number density profile 
...
Soft Fundamental Measure Theory (SFMT) is a method
...
Our SFMT functional is taylored to fit a WCA fluid, and 
...
There are four scalar weight functions, two vector weight functions, and 
...
The scalar weight functions are given by
...
The two parameters $\alpha$ and $\Xi$ make temperature-dependent error 
function potential $V_{erf}(r)$ approximate in the region of interest 
of the temperature-independent WCA potential which is difficult 
to implement computationally. $\Xi$ is determined by setting the second virial 
coefficient $B_2$ of the error function potential $V_{erf}(r)$ and of the
WCA potential equal.  
...
Using Equation~\ref{w2_w3_relation},
\begin{displaymath}
...
the error function potential $V_{erf}(r)$ given by Equation~\ref{eq:Verf} 
equal to the WCA potential given by Equation~\ref{eq:VWCA} when $r=\alpha$. 
This gives
...
We determine the value of $\Xi$ at a given temperature $T$,
by equating the second virial coefficient evaluated for the error 
function potential $V_{\operatorname{erf}}(r)$, and the second virial 
coefficient evaluated for the WCA potential $V_{WCA}$.
...
the internal energy of a system and is given by
\begin{equation}B_2=-\frac{1}{2}\int_{Volume}f(\vec{r})d\vec r\end{equation}
As shown in detail in the Appendix, evaluating the second virial coefficient 
$B_{2}$ for the error function potential $V_{erf}$ with the mayer function 
given by Equation~\ref{eq:VerfMayerfunction} 
gives 
...
\end{equation}
The parameter $\Xi$ is found to be the value that makes Equations~\ref{eq:B2erf} 
...
Phase transitions between the liquid and solid phases will occur when the liquid 
and solid are at the same temperature, pressure and chemical potential. 
Looking at one temperature, it is seen that as the density of the 
liquid increases, a maximum number density is reached for the liquid. 
This is the liquid phase transition number density, and 
marks the beginning of a coexistence region where various amounts 
of liquid and solid can exist together in equilibrium.
Moving on toward higher number densities, the end of the coexistence 
region is marked by the minimum number density that the solid can have.
This is the solid phase transition number density. At a phase transition 
there is an abrupt change in the number density as seen here where the
density changes suddenly, and abruptly from the liquid 
phase transition number density to the solid phase transition number density.
Within the coexistence region, the number density of the liquid is fixed 
at the liquid phase transition number density, and the number density of 
the solid is fixed at the solid phase transition number density, while
varying amounts of liquid and solid combine to give the infinite number 
of possible densities seen within the coexistence region. 

\begin{figure}
  \centering
  \includegraphics[height=9cm]{figs/Phase_Diagram_of_T_vs_n}
  ...
  region generated from Monte-Carlo of the WCA fluid.}
...
\begin{figure}
  \centering
  \includegraphics[height=9cm]{figs/Phase_Diagram_of_P_vs_T}
  \caption{Phase Diagram of the reduced pressure verses the reduced 
  temperature. The blue region is solid, and the red region is liquid.
  The white line marks the solid-liquid boundary as predicted by 
...
and indicates the various temperature-pressure combinations at which a 
phase transition occurs, and where various amounts of solid and liquid 
can coexist. 
...
The slope of the phase boundary $\frac{dP}{dT}$ at some point (P,T) is given by
\begin{align} 

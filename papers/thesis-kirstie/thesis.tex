...
energy for the particular fluid being modeled 
from Equation~\ref{eq:Fexcess-simplified} can be challenging. 
...
and will be adapted in the Methods section of this paper to fit the 
soft-sphere WCA fluid.
...
The resulting equation is known as Widom's Insertion Formula.
...
Considering that at each point in the fluid, there is a probability that 
either a hard sphere of radius R could be inserted, or not, and using 
these two probablities to evaluate $<e^{-\beta{V_{diff}}}>$ this becomes
\begin{equation}{~}=-k_BT\ln\left(e^{-\beta{V_{diff}}}+1\right)\end{equation}
\begin{equation}{~}=V_{diff}=W_{rev}(R)\end{equation}
Returning to Widom's insertion formula Equation~\ref{widoms-insertion-formula},
and considering that it takes no energy for a hard sphere to be placed at a point
in the fluid where it does not overlap with any other spheres, but would require
infinite energy to place a sphere where it would overlap another sphere, 
Equation~\ref{widoms-insertion-formula} becomes
\begin{equation}\mu_{excess}=-k_BT\ln(1-n\frac{4\pi}{3}R_{cavity}^3)\end{equation}
where the probability of a position existing in a homogeneous Hard-sphere fluid 
with number density n that a hard sphere of radius R can be placed without 
overlapping another sphere is given by the number density times the volume 
of the sphere, $nV_{Sphere}$.
...
by equating the second virial coefficient, $B_2$, evaluated for the error function 
...
%
 \begin{figure}[h!]
    \centering
    %\includegraphics[height=6cm]{GaussandW2_actual.png}
    \includegraphics[height=7cm]{weightandGuass1.pdf}
    %\caption{A Gaussian centered at $\vec{R}$ is shown in blue, and a cross-sectional 
...
%
 \begin{figure}[h!]
    ...
    \includegraphics[height=7cm]{weightandGuass2.pdf}

...
with one another like billiard balls%~\cite{kinetictheory}
. This model, called the Hard-Sphere fluid, has served as 
the primary reference system for fluids. This is due to the fact that the repulsion between atoms is 
much stronger than their attraction, and so the repulsive potential, which is descriptive of hard spheres, 
dominates the interactions between atoms. 

%Van der Waal took into account both the attraction, and the respulsion 
%of the impenetrable spheres in devopling his equation of state for an ideal gas in 1xxx.

In the late 1950s, Scaled Particle Theory (SPT) was developed~\cite{Reiss Frich Lebowitz} from which an equation of state, and
an expression for the excess Helmholtz free energy of a homogeneous Hard-Sphere fluid can be obtained~\cite{Hansen}. 
%Percus and Yevick took another approach to the Hard-Sphere fluid.
In 1961 Morita and Hiroike introduced a density functional approach to finding an expression for the excess Helmholtz free energy of inhomogeneous fluids~\cite{MoritaDFT}.
% ?  Percus-Yevick applied Density Functional Theory (DFT) to find an expression for the excess Helmholtz free energy of an inhomogeneous Hard-Sphere fluid. 
In 1967 %the late 1960s, and early 1970s, 
Barker and Henderson~\cite{barker1967perturbation} adapted perturbation methods to use with fluids composed of 
spheres with softly fading potential functions rather than the hard edges of the spheres 
in the Hard-Sphere fluid. This was done by making the radius of the spheres temperature dependent, 
so that at low temperatures the Soft-Sphere fluid could be modeled by hard spheres with large radii, 
and at high temperatures the Soft-Sphere fluid could be modeled by hard spheres with smaller radii. 
%A larger radii cooresponds to a deeper level of penetration of two soft spheres into each other when they collide at higher temperatures.
%The different size radii coorespond to the different depths to which two soft spheres would penetrate into each other when they collide. 
This reflects the idea that soft-spheres penetrate more deeply into each other when they collide at higher temperatures. 
Weeks, Chandler, and Anderson studied the soft repulsive portion of the Leonard-Jones potential~\cite{andersen1971relationship}, 
which became known as the Weeks-Chandler-Anderson model fluid. 

DFT Evans 1979

In 1989 Rosenfeld introduced Fundamental Measure Theory (FMT)~\cite{rosenfeld1989} which provides an expression for the excess 
Helmholtz free energy of an inhomogeneous Hard-Sphere fluid. 
Tarazona later improved on the method by introducing a tensor version of the functional~\cite{tarazonaphi3}. 
In 1999 Schmidt adopted FMT for soft spheres forming Soft Fundamental Measure Theory (SFMT)\cite{schmidt1999density}. In this paper, 
Density Functional Theory will be used to predict the freezing of a Weeks-Chandler-Anderson model fluid 
by applying SFMT with Schmidt's proposed erf model\cite{schmidt2000fluid}. %for generating the weight funcions, 
%and Tarazona's tensor functional. 
...
%In this paper a system with a fixed temperature, volume, and number of particles will be examined in which case it is the Helmholtz energy as a functional of the number density profile $F[n(\vec r)]$ that is of interest. 
...
\begin{equation}df_{N-System}=-\left(\frac{T}{N}\right)dS_{System+Surroundings} \end{equation}which, for fixed N, is the same as
...
where the radius of the cavity $R_{cavity}$ is equal to the radius of a hard sphere, R, and the cavity is either filled or not. \color{black}%For cavities that are much larger than one of the hard spheres, 
...
%\cite{schmidt2000fluid, sweatman2002fundamental}
\cite{weeks1971}
%\cite{rosenfeld1989}
%\cite{schmidt1999density}
...
%\cite{schmidt1999density} 
%\cite{schmidt2000fluid}
%\cite{schmidt1999density, schmidt2000fluid}
%\cite{schmidt1999density}
%\citenum{schmidt2000fluid}
%\citenum{schmidt2000fluid}
%a Face Centered Cubic (FCC) lattice is formed~\cite{reference!}

...
properties, or to address particular problems like surface tension, 
interfaces, adsorption, wetting or confinment.
\textcolor{red}{A review of some model fluids and problems that have been
...
Percus (1963)~\cite{lobowitzandpercus}, and Mermin (1965)~\cite{merium}.
To use classical Density Functional Theory (cDFT), an expression for the free energy 
as a functional of a spatially varying average single-particle number density 
...
\textcolor{red}{In 1989 Rosenfeld introduced Fundamental Measure Theory (FMT)~\cite{rosenfeld1989,rosenfeld1988} 
...
\begin{displaymath}
   \theta(|\vec r - \vec r'|-(R_i + R_j)) = w_i^{(3)} \otimes w_j^{(0)} + w_j^{(3)} \otimes w_i^{(0)} + w_i^{(1)} \otimes w_j^{(2)} + w_j^{(1)} \otimes w_i^{(2)}\end{displaymath}
...
Tarazona later solved this problem by introducing a tensor version of 
...
confinment is reduced from a three-dimensional volume (as in a bulk fluid), 
...
with soft, short-range repulsive and attractive potentials, and developed 
...
\textcolor{red}{For some more recent developments, see the review of new developments in 
classical Density Functional Theory by Evans ~\cite{evansreview} (2016), 
Theory of Simple Fluids by Hansen and McDonald~\cite{Hansen} (2013), 
and a recent generalization of the Barker-Henderson model to inhomogeneous 
fluids by Tschopp, Vuijk, Sharma and Brader~\cite{BHextended} (2020).}

\textcolor{red}{Classical density functional theory has also been applied 
to phase transitions, and freezing, but not until recently has there
been good progress. Ramakrishnan and Yussouff studied freezing by viewing a 
crystal as a highly inhomogeneous liquid~\cite{} (1979). Oettel and fellow 
researchers studied hard-sphere crystals with ~\cite{} (2010). Mortazavifar and Oettel
studied the fluid and crystal phases of the Asakura-Oosawa model fluid (2016). 
Lutsko and Lam studied studied crystallization of the Leonard-Jones fluid~\cite{lutsko} (2018),
and Lutsko and Schoonen looked at cDFT applied to the solid state~\cite{2020}.
In all these studies the hard-sphere fluid has been used as the reference system,
which is implemented with FMT, and to which other terms are added to create 
...
the density will change abruptly at the phase transition.
The particular structure and density that result are affected 
...
A phase transition occurs along a line on a 
Pressure-Temperature (P-T) phase diagram, such as can be seen in the phase 
diagram shown in Figure~\ref{fig:P-T_Diagram}. 
%
...
be a fluid.\textcolor{red}{liquid in this paper.}
The phase diagram indicates the equilibrium state a substance will be in at
different temperatures and pressures.
At a phase tranition, both the fluid and the solid have 
...
density at fixed temperature. 
 ...
 \begin{figure}
    ...
    \caption{Fluid and solid densities at the solid-fluid phase 
...
At the point of phase transition the chemical potential of the fluid is 
...
fluid and the solid at a given temperature. 
...
\begin{figure}
    ...
    the number densities of the fluid and the solid during a phase 
...
attractive interatomic potentials, and which represents the region where
vapor coexists with liquid as evidenced by an interface.  
...
of a backward Heaviside step function which, for a hard-sphere fluid 
composed of a mixture of spheres of radius $R_i$ and $R_j$, is
given by
\begin{align} 
    f(r) = -\Theta(r-(R_i + R_j))
\end{align}
where $r=|\vec r_i - \vec r_j|$ is the distance between the centers of two spheres.
...
where
\begin{align}
    V(r)=\left\{ \begin{array}{rc} \infty & 0<r \leq R_i+R_j \\ 0  & r> R_i+R_j \end{array}\right.
\end{align}
%
...
of two hard spheres added together. For a mixture of hard spheres of radius $R_a$ and $R_b$
this becomes
\begin{align}
       \int_{Volume} \Theta(r-(R_i+R_j))~d\vec r &=\frac{4\pi}{3}(R_i+R_j)^3  \label{exclusion-volume}\\
       &= \frac{4\pi}{3}R_i^3 + (4\pi R_i^2)R_j + R_i(4\pi R_j^2) + \frac{4\pi}{3}R_j^3  \nonumber \\
       &= V_i(1) + S_iR_j + R_iS_j+ V_j(1)
\end{align}  
where the volume, surface area, radius, and the number 1 can be written as
\begin{align}
        V &= \int_{Volume} \Theta(r-R)d\vec r \\
        S &= \int_{Volume} \delta(r-R) d\vec r \\
        R &= \int_{Volume} \frac{\delta(r-R)}{4\pi r} d\vec r \\
                1 &= \int_{Volume} \frac{\delta(r-R)}{4\pi r^2} d\vec r   
\end{align}
Limiting this discussion to a homogeneous hard-sphere fluid composed of hard spheres that have the same
radius R, the lowest order term of the virial expansion of the excess Helmholtz free energy which is given in Equation~\ref{eq:Fexcess} 
...
\end{align}
with weight functions defined as
\begin{align} 
      w_3 &= \Theta(r-R)  \label{scalor-weight3} \\
      w_2 &= \delta(r-R)  \\
      w_1 &= \frac{w_2}{4\pi r}  \\
      w_0 &= \frac{w_2}{4\pi r^2} \label{scalor-weight0}
\end{align}
...
$n\rightarrow n(\vec r)$, Equation~\ref{eq:Fexcess} becomes
\begin{align}
...
position vector to another sphere, and $|\vec r_i - \vec r_j|$ 
...
\begin{align}
    f(|\vec r_i-\vec r_j|)=-\Theta(|\vec r_i-\vec r_j|-2R)= -2(w_3 \otimes w_0 + w_2 \otimes w_1 + \vec{w_2} \otimes \vec{w_1})
\end{align} 
%where the weights are the same ones given in Equations~\ref{scalor-weight3} through~\ref{scalor-weight0}, 
...
%
The convolution of the weight functions can be written as
\begin{align}
...
\end{align}
For the scalor weights this is the same as
\begin{align} 
                                                   &= \int w_\gamma(\vec u - \vec r_i)w_\alpha(\vec u - \vec r_j)  d\vec u  
\end{align}
and for the vector weights this becomes
\begin{align} 
                                                   &= -\int \vec {w}_\gamma(\vec u - \vec r_i)\cdot\vec {w}_\alpha(\vec u - \vec r_j)  d\vec u  
\end{align}
%
...
\begin{align}
     F_{excess} &= k_BT\int d\vec r_i \int d\vec r_j~n(\vec r_i)~n(\vec r_j)~\left(\frac{-f(|\vec r_i-\vec r_j|)}{2}\right)  \\
                ...
                &= k_BT\int d\vec r_i \int d\vec r_j~n(\vec r_i)~n(\vec r_j)~(w_3\otimes w_0+ w_2 \otimes w_1+\vec {w}_2\otimes \vec {w}_1)  \\
                &= k_BT\int d\vec r_i \int d\vec r_j~n(\vec r_i)~n(\vec r_j)\left(\int w_0(\vec u - \vec r_i)w_3(\vec u - \vec r_j) d\vec u \color{white}\right)  \nonumber \\
                &+ \color{white}\left(\color{black}\int w_1(\vec u - \vec r_i)w_2(\vec u - \vec r_j) d\vec x ~-\int \vec {w}_1(\vec u - \vec r_i)\cdot\vec {w}_2(\vec u - \vec r_j) d\vec u\right)    \\
                &= k_BT\int \left[\int n(\vec r_i) w_0(\vec u - \vec r_i)~d\vec r_i \int n(\vec r_j) w_3(\vec u - \vec r_j)~d\vec r_j \color{white}\right]  \nonumber \\
                ...
                &= k_BT\int n_0n_3 + n_1n_2 - \vec n_1\cdot\vec n_2~d\vec u  \label{Fexlowden-inhomo}\\
                &= k_BT\int \Phi_1 + \Phi_2 + \Phi_3~d\vec u  \\
                &= k_BT\int \Phi(\{n_\alpha(\vec u)\})~d\vec u  \\
                %&\text{or replacing u notation with r} \nonumber \\
                &= k_BT\int \Phi(n(\vec r))~d\vec r             
\end{align}
where the weighted densities $n_\alpha$ have the form
\begin{align}
    n_\alpha(\vec u)=\int n(\vec r')w_\alpha(\vec u-\vec r')~d\vec r'  
    %n_\alpha(\vec u)=\int n(\vec r')w_\alpha(\vec u-\vec r')~d\vec r'  
\end{align}  
For the homogenesous case $\vec n_1\cdot\vec n_2$=0, and Equation~\ref{Fexlowden-inhomo} reduces to Equation~\ref{Fexlowden-homo}.
...
"exclusion volume" which is given by Equation~\ref{exclusion-volume}, 
and an "overlap volume".
...
terms with $f_{ij}=f(|\vec r_i - \vec r_j|)$ as has been seen. The overlap volume is 
the volume of intersection between two spheres that overlap, and is used in conjunction 
with the exclusion volume to describe higher order terms in the expansion
in Equation~\ref{eq:Fexcess-simplified}~\cite{rosenfeld1988}. 
%
The resulting expression for the excess Helmholtz free energy with an 
updated version of $\Phi_3$ that was later constructed by Tarazona is given by
\begin{align}
...
\end{align}
where in addition to the weight functions already defined, a tensor weight function is given by
...
\end{align}
This result reduces to same result obtained from Scaled Particle Theory 
...
Taking the derivative of the Mayer function provides 
another important relation expressed in terms of $w_{2}(r)$ 
%that will be important 
%to keep in tact when moving to SFMT~\cite{schmidt2000fluid}.
...
Once the Helmholtz free energy for both the crystal and fluid states of 
the fluid are found for a given temperature and density, they can be 
used to construct phase diagrams.
Outside the region of coexistence which exists between the fluid and 
crystal states, the fluid will be completely in a crystalline state if the 
crystal free energy is lower than the fluid free energy, and it will be 
completely in a fluid state if the fluid free energy is lower than 
the crystal free energy. 
We determine the actual number densities of the crystal and fluid states
...
fluid increases, a maximum number density is reached for the fluid. 
...
\begin{figure}
  ...
  and the black line shows the phase boundary where the solid and fluid

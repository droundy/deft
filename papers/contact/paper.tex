...
\documentclass[letterpaper,twocolumn,amsmath,amssymb,10pt,pre]{revtex4-1}
...
evaluated at contact, which is required for both the chain and
...
correlation function of Yu and Wu\cite{bryk2006density} and another that
...
\end{align}
The contact value of the correlation function is this
correlation function's value when evaluated at a distance of the
diameter $\sigma$.  In a homogeneous fluid, the pair correlation only depends on the
distance $|\rr_1-\rr_2|$ and can be expressed as a function of a
single variable. The pair correlations for an inhomogeneous fluid are of course
not as simple but it is desirable for reasons of computational efficiency and theoretical elegance
to limit these classical density functionals
to one-center convolutions, which leads us to seek a
...
value for $g_\sigma$ that is dependent upon only one position variable $\rr$.
There are two reasonable options for defining
...
Each sphere will be in contact with spheres that are one diameter away
from its center so we expand a number of contact area spheres
of radius $\sigma$ and divide by two.  We can thus readily compute the derivative of free energy with respect
to hard-sphere diameter, which is half the force on \emph{all} the
hard spheres---since we're changing all their diameters at once.  To
compute the pressure on the spheres, we need to divide the total force
by the total area of contact which means we divide by $N 4\pi \sigma^2$.
\begin{align}
  p_\sigma &= \frac{1}{N 4\pi \sigma^2} \frac12 \frac{dA_{HS}}{d\sigma} \\
...
the appendix.  The equation for $g_\sigma^A$ requires
...
We will show here the derivation of the $\beta\frac{\delta n_3^{A}(\mathbf{r}')}{\delta \sigma(\mathbf{r})/2}$, etc., terms for the asymmetric case.  The derivation of the S correlation follows the same format using the symmetric conceptual approach introduced in \ref{sec:g-S} and the $\frac{\delta A_{HS}}{\delta n(\mathbf{r}')}$, etc., terms are straightforward (if tedious) derivatives.

The derivation of $g_\sigma^A(\rr)$ requires that we express our weighted
densities so that the radius of the weighting kernel is determined according
to the position of the sphere in question, the location where the density
itself is evaluated.  Thus for the derivative of $n_3^A$, we have
\begin{align}
...
\begin{align}
  ...
  2 \frac{n(\rr)}{\pi
    \sigma(\rr)^3}\delta(\sigma(\rr)/2 -\left|\rr' - \rr\right|)
\end{align}
The vector-weighted densities have no additional occurrences of $\sigma$.  Their functional derivatives come out similarly:
\begin{align}

...
\end{align}
The functional derivative $2 \frac{\delta A_{HS}}{\delta \sigma(\mathbf{r})}$
...
$2 \frac{\delta A_{HS}}{\delta \sigma(\mathbf{r})}$ in Equations \ref{eq:g-A-exact} 
...
These qualitative concepts lead us to the derivative terms in $2 \frac{\delta A_{HS}}{\delta \sigma(\mathbf{r})}$,
...
\begin{align}
  ...
  &= \frac{1}{n_0(\rr)^2}\frac{1}{ k_BT 2\pi \sigma^2} \frac{\delta
    A_{HS}}{\delta \sigma(\mathbf{r})} \label{eq:g-A-exact}
\end{align}
where the functional derivative $2 \frac{\delta A_{HS}}{\delta
...
In our expressions for pressure (eq \ref{eq:p_{HS}^A} and eq \ref{eq:p_{HS}^S}) we have $2 \frac{\delta A_{HS}}{\delta \sigma(\mathbf{r})}$ terms that must be derived for both the S and A cases.  We break these terms up as follows:
...
We must therefore compute both the $\frac{\delta A_{HS}}{\delta n(\mathbf{r}')}$, etc., terms and the $2 \frac{\delta n(\mathbf{r}')}{\delta \sigma(\mathbf{r})}$, etc., terms.  We will compute the $\frac{\delta A_{HS}}{\delta n(\mathbf{r}')}$, etc., terms for both the functional derived in the White Bear I formulation and for that in the White Bear II formulation.  Following this we will compute the $2 \frac{\delta n(\mathbf{r}')}{\delta \sigma(\mathbf{r})}$, etc., terms for both the A and S cases.  These latter derivitives will differ for the A and S cases according to the qualitive concepts described above.
...
\begin{align}
  2 \frac{\delta n_3^{A} (\rr')}{\delta \sigma(\rr)} &=
  \int n (\rr'') \delta(|\rr' - \rr''| - \sigma(\rr'')/2) \delta(\rr-\rr'') d\rr'' \\
   &= n (\rr) \delta(|\rr' - \rr| - \sigma(\rr)/2)
\end{align}
Similarly, for $n_2^A$, we find
\begin{align}
  n_2^{A}(\rr') &= \int n(\rr'') \delta(|\rr' - \rr''| - \sigma(\rr'')/2) d \rr''\\
  2 \frac{\delta n_2^{A}(\rr')}{\delta \sigma(\rr)} &= -n(\rr) \delta'(|\rr' - \rr| - \sigma(\rr)/2)\\
\end{align}
...
\begin{align}
  ...
  2 \frac{\delta n_1(\rr')}{\delta \sigma(\rr)}
  &= -\frac{n(\rr)}{2\pi
...
\begin{align}
  ...
  \delta(|\rr'-\rr''| - \sigma(\rr'')/2) \\
  2 \frac{\delta n_0(\rr')}{\delta \sigma(\rr)}
  &= -\frac{n(\rr)}{\pi
    \sigma(\rr)^2}\delta'(|\rr'-\rr| - \sigma(\rr)/2)
  -
  4 \frac{n(\rr)}{\pi
...
\begin{align}
  ...
  2 \frac{\delta \mathbf{n}_{V2}^{A}(\rr')}{\delta \sigma(\rr)} &= n(\rr) \delta'(|\rr' - \rr| - \sigma(\rr)/2)
    \frac{\rr-\rr'}{|\rr-\rr'|}\\
\end{align}
...
\begin{align}
  ...
  2 \frac{\delta \mathbf{n}_{V1}^A(\rr')}{\delta \sigma(\rr)}
  &= \frac{n(\rr)}{2\pi
...
\begin{align}
  2 \frac{\delta n_3^{S} (\rr')}{\delta \sigma(\rr)} &=
   \delta(\rr'-\rr) \int n (\rr'') \delta(|\rr' - \rr''| - \sigma(\rr')/2)
   \delta(\rr'-\rr) d\rr''
   \\
...
takes a simple form:
\begin{align}
  \int 2 \frac{\delta A_{HS}}{\delta n_3^{S}(\rr')}
  \frac{\delta n_3^{S}(\rr')}{\delta \sigma(\rr)} d\rr'
  &= \int 2 \frac{\delta A_{HS}}{\delta n_3^{S}(\rr')} \delta(\rr'-\rr)
  n_2^S(\rr') d\rr' \\
  &= 2 \frac{\delta A_{HS}}{\delta n_3^{S}(\rr)} n_2(\rr)
\end{align}
...
\begin{align}
  ...
  r''\\
  2 \frac{\delta n_2^{S}(\rr')}{\delta \sigma(\rr)} &= -\delta(\rr-\rr') \int n(\rr'')
  \delta'(|\rr'-\rr''| - \sigma(\rr')) d\rr''
\end{align}
...
\begin{align}
  n_1^{S}(\rr') &= \int \frac{n(\rr'')}{2\pi \sigma(\rr')} \delta(|\rr' - \rr''| - \sigma(\rr')/2)d\mathbf r''\\
    2\frac{\delta n_1^{S}(\rr')}{\delta \sigma(\rr)} &=
    -\delta(\rr-\rr') \left( \int \frac{n(\rr'')}{2\pi \sigma(\rr)}
    \delta'(|\rr'-\rr''| - \sigma(\rr')/2) d\rr''
    + n_0^s(\rr') \right)
\end{align}
\begin{align}
  n_0^{S}(\rr') &= \int \frac{n(\rr'')}{\pi \sigma(\rr')^2} \delta(|\rr' - \rr''| - \sigma(\rr')/2)d\mathbf r''\\
    2 \frac{\delta n_0^{S}(\rr')}{\delta \sigma(\rr)}
    &= -\delta(\rr-\rr') \left( \int \frac{n(\rr'')}{\pi \sigma(\rr)^2}
    \delta'(|\rr'-\rr''| - \sigma(\rr')/2) d\rr''
    +
    \frac{4}{\sigma} n_0^S(\rr') \right)
\end{align}
\begin{align}
  \mathbf{n}_{V2}^{S}(\rr') &= \int n(\rr'') \delta(|\rr' - \rr''| - \sigma(\rr')/2)
  \frac{(\rr' - \rr'')}{|\rr' - \rr''|} d \rr''\\
  2 \frac{\delta \mathbf{n}_{V2}^{S}(\rr')}{\delta \sigma(\rr)} &= -\delta(\rr - \rr')
  \int n(\rr'') \delta'(|\rr' - \rr''| - \sigma(\rr')/2)
  \frac{(\rr - \rr'')}{|\rr - \rr''|} d\rr''
\end{align}
\begin{align}
 \mathbf{n}_{V1}^{S}(\rr') &= \int \frac{n(\rr'')}{2\pi \sigma(\rr')} \delta(|\rr' - \rr''| - \sigma(\rr')/2)d\mathbf r''
   \frac{(\rr' - \rr'')}{|\rr' - \rr''|}\\
 2 \frac{\delta \mathbf{n}_{V1}^{S}(\rr')}{\delta \sigma(\rr)}
 &= -\delta(\rr-\rr')\left(\int \frac{n(\rr'')}{2\pi \sigma(\rr)}
   \delta'(|\rr'-\rr''| - \sigma(\rr')/2) \frac{(\rr - \rr'')}{|\rr - \rr''|}  d\rr''
   + \frac{\mathbf{n}_{V2}^S(\rr')}{\pi \sigma(\rr)^2} \right)
\end{align}

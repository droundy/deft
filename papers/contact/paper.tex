...
several subsequent functionals\cite{fu2005vapor-liquid-dft,
  bryk2006density}.  A key input (FIXME: term, factor?) to this
functional is the contact value of the correlation function of a
hard-sphere fluid.

The contact value of the correlation function---also known as the
cavity correlation function---is the the value of the
correlation function evaluated a distance equal to the hard-sphere
diameter.  Yu and Wu introduce an approximation based on the
fundamental measures of Fundamental Measure Theory (see
Section~\ref{sec:yuwu}).  In this paper, we will report on the
accuracy of this approximation by comparing with Monte Carlo
simulations of the inhomogeneous hard-sphere fluid.  In addition, we
will introduce and test differing approximations for the cavity

...
\begin{align}
  ...
  g_\sigma^A(\rr) &= \frac{\ncontact(\rr)}{n_A(\rr)} \label{eq:g-A-exact-a}\\
...
for $g_\sigma^A$, which is given in the Appendix.  This
...
For the computation of the symmetrically averaged correlation function 
we must formulate derivatives with respect to $R$ in a symmetric fashion.  
Conceptually we expand the radii of spheres that just touch a specific 
point of contact and only expand them outward at that point of contact.  
Therefore $R(\mathbf{r})$ in our calculations refers to the radii of 
spheres that touch $\mathbf{r}$ and $\delta R(\mathbf{r})$ refers to a 
change of these radii at only the point $\mathbf{r}$.  One can imagine 
a shell that touches $\mathbf{r}$ that has an infinitesimal bit of shell 
area protruding outward at $\mathbf{r}$.  Figure~\ref{fig:contact} 
illustrates such a change in $R$.  The full derivations are found in Appendix~\ref{appendix:g-S} 
...
as in the assymmetric case we will instead divide by $n_0(\mathbf{r})^2$ 
...
\begin{align}
  ...
  \ncontact(\rr) &= \frac{1}{n_0(\rr) k_BT 4\pi \sigma^2} \frac{\delta
    A_{HS}}{\delta R(\mathbf{r})} \\
  g_\sigma^A(\rr) &= \frac{\ncontact(\rr)}{n_0(\rr)} \label{eq:g-S-exact-a}\\
  g_\sigma^A(\rr) &= \frac{1}{n_0(\rr) n_0(\rr)}\frac{1}{ k_BT 4\pi \sigma^2} \frac{\delta
    A_{HS}}{\delta R(\mathbf{r})} \label{eq:g-S-exact}

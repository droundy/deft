...
In the expression for our A correlation function $g_\sigma^A(\rr)$ (eq \ref{eq:g-A-exact}) we have $\frac{\delta A_{HS}}{\delta
  \sigma(\mathbf{r})}$ terms that must be derived.  We break these terms up as follows:
...
We will show here the derivation of the $\beta\frac{\delta n_3^{A}(\mathbf{r}')}{\delta \sigma(\mathbf{r})/2}$, etc., terms for the assymetric case.  The derivation of the S correlation follows the same format using the symmetric conceptual approach introduced in \ref{sec:g-S} and the $\frac{\delta A_{HS}}{\delta n(\mathbf{r}')}$, etc., terms are straightforward (if tedious) derivatives.

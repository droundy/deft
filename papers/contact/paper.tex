...
options for defining such a local function:  a symmetric (which we refer to as S) and an
asymmetric (which we refer to as A) formulation.

For the S case:
\begin{align}
  g^S_\sigma(\rr) &= \frac{n^S_\textit{contact}(\rr)}{n_S(\rr)}
\end{align}
\begin{align}\label{eq:n-contactS}
  n^S_\textit{contact}(\rr) &= \frac1{4\pi R^2}\int n^{(2)}(\rr - \rr', \rr + \rr')\delta(|\rr'| -
  R)d\rr'
\end{align}
\begin{align}
  n_S(\rr) &= \frac{1}{4\pi R^2}\int n(\rr')\delta(|\rr-\rr'|-R) d\rr'
\end{align}
...
\begin{align}
  ...
  n^A_\textit{contact}(\rr) &= \frac1{16\pi R^2}\int n^{(2)}(\rr, \rr + \rr')
  \delta(|\rr'| - 2R)d\rr' \\
  n_A(\rr) &= \frac1{16\pi R^2}\int n(\rr') \delta(|\rr-\rr'|-2R) d\rr'
\end{align}
...
we also defined four averaged densities, two \emph{contact
  densities}, $n^S_\textit{contact}(\rr)$ and $n^A_\textit{contact}(\rr)$,
and two \emph{contacting densities}, $n_S(\rr)$ and $n_A(\rr)$.  The
...
contact.  In the homogeneous limit, $n_S = n_A = n$ and
$n^S_\textit{contact} = n^A_\textit{contact} = g(\sigma)n$.  It is an
...
A_{HS} &= \int \left\{
    -n_0 \ln\left(1 - n_3\right)
    + (1 + \frac{1}{9}n_3^2 \phi_2(n_3))   \frac{n_1 n_2 - \mathbf{n}_{V1} \cdot\mathbf{n}_{V2}}{1-n_3}
...
We also have
\begin{align}
  \frac{\delta A_{HS}}{\delta n_2} &= (1 + \frac{1}{9}n_3^2\phi_2(n_3))\frac{n_1}{1-n_3} + (1 - \frac{4}{9}n_3\phi_3)(\frac{3n_2^2 - 3\mathbf{n}_2 \cdot \mathbf{n}_2}{24\pi(1 - n_3)^2})\\
  ...
  - (1 - \frac{4}{9}n_3\phi_3)\frac{6\mathbf{n}_2}{24\pi(1 - n_3)^2}
\end{align}

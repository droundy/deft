...
the surface tension by a factor of about 1.5, and are unsuited for studying
systems with molecular-scale inhomogeneities.

\subsection{Contact-value theorem}

The contact value theorem states that the pressure of a fluid at a
hard wall interface is proportional to the contact density of the fluid,
$n_c$.  Specifically,
\begin{equation}\label{eq:contact}
  P = n_ck_BT
\end{equation}
where $k_B$ is the Boltzmann constant and $T$ is the temperature of the
fluid. This leads to the property that the free energy of solvation of
...
theorem and its limiting case of small solutes (Equation
\ref{contactvaluethm}). These properties are retained by the total
functional, provided the remaining terms are \emph{purely nonlocal}.
...
data plotted in Figure \ref{fig:energy-vs-diameter}.}

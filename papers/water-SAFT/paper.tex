...
over-structuring\cite{lin2009importance, wang2011density,
  mogelhoj2011ab, jonchiere2011van}.  It has also been bound that
inclusion of nuclear quantum effects can have a similar
effect\cite{morrone2008nuclear}.  Each of these corrections imposes an
additional computational burden on an approach that is already
feasible only for very small number of water molecules. A more
...
%% describe water as non-interacting molecules in a an
...
%% predict the nonlinear dielectric response in water, but it must be paramaterized
...
We construct a new classical density functional for water.  We
start with a treatment of each individual water molecule as a purely
classical hard sphere and add association terms based on Statistical 
...
in the liquid\cite{muller2001molecular}. SAFT assumes a repulsive hard
sphere core for each atom with a number of short-ranged off center
attractive sites. This model can be applied to give an equation of state for
various systems even inhomogeneous fluids.
\fixme{Expand this a
...
\begin{equation}\label{contactvaluethm}
  F = n k_BT V\:.
\end{equation}\fixme{may be ambiguous about n}
...
\fixme{Expand or add to this description of density profiles and
...
from Figure ~\ref{fig:rods-energy-vs-distance} (144~mN/m) which
...
the four separate rods, which results in a critical separation of
\begin{equation}

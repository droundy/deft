...
similar to our hydrophobic rods, by using molecular dynamics with
Lennard-Jones potentials for nanotubes of diameter 1.25~nm and
separations ranging from about 0.3~nm to 1.5~nm. They compute the
force to constrain the nanotubes at different sepearation distances
and find that a drying transition begins at separations near 0.9~-~1.0~nm where two
layers of water molecules can exist in a metastable state between the
rods. This reduces to one layer at a spacing of 0.55~-~0.58~nm, and at
0.35~nm water is complete expelled and is in
equilibrium\cite{walther2004hydrophobic}. The final drying state is
smaller than what we find for hydrophobic rods of a similar size, as
expected due to the carbon-water interaction and the
van der Waals interaction between the rods that causes more attraction
than our rods.  Our model does not predict any of the transitional
states seen here\fixme{why?}.

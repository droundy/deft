...
int main(int argc, char *argv[]) {
  ...
  sprintf(datname, "papers/hughes-saft/figs/rods-in-water-%04.1fnm.dat", diameter/nm);
  
  FILE *o = fopen(datname, "w");
  //FILE *o = fopen("papers/hughes-saft/figs/rods-in-water.dat", "w");
  ...
  for (distance=0*nm; distance<1.6*nm; distance +=0.05*nm) {
    ...
    // constraint.epsNativeSlice("papers/hughes-saft/figs/rods-in-water-constraint.eps",
    ...
    potential = hughes_water_prop.liquid_density*constraint
      + 500*hughes_water_prop.vapor_density*VectorXd::Ones(gd.NxNyNz);
    ...
    //    //density.epsNative1d("papers/hughes-saft/figs/constrained-water-1D.eps",
    //    //			Cartesian(0,0,0), Cartesian(0,0,zmax),
    //    //			hughes_water_prop.liquid_density, 1, " ");
    ...
    Minimizer min = Precision(precision, PreconditionedConjugateGradient(f, gd, hughes_water_prop.kT,
                                                                         &potential,
    ...
    potential2 = hughes_water_prop.liquid_density*(constraint2.cwise()*constraint)
      + 400*hughes_water_prop.vapor_density*VectorXd::Ones(gd.NxNyNz);
    potential2 = -hughes_water_prop.kT*potential2.cwise().log();

    Minimizer min2 = Precision(1e-12, PreconditionedConjugateGradient(f, gd, hughes_water_prop.kT,
                                                                     &potential2,
                                                                     QuadraticLineMinimizer));
    ...
    snprintf(plotnameslice, 1024, "papers/hughes-saft/figs/rods-slice-%04.1f-%04.1f.dat", diameter/nm, distance/nm);
    ...
    if (min.energy() < min2.energy()) {
      ...
      Grid density(gd, EffectivePotentialToDensity()(hughes_water_prop.kT, gd, potential2));
    ...
    //double totalentropy = S.integral(hughes_water_prop.kT, potential)/width;
    ...
    // sprintf(plotname, "papers/hughes-saft/figs/rods-picture-%04.1f-%04.1f.dat", diameter/nm, distance/nm);

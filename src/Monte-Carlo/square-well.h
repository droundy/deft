...
bool overlap(const ball &a, const ball &b, const double periodic[3], double dr=0);
...
int overlaps_with_any(const ball &a, const ball *p, const double periodic[3],
                      bool count=false, double dr=0);

// Return true if p doesn't intersect walls
bool in_cell(const ball &p, const double walls[3], bool real_walls, double dr=0);

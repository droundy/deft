...
pub struct Expr<T: ExprKind> {
    inner: Intern<GenericOp<T>>,
}
impl<T: ExprKind> Copy for Expr<T> {}
impl<T: ExprKind> Clone for Expr<T> {
    fn clone(&self) -> Self {
        Expr { inner: self.inner.clone() }
    }
}
/// This lets you compare `Expr`s for equality.
impl<T: ExprKind> PartialEq for Expr<T> {
    fn eq(&self, other: &Self) -> bool {
        self.inner == other.inner
    }
}
// This lets `Expr`s be keys in `CommutativeMap`s.
impl<T: ExprKind> Fits64 for Expr<T> {
    fn to_u64(self) -> u64 {
        self.inner.to_u64()
    }
...
pub trait ExprKind {
    type Op: std::hash::Hash+Eq+PartialEq+Clone+std::fmt::Debug;
}

enum GenericOp<T: ExprKind> {
    AddOp(CommutativeMap<T>),
    SpecificOp(T::Op),
}

#[derive(Eq, PartialEq, Clone, Debug, Hash)]
struct RealSpace;
impl ExprKind for RealSpace {
    type Op = ();
}
#[derive(Eq, PartialEq, Clone, Debug, Hash)]
struct Scalar;
impl ExprKind for Scalar {
    type Op = ScalarOp;
}

#[derive(Eq, PartialEq, Clone, Debug, Hash)]
struct AddOp<T: ExprKind>(CommutativeMap<T>);
...
impl<T: ExprKind, RHS: Into<Expr<T>>> std::ops::Add<RHS> for Expr<T> {
    type Output = Self;
    ...
    fn add(self, other: RHS) -> Self {
        ...
        match (&self.inner.op, &other.inner.op) {
            (&T::Op::Add(ref lhs), &T::Op::Add(ref rhs)) => {
                sum = lhs.clone();
                sum.union(&rhs);
            },
...
#[derive(Debug, Clone)]
struct CommutativeMap<T: ExprKind> {
    inner: Map64<Expr<T>, f64>,
}

impl<T: ExprKind> CommutativeMap<T> {
    fn new() -> Self {
...
impl<T: ExprKind> std::hash::Hash for CommutativeMap<T> {
...
// interned, but only `CommutativeMap` needs a manual implementation.
impl<T: ExprKind> PartialEq for CommutativeMap<T> {
    fn eq(&self, other: &Self) -> bool {
...
impl<T: ExprKind> Eq for CommutativeMap<T> {}

...
such as the comparatively high melting, boiling, and critical temperatures.
The large dielectric constant (D~80) (CITE) is partially responsible for
...
(Previous/popular models of water summary/review??)
...
probability.  In the SAFT-VR model\cite{gil-villegas-1997-SAFT-VR}, $\Delta$ is given by
\begin{align}
...
in experiment due to the difficulty of creating such an ideal situation. The second
possibility is a large sphere brought close to a plane, which eliminates the 
need for perfect alignment.

We choose a chemical potential corresponding to a pressure of 1 atm and we apply 
a constraining potential where the density is equal to the saturated 
liquid density inside the slit. Outside the slit, we choose a small non-zero
density for numerical reasons. The slits studied range in size from 0.1 nm to 
8.0 nm. We minimize the energy and calculate density, energy, and $X$ on a lattice
...
of oscillations supports this. For larger slits (6.0 nm or more) the
...
0.6 nm (\textcolor{red}{Check this again}. For smaller slits, the initial 
liquid water inside will vaporize.
...
\end{center}
\caption{ Energy densities for a slab of water with molecules 
constrained to move in one dimension. \textcolor{red}{FIXME}}
\label{fig:energy-1D}
\end{figure}
...
\end{center}
\caption{ Free energy versus slit size for a slab of water with molecules 
constrained to move in one dimension. \textcolor{red}{Not sure if this needs
to have a bulk energy amount removed to be accurate.}}
\label{fig:cavitysize-vs-energy-1D}
\end{figure}

The value of $X$ behaves as expected for water constrained to a slit. We
show in Figure \ref{fig:xassoc-1D} the plot of $X$ for a slit of 4.0 nm.
Outside the slit, there is no hydrogen bonding ($X$ = 1), and inside $X$ is 
approximately equal to 0.13, or that of saturated liquid water.
...
tension. (FIXME - Data points are not correct yet) }

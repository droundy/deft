...
For comparison, we arbitrarily offset each free energy such that they all go 
to zero at large separations. For large $d$, the rods will 
...
per length on the rods is constant, and surprisingly not dependent on the separation. 
By finding the slope, we calculate this force per length to be 144~mN/m.
...
If we use a classical approximation where the energy on the rod is equal to 
the surface area multiplied by the bulk surface tension, then the energy per 
...
radius, we will use this classical approximation and propose that 

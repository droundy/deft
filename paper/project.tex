...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
 {We develop a classical density functional theory for
...
\end{center}
\caption{ Density profiles illustrating the transition from vacuum 
to liquid water between the rods. The top figure is at a separation of 0.2nm and the
bottom is 0.4nm. }
...
0.3nm to 1.2nm. All were arbitrarily set to go to zero at large separations. The
discontinuous transition corresponds to the phase change from
vacuum to liquid between the rods. }
...
separation for two hydrophobic rods of radii 0.5nm. All energies are arbitrarily
...
use hydrophobic rods of various radii at separation $d$ (see Figure
\ref{fig:rods}), ranging from $d=0$ (touching rods) to about 1nm. At small
separations ($d<$2nm-4nm) there is only vacuum between the rods. As the rods are
pulled apart, the vacuum region expands until some critical separation is
reached and liquid water fills the region between the rods. Figure
\ref{fig:density-rods} shows density profiles before and after this transition
for rods of radius 0.5nm. Since we are looking at only hard-walled
non-interacting rods, the critical separation for this transition will be
smaller than any system where there is an attractive force between rods. Walther
\emph{et al.}\cite{walther2004hydrodynamic} study the interactions between two
carbon nanotubes. They use molecular dynamics with Lennard-Jones potentials for
nanotubes of diameter 1.25nm and separations ranging from about 0.3nm to 1.5nm.
They also find a drying transition occurs, and find it to be at separations less
than 0.9-1.0nm\cite{walther2004hydrodynamic}. This is several times larger than
what we find for hydrophobic rods, as expected due to the carbon-carbon
interations between two nanotubes. The density oscillations are very similar to
those for the single hydrophobic rod in water, as seen in Figure
\ref{fig:density-single-rod}.

The free energy per length is calculated for several different radii and is plotted
in Figure \ref{fig:rods-energy-vs-distance}. Each data set is adjusted such that the
free energy per length goes to zero at large separations. Once a certain separation is
reached, the rods will simply act like the single rod in water and the energy will
reach an asymptote proportional to the surface tension (as in Figure \ref{fig:energy-vs-diameter}).


For one set of rods (radii of 0.5nm) the free energy is plotted with the internal energy
and the entropic energy contribution, where all are in units of energy per length (see Figure
\ref{fig:energy-rods}). 

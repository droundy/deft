...
for these nanoscale rods (0.3~-~1.0~nm), we find that in the
...
will include van der Waals and other non-associative interations. We use a
...
\end{align}
where $a_1$ and $a_2$ \fixme{change all to $a_1$ and $a_2$} are the first 
two terms in a high-temperature
...
values for $\lambdadisp$ and filling fractions.  Here $g_\sigma^{HS}$
is the hard-sphere fluid correlation function evaluated at contact and uses the
effective filling fraction for inhomogeneous density distributions.
The $\zeta$ factor
...
the water molecules in the slit act like the bulk. \fixme{Either add brief
description of bottom half of Figure~\ref{fig:energy-1D} or delete it}
Based on our computations, we
...
(touching rods) to $d=1$~nm. At small
...
on the radii of the rods, and is about 0.55~nm for the rods shown in 
Figure~\ref{fig:density-rods}. 
Since we are looking at only hard-walled
non-interacting rods, the critical separation will be
different for any system where there is an appreciable force between rods. 
...
solid stadium shaped object (approximately like a rectangle with semi-circles on the ends).
...
0.3~nm to 1.0~nm.
All were arbitrarily set to go to zero at large separations. The
...
in Figure~\ref{fig:rods-energy-vs-distance}. In this figure, the energy for a radius 0.5~nm
is the same as the free energy shown in Figure~\ref{fig:energy-rods}.
For comparison, we arbitrarily adjust each data set such that the
...
We see that all the rods studied have the same behavior, with a positive linear relation 
between free energy
per length and separation during the initial state where the rods have vapor between them. 
The slope of this line is equal to the force per length on the rods, which we calculate to 
be 144~mN/m, or twice the surface tension
found by the functional (see Figure~\ref{fig:surface-tension}). This indicates that
even though the rods are nanoscale, to the water molecules they can be seen classically.
To estimate how the critical transition depends on the radius,
we assume a classical approximation where the energy on the rod is equal to the
surface area multiplied by the bulk surface tension. The surface area is just the
circumference multiplied by the width, and this width has the same value for both the
vapor-filled stadium shape and the liquid-filled separate rods. We propose that 
the critical separation
for the transition, $d_c$, will occur when the circumference of the stadium shape (see 
top of Figure~\ref{fig:density-rods}) and the circumference of the two rods (see bottom
of Figure~\ref{fig:density-rods}) are equal:
\begin{align}
C_{stadium} &= C_{rods} \\
...
Table~\ref{criticalseparationtable},
and they match fairly well for the 0.5~-~1.0~nm rods. We hypothesize that
larger radii rods
...
theory with SAFT is able to produce accurate results for hydrophobic interations.
\fixme{decribe any results for slit?}
We added the parameter \lscale to the functional and fit it to the surface tension
at room temperature, which we find to be consistent with results for hydrophobic
rods. The free energy per area for water surrounding a hydrophobic rod shows the
expected behavior with an asymptote equal to the surface tension.
Based on this result, we expect to be
able to describe classically rods with a radius of about 0.5~nm or larger. Even
though this is still nanoscale, from the perspective of the water molecules
the interaction with a hydrophobic rod is easily approximated classically.
For the interactions between water and two hydrophobic rods, we see that a
classical treatment of the phase transition critical separation works well
for rods larger than 0.5~nm, within 5\%. This transition from a vapor-filled
stadium shape to two rods with liquid between them (see Figure~\ref{fig:density-rods})
is commonly seen for objects of a similar geometry. Further evidence for the
...
This functional produces many of the results of \emph{ab initio} calculations
while being much more computationally efficient. This allows for future work
involving hydrophilic interactions with more complex electrostatic potentials.
\fixme{more about efficiency or future work?}

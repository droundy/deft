...
vapor density, liquid density, surface tension, enthalpy and
...
\fixme{at $(\pi-2)r$?} from a vapor state to a liquid state in the area between
the rods. Even for rods of a small radius (0.3~-~0.9~nm), we find that for the
...
perturbation expansion and $\beta=\frac{1}{k_BT}$.  $A_1$ is the mean-field dispersion
...
\fixme{add analysis of circumferences of stadium shape vs two rods and
calculating the expected critical separation}
...
0.3~nm to 0.9~nm. \fixme{r=0.9 to be added when it finishes!}
...
even though the rods are nanoscale, to the water molecules they can be treated classically
\fixme{reference above equations/explanation with circumference, critical separation, etc.}
...
\needsworknow{Conclusions about slit, single rod, two hydrophobic rods (mostly). 
computationally efficient. future uses}
